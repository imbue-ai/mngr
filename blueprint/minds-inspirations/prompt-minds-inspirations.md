...
- The worker runs `build_inspiration.sh`: **reset to the FCT version the mind was based on** (clean base — no upstream fetch; the reset must drop tracked-but-not-in-base files), overlay only the selected paths, strip secrets, and make one commit; rewrite the `/welcome` stable region for the latest inspiration; run a boot smoke-check (must boot; selected apps may have holes).
