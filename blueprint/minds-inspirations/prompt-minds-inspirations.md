...
- Confirm **inline in chat** (no popup): the agent presents the editable title, description, repo name, private/public choice, and the thumbnail embedded as a markdown image. GitHub access goes through **latchkey permissioning** (`github-rest-api` / `github-write-repos`, approved in the minds app) — the `gh` CLI is banned; repo creation and the `minds-inspiration` topic go through the REST API via `latchkey curl`, and the git push authenticates with the mind's standard `GH_TOKEN` (absent → the agent stops and says so).
