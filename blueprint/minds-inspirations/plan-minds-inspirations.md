...
- Deliverables:
  ...
  - **Latchkey GitHub permissioning** (no `gh` CLI): the agent self-initiates a `github-rest-api` / `github-write-repos` permission request the user approves in the minds app; repo creation, description, and the `minds-inspiration` topic go through the REST API via `latchkey curl`; the git push authenticates with the mind's standard `GH_TOKEN` (absent → the agent stops and says so).
...
- GitHub access goes through latchkey permissioning (the `gh` CLI is banned from the flow): the agent probes the API via `latchkey curl`, self-initiates a `github-rest-api` / `github-write-repos` permission request the user approves in the minds app, creates the repo (name + description + visibility) and sets the `minds-inspiration` topic via the REST API, and pushes with git authenticated by the mind's standard `GH_TOKEN` (latchkey never exposes raw tokens to the container; if `GH_TOKEN` is absent the agent stops and says so). The chat confirmation embeds the thumbnail as a markdown image.
- On confirmation, `/publish-inspiration` always creates a **new** GitHub repo (private by default; public if the user says so) via the REST API through `latchkey curl` (name + description + visibility in one call), then pushes the assembled branch with git authenticated by the mind's `GH_TOKEN`.
...
- **GitHub access** goes through latchkey permissioning, like every other connector: the agent probes `latchkey curl https://api.github.com/user`, initiates a `github-rest-api` / `github-write-repos` permission request itself when needed (approved by the user in the minds app), and performs all API operations (repo create, description, topic) via `latchkey curl`. The git push is not an HTTP call latchkey can inject into, so it authenticates with the mind's standard `GH_TOKEN`; if that is absent, the agent reports it and stops. The `gh` CLI is banned from the flow.
...
- Verify manually by exercising the flow inside a running mind (per minds-dev-workflow), not via pytest:
  ...
  - Publish from a mind without GitHub permission; confirm the agent initiates the latchkey request, the user approves it in minds, the API calls succeed, and the push works via `GH_TOKEN` (or the agent stops with a clear message when `GH_TOKEN` is absent).
