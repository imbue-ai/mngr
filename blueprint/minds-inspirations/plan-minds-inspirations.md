...
- Deliverables:
  ...
  - **Latchkey GitHub permissioning** (no `gh` CLI): the agent self-initiates the permission requests the user approves in the minds app — `github-rest-api` / `github-write-repos` for repo creation, description, and the `minds-inspiration` topic via `latchkey curl`, and `github-git` / `github-git-write` for the push itself, which runs through the latchkey gateway's native git smart-HTTP proxying. No GitHub token ever enters the container.
