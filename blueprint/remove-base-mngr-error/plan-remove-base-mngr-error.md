...
> * Delete the tests that exist to document the old two-tier hierarchy (`mngr/errors_test.py::test_consolidated_errors_are_mngr_errors`, `mngr_robinhood_claude/errors_test.py::test_robinhood_claude_errors_are_mngr_errors`) outright; scrub any live docs of the distinction (only changelog files still mention `BaseMngrError`, and those stay as historical records)
...
- `imbue/mngr_robinhood_claude/cli.py:71` -- `except BaseMngrError` -> `except MngrError`; update import.
- `imbue/mngr_robinhood_claude/orchestrator.py` -- `:298,376` `except BaseMngrError` -> `except MngrError`; `:698,702` `except (OSError, BaseMngrError)` -> `except (OSError, MngrError)`; update import; reword the `BaseMngrError` comments at lines ~233 and ~270 to reference `MngrError`.
- `imbue/mngr_robinhood_claude/errors_test.py` -- delete `test_robinhood_claude_errors_are_mngr_errors`; remove imports that become unused.
...
- `libs/mngr/changelog/`, `libs/imbue_common/changelog/`, `libs/mngr_robinhood_claude/changelog/`, `libs/mngr_tutor/changelog/`, `libs/mngr_lima/changelog/`, `libs/mngr_ovh/changelog/`.
