...
- **WebSockets keep working.** Terminal (ttyd), the proto-agent log stream, and the `/api/ws` state socket become `wss` over the TLS connection. hypercorn advertises RFC 8441 (`SETTINGS_ENABLE_CONNECT_PROTOCOL`), so Chromium carries these WebSockets over the h2 connection as Extended CONNECT rather than as a separate `http/1.1` upgrade. This means hypercorn's ASGI `websocket.receive` events (which always include both `text` and `bytes` keys, one set to `None`) reach the proxy's client→backend forwarder, so that forwarder must select the payload by value, not key presence — a binary frame carries `text: None` alongside its bytes. (WebSockets were never the constrained resource — they have a much higher browser connection limit.)
