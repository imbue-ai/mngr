...
## REMAINING ITEM 2 — remove the legacy `FullDiscoverySnapshotEvent` entirely

**Status:** the streaming pipeline is fully per-provider, but `mngr list` still writes a global snapshot as a side-effect (read by completion/resolution; ignored by the migrated stream consumers). It is **not** a hang vector (`list_agents` already uses per-provider `ErrorBehavior.CONTINUE`). The user wants it gone.
...
3. Delete `FullDiscoverySnapshotEvent`, `DISCOVERY_FULL`, `make_full_discovery_snapshot_event`, `write_full_discovery_snapshot`, `run_discovery_stream` + its now-orphaned helpers, `find_latest_full_snapshot_offset` (if only legacy after step 2 — but **check `tail_discovery_events_file`** below), `extract_agents_and_hosts_from_full_listing` (if unused), the `FullDiscoverySnapshotEvent` branch in `_replay_discovery_events_into_maps`, and the type from the `DiscoveryEvent` union.
4. Remove the defensive `FullDiscoverySnapshotEvent` guards in `forward_cli.py` + the aggregator + the latchkey comment.
...
6. **Tests** (the bulk of the churn): `discovery_events_test.py` (~30 tests use `write_full_discovery_snapshot`/`make_full_discovery_snapshot_event`/`find_latest_full_snapshot_offset`/`run_discovery_stream` — convert to per-provider or delete the legacy-specific ones, e.g. `test_full_discovery_snapshot_event_parses_legacy_lines_*`); `list_test.py` (assert per-provider side-effect); `complete_names_test.py` + `complete_test.py`; any `mngr_forward` tests asserting cached-snapshot-on-attach.
