...
- Spec files carry no test references; the coordinate is the handle external consumers use to refer to a unit. The test-side annotation convention (a `witnesses` pytest marker), its registration, and any derivation tooling are all follow-up work — nothing in this branch links tests to specs.
...
- Changelog entries for every touched project: `apps/minds` (update the existing entry), `dev` (new — skill, blueprint plan).
- Out of scope: any test-to-spec backlinks (the `witnesses` marker, its registration, annotations on existing tests, the `testing-overview.md` pointer); matrix/gap derivation tooling; process skills that consume this reference; spec content for any area beyond the authentication exemplar.
...
1. Delegate a Fable subagent to read the skill and the now-obsolete `authentication.md`, complete the migration into `apps/minds/specs/authentication/`, and ground it against the existing tests and code behavior (judgment-based verification only). No test-to-spec backlinks are added.
