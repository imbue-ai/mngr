...
if [ "$DRY_RUN" = true ]; then
    ...
    echo -e "  Existing tmux sessions, Modal environments, and crontab entries"
    echo -e "  still use the old prefix. To keep using them, add this to"
    echo -e "  ${CYAN}~/.mngr/config.toml${NC}:"
    echo ""
    echo -e "    ${CYAN}prefix = \"mng-\"${NC}"
    echo ""
    echo -e "  Or use ${CYAN}MNGR_PREFIX=mng- mngr <command>${NC} for one-off commands."
fi
