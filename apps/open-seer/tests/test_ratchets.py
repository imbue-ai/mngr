...
def test_prevent_bare_print() -> None:
    # scripts/setup_debug_access.py is a CLI whose stdout/stderr IS its
    # interface: the public key goes to stdout for piping into the key
    # registry, instructions to stderr. Logging would break both.
    rc.check_bare_print(_DIR, snapshot(9))
...
def test_prevent_broad_exception_catch() -> None:
    # The tick's two top-level fault barriers (mirror sync, sweep dispatch):
    # a failure in one job must be logged and must not kill the cron or the
    # other job. Nothing narrower is correct at that boundary.
...
def test_prevent_silent_decode_error_catches() -> None:
    # parse_agent_list's JSONDecodeError handler is not silent: it reparses
    # the same text as JSONL (mngr list emits either shape) and any real
    # corruption still raises from the per-line json.loads.
...
def test_prevent_inline_imports() -> None:
    # tick.py's guarded `import modal`: modal exists only on the deployed
    # image, and tests/local one-shot ticks must import the module without it.
...
def test_prevent_bare_generic_types() -> None:
    # Sentry issue payloads are schemaless JSON blobs passed through
    # verbatim; there is no meaningful value type to declare.
...
def test_prevent_monkeypatch_setattr() -> None:
    # tests/test_tick.py fakes requests.get and subprocess.run at the module
    # boundary. tick.py deliberately has no monorepo imports (it is a
    # standalone Modal script), so the DI seams the rule prefers are not
    # available without adding abstraction to a file specced to stay a page.
    rc.check_monkeypatch_setattr(_DIR, snapshot(20))
...
def test_prevent_direct_subprocess() -> None:
    # tick.py shells out to git and `mngr create` by design (no monorepo
    # imports; ConcurrencyGroup is unavailable), and the secrets-gate tests
    # invoke the real scanner binaries the same way the fixer skill does.
