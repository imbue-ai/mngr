...
The sweep treats legacy aggregate manual-bug-report issues (one Sentry issue holding many unrelated user reports under a shared fingerprint) as human-triage territory: dropped from the batch like regressed issues, noted in the summary. Individually-fingerprinted bug reports remain normal input.
