...
- Changed: Raised the `supertokens-python` floor from `>=0.27.0` to `>=0.31.3` so a `uv lock --upgrade` keeps it at the latest 0.31.3 instead of backtracking to 0.30.3 (which would cap `aiosmtplib<4`).
