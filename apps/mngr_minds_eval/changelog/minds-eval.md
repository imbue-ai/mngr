...
- `clean-modal-workspaces` now cleans the shared Modal env host-side (no box) via `scripts/modal_nuke.py` -- it stops every Modal app and deletes the state volumes, entirely SSH-free. The previous path went through `mngr destroy`, which SSHes into each sandbox first and silently fails to tear down workspaces created by a box with a different key (leaving them running while reporting success).
