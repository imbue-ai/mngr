...
- Added `minds-evals list-modal-workspaces` (every workspace in the shared Modal env + each running box's memory) and `minds-evals view-modal-workspace <name>`, which opens a workspace by tunneling **directly** to it: it reads the sandbox's SSH endpoint from `mngr list`, then opens a host-side `ssh -L` to the workspace UI's fixed `:8000` using the shared key already on the host, and prints `http://localhost:<port>/`. No `mngr forward` in the path -- viewing is O(1) per workspace (never scales with env size, never OOMs) and branch-agnostic. If the sandbox is stopped/paused it restarts it first (streaming progress; `--no-restart` skips); a `DESTROYED` workspace errors. `--box` / `--new-box-on-mngr-branch` choose which box's Minds to read.

- The box no longer OOMs from minds' built-in forward. minds eagerly proxies every workspace in the env (a heavyweight `mngr event` subprocess each), so `box.ensure` kills that forward once right after boot -- branch-agnostic (no mngr/minds change; minds doesn't respawn it and the dashboard/list is independent of it). Boxes also run with a `--memory` cap, and stale (exited) boxes are pruned on `ensure` so they don't accumulate. Box output drops the old unreliable forward-login URL (removed `_forward_login`) and prints only the dashboard (http).
