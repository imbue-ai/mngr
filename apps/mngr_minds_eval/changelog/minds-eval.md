...
- Added `minds-evals list-modal-workspaces` (every workspace in the shared Modal env, plus each running box's memory) and `minds-evals view-modal-workspace <name>` (opens a **scoped** `mngr forward` for one workspace and prints a self-authenticating http login URL). The scoped forward proxies a single workspace, so viewing stays cheap no matter how many workspaces the env holds -- unlike the box's built-in forward, which eagerly proxies the whole env and OOMs past ~20 live workspaces. `view-modal-workspace` picks the least-loaded running box by default; `--box <container>` targets a specific box, `--new-box-on-mngr-branch <branch>` spins a fresh one, and `--service` selects which workspace service to forward (default `system_interface`).
