...
- Added `minds-evals list-modal-workspaces` (every workspace in the shared Modal env + each running box's memory) and `minds-evals view-modal-workspace <name>`. Both read the box's Minds API -- the same discovery the dashboard uses, and the source of truth for what workspaces exist (`GET /api/v1/workspaces`) and their SSH endpoints (`POST /api/v1/workspaces/<id>/ssh`, which authorizes our shared key and returns the sandbox address). `view` then opens a plain host-side `ssh -L` to the workspace UI's fixed `:8000` using that shared key and prints `http://localhost:<port>/` -- O(1) per workspace (never scales with env size) and branch-agnostic. A stopped/paused sandbox is restarted first (streaming progress; `--no-restart` skips); a `DESTROYED` workspace errors. `--box` / `--new-box-on-mngr-branch` choose which box's Minds to read. (Both commands surface a real error if discovery is still starting, rather than reporting an empty env.)

- Boxes run with a `--memory` cap (so a runaway process can't take down the whole Docker VM) and prune stale (exited) eval boxes on `ensure` so they don't accumulate. Box output drops the old unreliable forward-login URL and prints only the dashboard (http) plus the view hint. (An earlier attempt to kill minds' built-in `mngr forward` at boot to save memory was reverted: that forward *is* minds' discovery pipeline, so killing it left the dashboard stuck on "Discovering agents" and every list/view empty. Discovery is left intact; the memory cap bounds the blast radius instead.)

- `clean-modal-workspaces` now cleans the shared Modal env host-side (no box) via `scripts/modal_nuke.py` -- it stops every Modal app and deletes the state volumes, entirely SSH-free. The previous path went through `mngr destroy`, which SSHes into each sandbox first and silently fails to tear down workspaces created by a box with a different key (leaving them running while reporting success). It runs the script non-interactively (`--force`) and with `TERM=dumb`, since modal 1.4.x bleeds ANSI color codes into `list --json` output even when piped, which would otherwise break the script's JSON parse.
