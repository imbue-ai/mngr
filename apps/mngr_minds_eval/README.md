...
- **Per-batch Modal envs** — every batch gets its own Modal env (`minds-staging-<batch-slug>`, via
  the Modal provider's `user_id`). A box only ever discovers its own batch's workspaces, so
  discovery stays small and fast. The env, the mngr SHA, and the branch are recorded in the batch's
  S3 config — which is what makes `visit-batch` exact.
...
minds-evals inspect combined_20260715-005237-076811

# score a finished batch (S3 + Anthropic only, no box); writes results back to S3
ANTHROPIC_API_KEY=sk-ant-... minds-evals evaluate combined_20260715-005237-076811

# rebuild the batch's exact Minds computer and enter its desktop in your browser
minds-evals visit-batch combined_20260715-005237-076811
...
`launch` builds/boots a headless box for the batch (pinned to the branch tip SHA), creates the
workspaces inside it, then removes the box — the workspaces self-run on Modal and write to S3.
