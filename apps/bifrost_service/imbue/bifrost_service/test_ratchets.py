...
def test_prevent_direct_subprocess() -> None:
    # Bifrost is a Go binary run as a subprocess. One call site launches it
    # (start_bifrost_subprocess); two more are forward-reference type
    # annotations (subprocess.Popen[bytes]) on helper function signatures.
    # ConcurrencyGroup cannot be used here because app.py is a self-contained
    # Modal deploy with no monorepo imports.
...
def test_prevent_init_methods_in_non_exception_classes() -> None:
    # BifrostAdminClient owns an httpx.Client constructed from runtime-only
    # config (base_url, admin_token) -- matches the HttpCloudflareOps pattern
    # in remote_service_connector. FakeBifrostAdminClient inherits from it
    # and overrides __init__ to skip the real HTTP client setup.
