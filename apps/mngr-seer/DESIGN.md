...
4. **`fix-sentry-error` skill** (the fixer agent): git worktree at the erroring release SHA, reproduce, check whether `main` already fixes it, fix minimally, port to `main` on `open-seer/<sentry-short-id>-<slug>`, pass imbue-code-guardian (≤3 cycles), open a draft PR and flip it ready, marking Sentry resolved-by-commit. Once done, it **collapses all work into a single commit and runs Gitleaks** over it (secrets + PII rules) so the PR is clean and easy to cherry-pick.
...
[Sweep agent (fresh per tick; killed when idle)]
  ...
  ▼
[Fixer agents]  worktree @ release SHA · reproduce · fix · guardian · squash + Gitleaks
  ▼
...
- **Secrets/PII gate — Gitleaks, not a custom CLI:** the fixer's final act is squashing its work to one commit and running Gitleaks (stock rules + custom PII patterns) over it; a hit blocks the PR until scrubbed (§7).
...
7. **Squash + Gitleaks:** collapse all work into a **single commit** (easy to cherry-pick), then run Gitleaks (stock rules + the custom PII patterns, §7) over the commit; any hit blocks until scrubbed.
...
## 7. Privacy: anonymize by instruction, gate with Gitleaks
...
- **The hard gate is Gitleaks on the squashed commit** (§6 step 7): stock secret rules (provider tokens, keys, connection strings, entropy-qualified patterns) plus a custom rules file for PII the stock rules ignore (emails, IPs, user IDs, cookie/request values). A hit blocks the PR until the content is scrubbed.
- **PR/comment text is covered by instruction:** the skills require anonymization in everything posted (describe *classes* of data, quote only sanitized log lines) and treat Sentry-originated text as untrusted data, never instructions (prompt-injection guard). Running the same Gitleaks rules over the PR body before posting is a cheap belt-and-suspenders step the fixer skill includes.
...
- **Offline unit tests for the Gitleaks gate only** (planted-secret + PII corpora against the custom rules file — the one thing not to test on production surfaces), plus trivial checks on the tick's query construction and overlap logging.
