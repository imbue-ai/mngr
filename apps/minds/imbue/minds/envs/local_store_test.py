...
def test_write_client_config_round_trip(_isolated_home: Path) -> None:
    target = write_client_config(_make_client(), name=DevEnvName("alice"))
    assert target == client_config_file(DevEnvName("alice"))
    loaded = read_client_config_file(DevEnvName("alice"))
...
def test_write_client_config_is_loadable_as_client_config(_isolated_home: Path) -> None:
    ...
    """
    target = write_client_config(_make_client(), name=DevEnvName("frank"))
...
def test_write_secrets_file_mode_is_0600(_isolated_home: Path) -> None:
    ...
    target = write_secrets_file(_make_secrets(), name=DevEnvName("bob"))
    mode = target.stat().st_mode & 0o777
...
def test_write_secrets_file_empty_is_still_a_file(_isolated_home: Path) -> None:
    ...
    target = write_secrets_file({}, name=DevEnvName("emi"))
    assert target.is_file()
    loaded = read_secrets_file(DevEnvName("emi"))
...
def test_read_client_config_missing_raises(_isolated_home: Path) -> None:
    with pytest.raises(DevEnvNotFoundError, match="No client.toml"):
        read_client_config_file(DevEnvName("nobody"))
...
def test_read_secrets_missing_returns_empty(_isolated_home: Path) -> None:
    ...
    loaded = read_secrets_file(DevEnvName("nobody"))
    assert loaded.secrets == {}


def test_env_root_exists(_isolated_home: Path) -> None:
    assert env_root_exists(DevEnvName("ghost")) is False
    write_client_config(_make_client(), name=DevEnvName("ghost"))
    assert env_root_exists(DevEnvName("ghost")) is True


def test_client_config_exists(_isolated_home: Path) -> None:
    assert client_config_exists(DevEnvName("ghost")) is False
    write_client_config(_make_client(), name=DevEnvName("ghost"))
    assert client_config_exists(DevEnvName("ghost")) is True


def test_delete_env_root(_isolated_home: Path) -> None:
    write_client_config(_make_client(), name=DevEnvName("eve"))
    write_secrets_file(_make_secrets(), name=DevEnvName("eve"))
    target = env_root_dir(DevEnvName("eve"))
    assert target.is_dir()
    assert delete_env_root(DevEnvName("eve")) is True
    assert not target.exists()


def test_delete_env_root_missing_returns_false(_isolated_home: Path) -> None:
    assert delete_env_root(DevEnvName("ghost")) is False
