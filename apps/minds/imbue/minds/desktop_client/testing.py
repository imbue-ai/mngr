...
def vault_kv_stub_body(keys: Sequence[str], value: str) -> str:
    """Shell lines for a ``vault`` stub whose one KV directory lists ``keys``, each a leaf holding ``value``.

    Answers ``vault kv list`` and ``vault kv get`` in the split-layout shapes ``vault_reader``
    parses, so a test can put a shared credential in front of the Vault fallback.
    """
    listing = json.dumps(list(keys))
    leaf = json.dumps({"data": {"data": {"value": value}}})
    return f"case \"$2\" in list) echo '{listing}';; get) echo '{leaf}';; esac"
