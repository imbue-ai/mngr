"""Unit tests for the create-form host-name availability check in ``app``.

Covers the snapshot-reading helper (``_taken_host_names_on_provider``) and the
``GET /api/host-name-available`` endpoint that the create form's Name field
polls for live feedback: provider/account scoping, exclusion of destroyed
workspaces, and case-insensitive matching.
"""
...
from imbue.minds.desktop_client.app import _taken_host_names_on_provider
...
def test_taken_host_names_scopes_to_provider_and_excludes_destroyed() -> None:
    ...
    assert _taken_host_names_on_provider(resolver, "docker") == {"my-mind"}


def test_taken_host_names_is_per_provider_instance() -> None:
    resolver = _resolver_with_sample_workspaces()
    assert _taken_host_names_on_provider(resolver, _ACCOUNT_PROVIDER) == {"cloud-mind"}
    # A provider with no workspaces has nothing taken.
    assert _taken_host_names_on_provider(resolver, "lima") == set()
...
def _available(client: FlaskClient, **params: str) -> bool:
    response = client.get("/api/host-name-available", query_string=params)
...
def test_availability_endpoint_requires_authentication(tmp_path: Path) -> None:
    ...
    client = app.test_client()
    response = client.get("/api/host-name-available", query_string={"name": "My-Mind", "launch_mode": "DOCKER"})
