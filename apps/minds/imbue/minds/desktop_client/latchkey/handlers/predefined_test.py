...
def test_grant_with_unknown_credentials_proceeds_without_invoking_auth_browser(tmp_path: Path) -> None:
    ...
    # (a rawCurl credential it can't validate, or a catalog scope that
    # is not a registered latchkey service at all, like the ``minds``
    # peer-spawn scope served by a gateway extension); the grant must
    # proceed without prompting the user or invoking browser.
    binary = tmp_path / "latchkey"
    auth_recording = tmp_path / "auth_latchkey_report.jsonl"
    binary.write_text(
        "#!/usr/bin/env python3\n"
