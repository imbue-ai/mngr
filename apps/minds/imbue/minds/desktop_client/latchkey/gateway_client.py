...
# Permission request types accepted by the gateway extension. Kept in
# sync with ``VALID_REQUEST_TYPES`` in ``permission_requests.mjs``.
...
class StreamedPermissionRequest(FrozenModel):
    ...
    ``payload`` shape depends on ``request_type``; consumers dispatch
    on the type to pick the right concrete payload.
    """
    ...
    request_type: str = Field(
        description="Request kind: ``predefined`` (detent scope/perm grant) or ``file-sharing``.",
    )
    payload: dict[str, JsonValue] = Field(
        description="Type-specific payload (see ``PredefinedRequestPayload`` / ``FileSharingRequestPayload``).",
    )
    target: str = Field(
        description="Absolute path of the permissions.json that an approval would modify.",
    )
    effect: dict[str, JsonValue] = Field(
        description=(
            "Pre-computed patch (``rules`` / ``schemas``) the gateway will splice into ``target`` "
            "when this request is approved. Carried here mainly for traceability; the desktop "
            "client does not have to interpret it."
        ),
    )

    def as_predefined_payload(self) -> PredefinedRequestPayload:
        """Parse ``payload`` as a predefined-type payload. Raises ``ValueError`` on type mismatch."""
        if self.request_type != REQUEST_TYPE_PREDEFINED:
            raise ValueError(
                f"as_predefined_payload called on request_type={self.request_type!r}",
            )
        return PredefinedRequestPayload.model_validate(self.payload)

    def as_file_sharing_payload(self) -> FileSharingRequestPayload:
        """Parse ``payload`` as a file-sharing-type payload. Raises ``ValueError`` on type mismatch."""
        if self.request_type != REQUEST_TYPE_FILE_SHARING:
            raise ValueError(
                f"as_file_sharing_payload called on request_type={self.request_type!r}",
            )
        return FileSharingRequestPayload.model_validate(self.payload)
