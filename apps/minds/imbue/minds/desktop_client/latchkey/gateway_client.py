...
class LatchkeyGatewayClient(MutableModel):
    ...
    def delete_permission_rule(self, permissions_file_path: Path, rule_key: str) -> None:
        """Remove the rule for ``rule_key`` from ``permissions_file_path``.

        Wraps ``DELETE /permissions/rules?path=<...>&rule_key=<...>``.
        A 404 (no such rule) is tolerated so destruction cleanup is
        ...
        try:
            ...
            raise self._wrap_transport_error(e, f"DELETE {url} failed") from e
        if response.status_code == 404:
            logger.debug("DELETE {} returned 404; rule already absent", url)
            return
        if response.status_code >= 400:
            raise LatchkeyGatewayClientError(
                f"DELETE {url} returned {response.status_code}: {response.text.strip()}",
    ...
    def set_permission_schema(
        self,
        permissions_file_path: Path,
        schema_name: str,
        schema_body: JsonValue,
    ) -> None:
        """Add or replace the inline schema named ``schema_name``.

        Wraps ``POST /permissions/schemas?path=<...>&schema_name=<...>``
        with a JSON-object body (the schema definition). The extension
        creates the target file if it does not yet exist. ``schema_body``
        is typed as :class:`JsonValue` (rather than ``Mapping[str,
        JsonValue]``) so callers can pass a value whose static type
        carries through nested-dict tokens like ``{"properties": ...}``
        without an extra cast; the gateway extension rejects bodies
        that are not JSON objects at the wire level.
        """
        self.ensure_initialized()
        url = f"{self._require_base_url().rstrip('/')}/permissions/schemas"
        params = {"path": str(permissions_file_path), "schema_name": schema_name}
        try:
            with self._one_shot_client() as client:
                response = client.post(
                    url,
    ...
    def delete_permission_schema(self, permissions_file_path: Path, schema_name: str) -> None:
        """Remove the inline schema named ``schema_name``.

        Wraps ``DELETE /permissions/schemas?path=<...>&schema_name=<...>``.
        A 404 (no such schema) is tolerated so destruction cleanup is
        idempotent across crashes.
        """
        self.ensure_initialized()
        url = f"{self._require_base_url().rstrip('/')}/permissions/schemas"
        ...
        try:
            ...
            raise self._wrap_transport_error(e, f"DELETE {url} failed") from e
        if response.status_code == 404:
            logger.debug("DELETE {} returned 404; schema already absent", url)
            return
        if response.status_code >= 400:
            raise LatchkeyGatewayClientError(
                f"DELETE {url} returned {response.status_code}: {response.text.strip()}",
