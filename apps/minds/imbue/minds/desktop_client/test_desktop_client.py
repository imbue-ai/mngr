...
def _install_fake_mngr(
    client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    exit_code: int = 0,
) -> Path:
    """Install a fake `mngr` that records its args and exits with the given code.

    Writes the fake script on disk, exposes it via PATH (for anything shell-
    resolving it), and injects the absolute path into ``app.state.mngr_binary``
    which is what the workspace-delete handler actually reads.
    """
    ...
    app.state.mngr_binary = str(fake_mngr)
    return log_path
...
def test_delete_workspace_invokes_mngr_destroy_and_returns_redirect(
    ...
    log_path = _install_fake_mngr(client, monkeypatch, tmp_path, exit_code=0)
    _authenticate_client(client=client, auth_store=auth_store)
    agent_id = AgentId()
...
def test_delete_workspace_returns_500_on_mngr_destroy_failure(
    ...
    client, auth_store, _ = _create_test_server_with_agent_creator(tmp_path)
    _install_fake_mngr(client, monkeypatch, tmp_path, exit_code=1)
    _authenticate_client(client=client, auth_store=auth_store)
    agent_id = AgentId()
...
def test_delete_workspace_removes_minds_data_dir(
    ...
    client, auth_store, _ = _create_test_server_with_agent_creator(tmp_path)
    _install_fake_mngr(client, monkeypatch, tmp_path, exit_code=0)
    _authenticate_client(client=client, auth_store=auth_store)
    agent_id = AgentId()
