...
_VAULT_STUB_TOKEN = " tok-from-vault-70213 "
...
def test_the_vault_fallback_yields_the_shared_leaf_s_value_under_a_group_of_its_own(
    ...
    install_stub_on_path(
        tmp_path / "bin", monkeypatch, "vault", vault_kv_stub_body([INTERNAL_GIT_TOKEN_VAULT_KEY], _VAULT_STUB_TOKEN)
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (vault_kv_stub_body(["OTHER_KEY"], _VAULT_STUB_TOKEN), "the entry has no token leaf"),
        (vault_kv_stub_body([INTERNAL_GIT_TOKEN_VAULT_KEY], "  "), "the leaf is blank"),
    ],
)
