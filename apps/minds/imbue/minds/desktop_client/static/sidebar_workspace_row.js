...
//
// Usage:
//   var row = window.mindsSidebarRow.buildRow(workspace, { isCurrent: bool });
...
//
// ``workspace`` is { id, name, accent?, is_stale?, backup_warning? }. Each
// local row carries a settings gear (always visible). ``isCurrent`` marks the
// row selected (highlighted background). Event wiring (click / context-menu)
...
(function () {
  ...
  function buildSettingsBtn(agentId) {
    // A w-3.5 (14px) gear centered in the shared 24x24 button, reading as a
  ...
  function buildRow(workspace, options) {
    ...
    // Row action icon, always visible (no hover-reveal): the settings gear,
    // on every local row in both modes.
