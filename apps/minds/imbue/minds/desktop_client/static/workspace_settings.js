...
(function () {
  ...
  if (hexInput && swatchContainer && errorEl && !isStale) {
    ...
    function previewChromeAccent(hex) {
      // Optimistic local repaint: the settings page renders on the trusted
      // chrome surface, so it calls the shell bridge directly; main retargets
      // this bundle's chrome view. Cross-machine sync still happens via the
      // normal SSE flow; this just shortcuts the local-window paint so the user
      // sees their pick immediately instead of waiting for ``mngr label`` + the
      // SSE round-trip. Falls through silently in browser mode (no bridge) --
