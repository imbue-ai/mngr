...
def _workspace_agent(
    host_id: HostId,
    agent_id: AgentId,
    agent_name: str = "my-claude-agent",
    workspace_name: str = "babaabab",
) -> DiscoveredAgent:
    """A ``workspace`` + ``is_primary`` labeled agent, matching how minds creates them."""
    return DiscoveredAgent(
        host_id=host_id,
        agent_id=agent_id,
        agent_name=AgentName(agent_name),
        provider_name=ProviderInstanceName("docker"),
        certified_data={"labels": {"workspace": workspace_name, "is_primary": ""}},
    )
...
def test_workspace_cache_keeps_tile_visible_after_discovery_drops_agent(tmp_path: Path) -> None:
    """The SSH-dead reproducer: discovery loses the workspace agent but the tile must still render.

    Mirrors the live failure mode -- ``pkill sshd`` inside the docker
    container -- where the next ``update_agents`` call comes in with no
    ``discovered_agents`` for that host. The landing page should still
    surface the workspace via ``list_known_or_cached_workspace_ids`` and
    ``get_workspace_name``.
    """
    cache_path = tmp_path / "workspaces_cache.json"
    resolver = MngrCliBackendResolver(workspaces_cache_path=cache_path)
    host = HostId.generate()
    workspace_agent = AgentId.generate()
    resolver.update_agents(
        ParsedAgentsResult(
            agent_ids=(workspace_agent,),
            discovered_agents=(_workspace_agent(host, workspace_agent, workspace_name="babaabab"),),
        )
    )
    # SSH dies; discovery drops the agent.
    resolver.update_agents(ParsedAgentsResult())

    assert resolver.list_known_workspace_ids() == ()
    assert resolver.list_known_or_cached_workspace_ids() == (workspace_agent,)
    assert resolver.get_workspace_name(workspace_agent) == "babaabab"


def test_workspace_cache_persists_entry_to_disk(tmp_path: Path) -> None:
    """First time discovery surfaces a primary workspace agent, the cache is written through to disk."""
    cache_path = tmp_path / "workspaces_cache.json"
    resolver = MngrCliBackendResolver(workspaces_cache_path=cache_path)
    host = HostId.generate()
    workspace_agent = AgentId.generate()
    ...
    assert cache_path.exists()
    persisted = json.loads(cache_path.read_text(encoding="utf-8"))
    assert persisted == {str(workspace_agent): {"name": "babaabab"}}


def test_workspace_cache_loads_from_disk_on_construction(tmp_path: Path) -> None:
    """A persisted cache from a previous minds run rehydrates at construction."""
    cache_path = tmp_path / "workspaces_cache.json"
    workspace_agent = AgentId.generate()
    cache_path.write_text(
        json.dumps({str(workspace_agent): {"name": "babaabab"}}),
        encoding="utf-8",
    )

    resolver = MngrCliBackendResolver(workspaces_cache_path=cache_path)

    # No live discovery yet -- cache alone supplies the tile.
    assert resolver.list_known_or_cached_workspace_ids() == (workspace_agent,)
    assert resolver.get_workspace_name(workspace_agent) == "babaabab"


def test_workspace_cache_ignores_malformed_persisted_file(tmp_path: Path) -> None:
    """A garbage cache file is treated as empty rather than crashing minds startup."""
    cache_path = tmp_path / "workspaces_cache.json"
    cache_path.write_text("not json {{{", encoding="utf-8")

    resolver = MngrCliBackendResolver(workspaces_cache_path=cache_path)
    assert resolver.list_known_or_cached_workspace_ids() == ()
    assert resolver.get_workspace_name(AgentId.generate()) is None


def test_workspace_cache_prefers_live_workspace_name_when_present(tmp_path: Path) -> None:
    """When the live snapshot has the agent, its label wins over a stale cached name."""
    cache_path = tmp_path / "workspaces_cache.json"
    workspace_agent = AgentId.generate()
    cache_path.write_text(
        json.dumps({str(workspace_agent): {"name": "old-name"}}),
        encoding="utf-8",
    )
    resolver = MngrCliBackendResolver(workspaces_cache_path=cache_path)
    host = HostId.generate()
    resolver.update_agents(
        ParsedAgentsResult(
            agent_ids=(workspace_agent,),
            discovered_agents=(_workspace_agent(host, workspace_agent, workspace_name="renamed-workspace"),),
        )
    )

    assert resolver.get_workspace_name(workspace_agent) == "renamed-workspace"


def test_workspace_cache_does_not_duplicate_live_ids_in_augmented_list(tmp_path: Path) -> None:
    """``list_known_or_cached_workspace_ids`` dedupes: a live id is not also returned from cache."""
    cache_path = tmp_path / "workspaces_cache.json"
    resolver = MngrCliBackendResolver(workspaces_cache_path=cache_path)
    host = HostId.generate()
    workspace_agent = AgentId.generate()
    discovered = ParsedAgentsResult(
        agent_ids=(workspace_agent,),
        discovered_agents=(_workspace_agent(host, workspace_agent),),
    )
    resolver.update_agents(discovered)
    # Same update again -- cache already has the entry; augmented list should
...
def test_evict_cached_workspace_drops_entry_from_memory_and_disk(tmp_path: Path) -> None:
    """``evict_cached_workspace`` removes the entry so a confirmed destroy clears the tile."""
    cache_path = tmp_path / "workspaces_cache.json"
    resolver = MngrCliBackendResolver(workspaces_cache_path=cache_path)
    host = HostId.generate()
    workspace_agent = AgentId.generate()
    ...
    # Discovery loses the agent (e.g. destroy completed). Cache still has it
    # until the landing-page DONE-detect path tells us to evict.
    resolver.update_agents(ParsedAgentsResult())
    assert resolver.list_known_or_cached_workspace_ids() == (workspace_agent,)

    resolver.evict_cached_workspace(workspace_agent)
...
def test_evict_cached_workspace_is_a_no_op_for_unknown_id(tmp_path: Path) -> None:
    """Evicting an id that was never cached must not error or corrupt the cache."""
    cache_path = tmp_path / "workspaces_cache.json"
    resolver = MngrCliBackendResolver(workspaces_cache_path=cache_path)

    # Should not raise; no on-disk side effects either (cache stays absent).
    resolver.evict_cached_workspace(AgentId.generate())

    assert not cache_path.exists()


def test_workspace_cache_ignores_non_primary_workspace_agents(tmp_path: Path) -> None:
    """Only primary workspace agents are cached; the system-services sibling is skipped."""
    cache_path = tmp_path / "workspaces_cache.json"
    resolver = MngrCliBackendResolver(workspaces_cache_path=cache_path)
    host = HostId.generate()
    workspace_agent = AgentId.generate()
    ...
    resolver.update_agents(
        ParsedAgentsResult(
            agent_ids=(workspace_agent, services_agent),
            discovered_agents=(
                _workspace_agent(host, workspace_agent, workspace_name="babaabab"),
                _discovered_agent(host, services_agent, "system-services"),
            ),
        )
    )
    ...
    assert resolver.list_known_or_cached_workspace_ids() == (workspace_agent,)
    persisted = json.loads(cache_path.read_text(encoding="utf-8"))
    assert services_agent not in persisted
    assert str(services_agent) not in persisted
