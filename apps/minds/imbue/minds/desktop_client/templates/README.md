...
| `static/tokens.css` | `.minds-card`, `.spinner` + `.spinner-accent`, `.code-pill`, `.accent-spine`, `.sidebar-item`, `.accent-swatch`, `.titlebar-title` / `.titlebar-btn` / `.titlebar-btn-danger`, `--shadow-seam`, `--workspace-accent` / `--titlebar-bg` / `--titlebar-fg` (set via inline style on the document root by chrome.js). |
