...
| `.sidebar-dot` | Per-workspace accent circle in the workspace menu rows. Sized by Tailwind (`w-2.5 h-2.5 rounded-full`); colored inline per workspace by `overlay_sidebar.js` / `chrome.js`. Not an app.css class -- listed here as the accent-surface to keep in sync. (The workspace row itself carries the `.sidebar-item` class purely as a JS selector hook + `is-current` / `is-stale` state marker; it has no app.css styling.) |
...
| `.minds-tooltip` / `.minds-tooltip-shortcut` | Custom hover-tooltip bubble built in JS by `overlay.js` on the overlay surface (see `pages/OverlayHost.jinja`). High-contrast inverse surface (like a native tooltip) with `--shadow-overlay`; `-shortcut` is the optional keyboard-shortcut chip. Not a JinjaX component -- CSS-only recipe. |
