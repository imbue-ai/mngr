...
| `CopyField` | Read-only monospace value in a subtle-fill box; the input selects itself on click. Children render as trailing controls (a secondary Copy Button). `value` / `extra` (parent-owned spacing on the box); other HTML attrs (`id=`, `aria-label=`) pass through to the input. Used by the sharing editor's link row and the Template page's repo / skill-message displays. |
