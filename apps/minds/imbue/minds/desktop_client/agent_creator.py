        ...
        try:
            with log_span(
                ...
                if _is_local_path(repo_source):
                    ...
                    # Don't shallow-clone the remote URL: mngr's later mirror push
                    # to the agent's bare repo rejects shallow updates ("shallow
                    # update not allowed"). Full clone is the simplest fix.
                    clone_git_repo(
                        GitUrl(repo_source),
                        clone_target,
                        on_output=emit_log,
