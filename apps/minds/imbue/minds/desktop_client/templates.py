...
_RECOVERY_SCRIPT: Final[str] = """\
      (function () {
        ...
        function applyHealth(data, autoDispatch) {
          ...
          // mngr start terminates+resumes the live sandbox, too destructive to fire
          // without explicit consent -- render the manual Restart button so the user
          // clicks (the click hits the host-restart endpoint, which skips the
          // unsupported stop step). Docker/lima keep their auto-dispatch below.
