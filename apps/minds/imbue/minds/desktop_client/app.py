...
def _is_api_authenticated() -> bool:
    """Authenticate an API request via either the session cookie or the central minds API key.

    The bearer token is the per-startup ``MINDS_API_KEY`` the latchkey
    gateway's ``minds-api-proxy`` extension injects as ``Authorization:
    Bearer <key>`` on every request it forwards. Lets a peer mind drive
    ``/api/create-agent`` (and its status/logs siblings) through the
    gateway without ever seeing the credential itself.
    """
    if _is_request_authenticated():
        return True
    minds_api_key = get_state().minds_api_key
    if minds_api_key is None:
        return False
    return is_request_authenticated(request.headers.get("authorization"), minds_api_key)
...
def _handle_create_agent_api() -> Response:
    ...
    callers) or a bearer token (latchkey-mediated peer-mind callers).
    """
    if not _is_api_authenticated():
        return make_response(status_code=403, content='{"error": "Not authenticated"}', media_type="application/json")
