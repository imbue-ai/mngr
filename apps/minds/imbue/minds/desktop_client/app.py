...
# name (`WORKSPACE_SERVER_SERVICE_NAME`) is imported from agent_creator.
...
async def _handle_workspace_forward_http(request: Request) -> Response:
    ...
    return await _forward_workspace_http(
        request=request,
        agent_id=agent_id,
        workspace_backend_url=workspace_url,
...
def _tmux_window_target_for_workspace_server(workspace_name: str) -> str:
    ...
    return f"{session_name}:svc-{WORKSPACE_SERVER_SERVICE_NAME}"
...
async def _handle_agent_health_probe(
    ...
    workspace_url = backend_resolver.get_backend_url(parsed_id, WORKSPACE_SERVER_SERVICE_NAME)
    if workspace_url is None:
