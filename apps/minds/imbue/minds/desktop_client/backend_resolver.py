...
class MngrStreamManager(MutableModel):
    ...
    def _watch_observe_process(self) -> None:
        ...
        stuck on "Discovering agents..." forever (e.g. `mngr` crashes during
        module loading because of missing workspace packages). Without this
        log, user bug reports contain no signal at all about what went wrong.
        """
        process = self._observe_process
        if process is None:
            return
        # Poll via threading.Event.wait (not time.sleep) so the thread stays
        # interruptible and the ratchet stays happy.
