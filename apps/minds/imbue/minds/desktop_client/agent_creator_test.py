...
def test_build_mngr_create_command_uses_main_template_and_omits_message_arg() -> None:
    ...
    # Every mode passes ``--new-host``, which mngr rejects together with
    # ``--reuse``; ``--update`` (which itself requires ``--reuse``) goes too.
...
def test_build_mngr_create_command_imbue_cloud_targets_account_provider() -> None:
    ...
    # IMBUE_CLOUD passes ``--new-host``, which mngr rejects together with
    # ``--reuse``. The bake's pre-baked ``system-services`` agent is adopted
    # in place by ``ImbueCloudHost.create_agent_state``; mngr's duplicate-name
    # check exempts ``host.pre_baked_agent_id`` so no ``--reuse`` is needed.
    # No ``--id`` either: the canonical id is parsed from the JSONL
    # ``created`` event.
    assert "--id" not in command
    assert "--reuse" not in command
    assert "--update" not in command
