...
def test_prevent_direct_subprocess() -> None:
    # ``_latchkey_gateway_launcher.py`` intentionally uses ``subprocess.Popen``
    # with ``start_new_session=True`` so that the spawned ``latchkey gateway``
    ...
    # so we exclude the launcher specifically; see its module docstring for
    # the full justification.
    excluded = TEST_FILE_PATTERNS + ("testing.py", "scripts/*.py", "*/_latchkey_gateway_launcher.py")
