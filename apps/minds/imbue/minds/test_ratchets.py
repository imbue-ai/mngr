...
def test_prevent_time_sleep() -> None:
    # Ten matches across two clusters:
    #
    # Operational pauses on the backend:
    # - ``destroying_test.py`` (a real test poll loop),
    ...
    #   deploy-side),
    # - ``deployment_tests/test_deploy_new_version.py::
    ...
    # - ``deployment_tests/test_deploy_rollback.py::_poll_for_deploy_id``
    #   (polling /version after a forced auto-rollback to confirm the
    #   rolled-back version is the one actually serving traffic; same
    #   Modal swap-window justification).
    #
    # In-VM harness polling, which lives outside the monorepo runtime:
    # - Four matches in ``scripts/vm-testing/harness/run-harness.py``,
    #   which polls the backend / agent-creation status from inside a
    #   fresh macOS VM with only stdlib on the path. ``wait_for`` is not
    #   reachable in that runtime context.
