Feature: Minds desktop client invariants
  ...
  Rule: No negative verdict rests on time this process was not running
    ...
    Nor is a measurement taken in the window just after such an interval, while this device is still rebuilding the connections the interval killed: a failure run that opens there must outlast that rebuild before it can convict.
