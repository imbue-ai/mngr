...
**MET (`state`, fixed on this branch)** / **PARTIAL (`redirect_uri`)**. A CSRF `state` parameter is now generated, injected, and verified entirely **client-side**: the CLI mints a `secrets.token_urlsafe(32)` state per flow and injects it into the authorize URL itself via `_authorize_url_with_state` (dropping any pre-existing `state` so the client's value is the one echoed back), then verifies the echoed `state` in constant time (`secrets.compare_digest`, with a non-ASCII guard) **before** exchanging the code — a missing or mismatched state aborts the flow ([`mngr_imbue_cloud/cli/auth.py`](../../../libs/mngr_imbue_cloud/imbue/mngr_imbue_cloud/cli/auth.py)). Keeping state a pure client concern means sign-in works against any connector version (no lockstep deploy); the connector is not involved in state. Tests cover URL-injection and reject-on-mismatch/missing/non-ASCII, and the injection was verified end-to-end against the live connector. `redirect_uri` (`callback_url`) still has no server-side allow-list check on the connector — it remains mitigated by the IdP's own registered-redirect-URI allow-list; adding a loopback-only allow-list on the connector is a remaining defense-in-depth item.
...
**MET (fixed on this branch).** Desktop-client-owned HTML already used autoescaping Jinja, and semi-trusted agent content is isolated by origin (each agent on its own `agent-<id>.localhost` subdomain — see [`security-boundaries-audit.md`](security-boundaries-audit.md)). Added: a Flask `after_request` hook ([`app.py`](../imbue/minds/desktop_client/app.py)) that sets `X-Content-Type-Options: nosniff` and a restrictive first-party `Content-Security-Policy` on the desktop client's **own** responses, scoped by host so it is **not** applied to proxied `*.localhost` agent responses (agents control their own CSP, so clamping one on would break agent web apps). `connect-src` pins to `'self'`, loopback websockets, and the Sentry ingest origin (so opt-in browser error reporting keeps working). `frame-src` admits only loopback origins (`localhost`, `127.0.0.1`, `*.localhost`, both schemes): browser (non-Electron) mode embeds workspace content in an iframe pointed at the `mngr forward` origin, which without this would fall back to `default-src 'self'` and silently blank the content pane. Framing a non-loopback host stays blocked, so the exfiltration guard is intact; framing an agent subdomain grants the parent no DOM access, since origin isolation (not the CSP) is the boundary there. `setdefault` semantics avoid clobbering handler-set headers. Tests assert first-party-gets-headers, agent-path-does-not-get-CSP, Sentry/loopback reachability, and that `frame-src` admits loopback while rejecting wildcard/scheme-only sources. `contextIsolation: true`/`nodeIntegration: false` remain enforced on all Electron views.
...
**N/A** (no webhook secrets exist). By analogy, all comparable secrets (session signing key, API key, DEKs) are generated at runtime via `secrets.token_urlsafe` and never hardcoded. A `gitleaks` pre-commit hook now scans staged changes for committed secrets (added on this branch, see 6.1.1), so an accidental secret commit is caught going forward.
...
- **3.2.2 (`state`)** — Client-owned (CLI-injected), constant-time-verified CSRF `state` on the OAuth flow.
