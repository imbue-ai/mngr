...
- **FCT push rejected by secret-scanning push protection (`GH013` / "Push cannot contain secrets").** FCT has push protection on; mngr apparently does not. So a credential that lives happily in mngr `main` blocks the *vendored* copy the moment `vendor/mngr` picks it up — even though the same bytes are already public in mngr. Seen with the Minds Google OAuth desktop-app client id/secret (`vendor/mngr/libs/mngr_latchkey/.../core.py`, from mngr #2246). The block is keyed to the new vendor commit, so it appears only on the refresh that first introduces the credential. **Do not redact it in FCT** — that breaks the vendor-match invariant and the feature. Resolution is a human web-UI decision (no `git`/`gh` flag): an authorized user opens each per-secret unblock URL the push prints, marks it allowed, then you re-push. The allow is per-commit, so a later re-sync to a new SHA re-blocks and needs re-allowing. Treat "is this actually safe to ship?" as a real question first (desktop-app client type vs web client; intentional vs leaked) — push protection firing is the prompt to confirm, not rubber-stamp.
