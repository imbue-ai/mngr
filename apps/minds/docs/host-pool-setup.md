...
or `--workspace-dir <dir>` (dev; a working tree, default `$FCT_DIR` -- from your
shell or a gitignored `apps/minds/.env` -- else the
`.external_worktrees/forever-claude-template` checkout). The minds desktop client always sends
