...
- `minds://` bare, or any unrecognized or malformed URL, just opens/focuses the app. The browser OAuth sign-in flow relies on this: the desktop client passes `--success-redirect-url minds://` to the plugin's `auth oauth` subcommand, whose sign-in success page then bounces the browser back to the app.
