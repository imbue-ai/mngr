...
- **Session restore**: on quit, every open window's content URL is recorded to `~/.<MINDS_ROOT_NAME>/window-state.json` (as `{ windows: [...], lastWorkspaceAgentId: string | null }`). On next launch (after the backend is ready) one window is reopened per recorded URL. URLs pointing at workspaces that no longer exist are silently dropped. `lastWorkspaceAgentId` carries the most-recently-opened workspace so its accent paints the titlebar across navigation away from the workspace; it's cleared on workspace deletion or account sign-out.
