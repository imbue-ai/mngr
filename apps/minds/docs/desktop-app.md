...
- **Close Window**: `Ctrl+W` / `Cmd+W` -- closes the focused window. On macOS the app (and backend) keep running even after the last window closes -- re-open from the dock icon (which restores the windows you had open) or `Cmd+N` (a fresh home window). On Windows/Linux the backend shuts down when the last window closes.
...
- **Session restore**: on quit and whenever a window closes, every open window's content URL is recorded to `~/.<MINDS_ROOT_NAME>/window-state.json` (as `{ windows: [{ url, x, y, width, height, displayId }, ...] }`). On next launch -- or on macOS, when the app is reopened from the dock or launched again with no window open -- one window is reopened per recorded URL once the backend is ready (workspace windows restore through the SPA's `/workspace/<id>` route). URLs pointing at workspaces that no longer exist are silently dropped; older file shapes are accepted.
...
error-reporting settings). Signing in makes the new account the default
