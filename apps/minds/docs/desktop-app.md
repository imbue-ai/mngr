...
The full-width titlebar (and the thin shell around the content view) adopt the active workspace's accent color while you're on a workspace-scoped screen, and fall back to a **neutral** chrome on every other minds screen. The neutral chrome background comes from the `--titlebar-bg` fallback in the shared `ChromeShell.jinja` layout (`var(--c-surface-primary)`: white in light mode, black in dark) -- `ChromeShell` is the trusted app shell every local page and the agent-content wrapper (`Chrome.jinja`) now render through; its foreground is not a stored value but is derived from the background in pure CSS by the `.titlebar-surface` recipe in `app.css` (an `lch(from …)` relative-color contrast), the same recipe that re-bases the foreground tokens under an active workspace accent. The same neutral surface is used by the startup/quitting/error loading screen (`shell.html`). Workspace accent swatches deliberately exclude pure black and white so a workspace's color can never collide with this neutral chrome (users can still type either into the settings hex input).

The accent is a **pure function of the window's current screen**, not a remembered value (`parseAccentSourceAgentId`: the workspace id on the workspace itself plus its settings / sharing / destroying / recovery screens, `null` on a general screen). After the content-in-chrome split the accent source comes from whichever surface changed: on an agent path the content view's `did-navigate` drives it (the titlebar is a separate `WebContentsView`, so main pushes the value over a single `accent-changed` IPC and the chrome renderer applies it); on a trusted local page the chrome view now renders the page itself, so its own `did-navigate` (`onChromeNavigate`) sets the accent directly in the renderer. Main also re-pushes the current value whenever a chrome view (re)loads (via `primeViewWithCachedChromeState`), which covers cold start, new windows, and crash-recovery rebuilds. The narrower "which workspace is actually being *displayed*" signal (`current-workspace-changed`) is separate and drives only the OS window title and the recovery-page auto-redirect. Browser mode derives the accent from its own path on each local-page load (`accentSourceFromPath`), and from the iframe URL in the poll loop on the agent-wrapper page.
