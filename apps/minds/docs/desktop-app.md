...
Each window is a frameless `BrowserWindow` (`frame: false` on Linux/Windows, `titleBarStyle: 'hiddenInset'` with `trafficLightPosition` on macOS) hosting ONE web context: the backend-served chrome page. That single page owns the titlebar, navigation among the local hub pages (an in-place swap engine keeps the titlebar persistent), the sandboxed cross-origin iframe that displays workspace content, and an in-DOM overlay layer hosting every modal (workspace switcher, inbox, help, sign-in, settings, accounts, workspace options) as a same-origin iframe. The identical page runs in a plain browser against a local `minds run` -- the desktop app adds only a slim native bridge (`window.mindsNative` from `preload.js`: window controls, native file picker, the main-process SSE relay, and the startup/error/quitting screens).
...
- **Session restore**: on quit, every open window's content URL is recorded to `~/.<MINDS_ROOT_NAME>/window-state.json` (as `{ windows: [{ url, x, y, width, height, displayId }, ...] }`). On next launch (after the backend is ready) one window is reopened per recorded URL (workspace windows restore through the `/_chrome?agent=...` wrapper). URLs pointing at workspaces that no longer exist are silently dropped; older file shapes are accepted.
...
- `minds://create?git_url=<repo>&branch=<ref>` focuses the most recent window and lands the user on the Create from Inspiration stepper -- the numbered walkthrough described under [Create from Inspiration](#create-from-inspiration) -- in one of two shells, chosen by context. **Outside a machine** (home/general screens) it navigates the shell to the full page (`/create/inspiration`). **Already inside a machine** it pops the same stepper as a modal (`/create/inspiration/modal`) over that machine, which is less disruptive than a full-page takeover. The modal hosts the *add* branch in place, targeting the machine they are already in: it drops the machine picker, and its last step simply says to paste the copied `/use-inspiration <repo>` message into that chat. That step stays up until the user acknowledges it with **Done** -- it never dismisses itself, so the instruction cannot vanish before it is read. Choosing **Create a new machine** is a bigger job than a popup should host, so it hands off to the full page (`?start=create`, which skips the now-answered chooser) and closes the modal. `main.js` reads the window's current machine id (from `bundle.currentWorkspaceId`, the shell's internal field) and passes it as `current_machine` to pick the shell and to name the machine. Because the modal is hosted in the shared overlay iframe, every navigation inside it goes through the `window.minds` bridge (`navigateContent` + `closeModal`) -- a plain `window.location` would load the destination inside the modal. `branch` accepts anything the create form's Branch input accepts (branch, tag, or commit); when absent it stays blank -- creation then resolves the linked repo's latest version. A `minds://create` link without a `git_url` navigates the shell to the plain create page. Values must be percent-encoded by the sender.
...
Deeplinks never force a sign-in: `/create` loads regardless of account state and the page's own remote-vs-local flow prompts for sign-in only when needed. A deeplink that arrives before startup navigation has settled (backend still starting, or an error takeover showing) is queued last-writer-wins and applied once startup succeeds. This holds on a genuine first run too: an explicit deeplink wins over the welcome screen, landing the new user directly on the pre-filled create page. Both the content-nav path (`deeplinkTargetPath`, the `/create` literal) and the modal path (`deeplinkModalPath`, the `/create/inspiration/modal` literal) are built from a fixed allowlist plus re-encoded query params; raw deeplink text is never handed to `loadURL` or `openModal`.
...
The accent is a **pure function of the window's current screen**, not a remembered value. The chrome page derives the accent source from its own navigation intents (`accentSourceFromPath` in `chrome.js`: the workspace id on the workspace itself plus its settings / sharing / destroying / recovery screens, `null` on a general screen) and applies it directly (`applyTitleAccent`); the color values come from the per-workspace cache the SSE `workspaces` events maintain, so a not-yet-cached accent paints on the next SSE tick. The same signal is announced on the in-page bus (`emitAccentChanged`) for the overlay layer's current-row highlight. The narrower "which workspace is actually being *displayed*" signal is separate and drives the recovery-page auto-redirect; in the desktop app, `main.js` independently tracks the displayed workspace from committed navigations for the OS window title and session restore, and re-primes a freshly loaded window with the cached SSE state (`primeWindowWithCachedChromeState`), covering cold start, new windows, and crash-recovery reloads. All of this behaves identically in Electron and browser mode.
