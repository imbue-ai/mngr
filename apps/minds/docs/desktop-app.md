...
- **Session restore**: on quit, every open window's content URL is recorded to `~/.<MINDS_ROOT_NAME>/window-state.json` (as `{ windows: [{ url, x, y, width, height, displayId, lastWorkspaceAgentId }, ...] }`). On next launch (after the backend is ready) one window is reopened per recorded URL. URLs pointing at workspaces that no longer exist are silently dropped. Each window's per-window `lastWorkspaceAgentId` is the workspace whose accent should tint that window's titlebar -- it tracks the window's *current* workspace-scoped screen (the workspace itself plus its settings / sharing / destroying / recovery screens, via `parseAccentSourceAgentId`), and is cleared to null on any general minds screen (Home, Create, accounts, inbox, ...) so the chrome paints the neutral, workspace-less color there. It's also cleared for matching windows on workspace deletion and for all windows on account sign-out.
