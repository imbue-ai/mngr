...
- **Stopped-workspace artifacts are operator-decryptable.** `mngr stop` uploads the slice VM's disks encrypted to a per-stop age identity, but that identity is wrapped by the **tier KEK** -- an operator-held key -- and stored on the workspace's connector row (see `docs/deploy/workspace-stop-start.md`). Stopping a workspace parks its data in operator-decryptable storage; only the restic backups are encrypted under material the user alone holds (synced inside the DEK-encrypted record).

For recovering from a lost or stolen device, see [the lost-device runbook](./deploy/lost-device-runbook.md).
