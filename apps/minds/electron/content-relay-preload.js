...
// shape so a malicious page cannot smuggle path/query characters into the URL
// the main process builds. The main process re-validates.
...
window.addEventListener('message', (event) => {
  ...
  // Error pages (e.g. the agent-origin loading / workspace-recovery page) ask
  // the shell to open the get-help / report-a-bug modal. ``agentId`` is optional
  // -- when present it scopes the report to that workspace; it is validated to
  // the server-issued shape (or accepted as empty) so a foreign page can't
  // smuggle path/query characters into the help URL the main process builds.
