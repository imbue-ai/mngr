...
// Extract the workspace id from the SPA's own /workspace/<id> route paths --
// the shapes notification deep links use: /workspace/<agent-id>?review=...
// (a request), ?chat=... (an agent message), and the workspace-scoped
// /workspace/<agent-id>/backups (a system event). Distinct from
// parseWorkspaceId: these are chrome-page routes, not workspace origins, so
// the origin//goto matcher above cannot see them. Path-only on purpose
// (callers only consult it for URLs parseWorkspaceId rejected).
