  ...
  try {
    ...
    if (isFirstStart && initialBundle && !initialBundle.window.isDestroyed()) {
      ...
      await new Promise((resolve) => {
        ...
        logger.logMain('info', `[startup] Consuming one-time code via ${authenticateUrl}`);
        const req = net.request({ url: authenticateUrl, method: 'GET', useSessionCookies: true });
        req.on('response', async (resp) => {
          logger.logMain('info', `[startup] /authenticate response status: ${resp.statusCode}`);
          resp.on('data', () => {});
          resp.on('end', async () => {
            try {
              const defaultCookies = await session.defaultSession.cookies.get({ name: 'minds_session' });
              logger.logMain('info', `[startup] Default session cookies after /authenticate: ${defaultCookies.length}`);
              const contentSession = session.fromPartition(CONTENT_PARTITION);
              ...
              logger.logMain('info', '[startup] Cookie synced to content partition');
            } catch (err) {
              logger.logMain('warning', '[startup] Failed to sync cookie to content partition: ' + (err && err.message ? err.message : err));
            }
            resolve();
          });
        });
        req.on('error', (err) => {
          logger.logMain('warning', '[startup] /authenticate request failed: ' + (err && err.message ? err.message : err));
          resolve();
        });
        req.end();
      });
