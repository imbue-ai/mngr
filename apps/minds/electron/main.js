...
function navigateBundle(bundle, url) {
  ...
  if (selectSurfaceForUrl(url) === SURFACE_CONTENT) {
    ...
    // previously-displayed workspace during the load. ``sendCurrentWorkspaceToBundleViews``
    // below forwards the readiness to the chrome, so it must be cleared first.
    bundle.contentWorkspaceReady = false;
    bundle.currentContentUrl = url;
    bundle.preErrorUrl = url;
    updateOsTitle(bundle);
    ...
    // workspace-scoped screens via parseAccentSourceAgentId. The content view is
    // hidden here, so no reachable workspace is displayed -- clear readiness too
    // (kept in lockstep with currentWorkspaceId, which it can only be true with).
    bundle.currentContentUrl = url;
    bundle.preErrorUrl = url;
    if (bundle.currentWorkspaceId !== null) {
      bundle.currentWorkspaceId = null;
...
// The Landing page -- a trusted local page on the chrome surface -- calls the
// `confirmStopMind` bridge; we re-validate the id here against the conservative
// agent-id shape (never trust the renderer).
