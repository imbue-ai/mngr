...
/**
 * Build the Solid frontend (client + SSR server bundles) and copy the
 * resulting artifacts into the packaged resources tree.
 *
 * Outputs:
 ...
 * with the same Electron-as-Node trick the latchkey shim uses.
 *
 * Skipped when the user has set `MINDS_SKIP_FRONTEND_BUILD=1` (useful
 * for iterating on the native build steps without recompiling JS).
 */
function buildFrontend() {
  if (process.env.MINDS_SKIP_FRONTEND_BUILD === '1') {
    console.log('Skipping frontend build (MINDS_SKIP_FRONTEND_BUILD=1).');
    return;
  }
  console.log('Building Solid frontend bundles...');
  execSync('pnpm exec vite build', { cwd: ROOT, stdio: 'inherit' });
  execSync(
    'pnpm exec vite build --ssr src/main/server.jsx ' +
      '--outDir ../frontend/dist-server --emptyOutDir',
    { cwd: ROOT, stdio: 'inherit' }
  );

  const sidecarDir = path.join(RESOURCES_DIR, 'frontend');
  const sidecarServerDir = path.join(sidecarDir, 'server');
  const sidecarBinDir = path.join(sidecarDir, 'bin');
  fs.mkdirSync(sidecarServerDir, { recursive: true });
  fs.mkdirSync(sidecarBinDir, { recursive: true });

  const builtServerDir = path.join(ROOT, 'frontend', 'dist-server');
  fs.cpSync(builtServerDir, sidecarServerDir, { recursive: true, dereference: true });
  // Mark the bundle dir as ESM so Node doesn't burn cycles parsing
  // CommonJS first and emit MODULE_TYPELESS_PACKAGE_JSON warnings each
  // time the sidecar boots.
  fs.writeFileSync(
    path.join(sidecarServerDir, 'package.json'),
    JSON.stringify({ name: 'minds-ssr-sidecar', private: true, type: 'module' }, null, 2) + '\n'
  );

  // The Vite SSR build emits the bundle into dist-server/assets/server.js
  // (the ``assets/[name].[ext]`` rule in vite.config.mjs); the shim resolves
  // it relative to its own location inside the packaged resources tree.
  const sidecarEntry = path.join('..', 'server', 'assets', 'server.js').replace(/\\/g, '/');
  const shimContent =
    '#!/usr/bin/env bash\n' +
    '# Auto-generated by scripts/build.js. Runs the Solid SSR sidecar under\n' +
    '# the Electron binary (invoked as Node via ELECTRON_RUN_AS_NODE=1).\n' +
    'set -eu\n' +
    'HERE="$(cd "$(dirname "$0")" && pwd)"\n' +
    'ENTRY="$HERE/' + sidecarEntry + '"\n' +
    'if [ -z "${MINDS_ELECTRON_EXEC_PATH:-}" ]; then\n' +
    '  echo "ssr-sidecar shim: MINDS_ELECTRON_EXEC_PATH not set; cannot locate Node runtime" >&2\n' +
    '  exit 1\n' +
    'fi\n' +
    'exec env ELECTRON_RUN_AS_NODE=1 "$MINDS_ELECTRON_EXEC_PATH" "$ENTRY" "$@"\n';
  const shimPath = path.join(sidecarBinDir, 'ssr-sidecar');
  fs.writeFileSync(shimPath, shimContent);
  fs.chmodSync(shimPath, 0o755);
  console.log(`SSR sidecar shim emitted at ${shimPath}`);
}
