...
Fix agent creation from a local git worktree on a target branch (the dev `minds-start` flow, and any local-worktree source): the clone is now checked out before the worktree overlay rsync. Previously the rsync'd files landed untracked and the subsequent checkout aborted with "untracked working tree files would be overwritten by checkout", failing the create. This affected docker and lima local-worktree creates too, not just smolvm.
