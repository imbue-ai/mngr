...
Re-based the spacing scale so a utility's number is its pixel value. Tailwind computes spacing as `calc(var(--spacing) * N)`; setting `--spacing: 1px` makes `p-2` = 2px and `py-64` = 64px (instead of the stock `0.25rem`, where `p-4` = 16px). Every numeric spacing / sizing utility across the templates, vanilla JS, and `templates.py` constants was multiplied by 4 so the rendered layout is unchanged -- it is a pure renaming for the on-scale values.

- Padding / margin / gap now follow a ten-step scale (2 / 4 / 6 / 8 / 12 / 16 / 24 / 32 / 48 / 64 px). The handful of off-scale spacings were snapped to the nearest step (10->8, 14->12, 20->16, 28->24, 40->32), e.g. inputs/buttons tighten slightly (`py-2.5`->`py-8`, `px-3.5`->`px-12`). Width / height / inset stay free px for layout (so component sizes are untouched), and large fixed dimensions keep their explicit `[NNpx]` values.

- The styleguide gains a "Spacing scale" section. NOTE for contributors: this diverges from Tailwind's documented scale -- here `p-4` is 4px, not 16px -- so follow the styleguide / `app.css`, not upstream examples.

Added two guard tests that hold the new scales: padding / margin / gap utilities must stay on the spacing scale, and corner radius must use `rounded-sm/-md/-lg/-xl` / `-full` / `-none` (no `rounded-2xl`/`-3xl`/`-xs`, no arbitrary `rounded-[..]` except the documented content-frame `rounded-[12px]`). Both scan the authored source while skipping SVG path data. The guard caught the floating workspace menu's `rounded-[10px]`, now snapped to `rounded-lg` (8px).
