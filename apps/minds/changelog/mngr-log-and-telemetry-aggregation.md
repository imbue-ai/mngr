...
The per-tier observability bring-up is documented in the operator runbook `apps/minds/docs/deploy/observability-bringup.md` (resource creation, Vault population, provisioning, the manual Modal workspace step, the dev-tier validation gates, fleet collector rollout, and ongoing operations), with checklist pointers recorded in `apps/minds/docs/deploy/next_deploy.md` for the next deployment.
