...
- The sharing editor no longer pops in and out: the access list and editor controls are visible from the first paint (seeded from the server-rendered config) instead of hiding behind a "Loading..." placeholder, and Update/Disable now refresh the editor in place from the existing sharing-status endpoint instead of reloading the page (the action buttons stay visible but disabled with the inline "Saving changes..." note while a save is in flight). This applies to both the sharing popup and the full-page browser fallback.
