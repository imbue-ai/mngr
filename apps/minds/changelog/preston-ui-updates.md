...
- The Minds Settings surface now holds only per-machine device settings: Appearance (dark mode, promoted from the dev-only styleguide toggle to a persisted setting rendered server-side on every page), Error reporting, a functional default Imbue Cloud region (the same preference the create form pre-selects and writes back), Backup password, and About/version, shown in a widened centered modal. Its connectors/permissions management UI was dropped for now.
