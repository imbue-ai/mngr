...
- The titlebar no longer flickers when opening a workspace: the `/_chrome` wrapper now seeds the workspace accent inline (resolved server-side and passed as `?accent=<agent-id>`), so the freshly loaded wrapper renderer paints the correct titlebar on its first frame instead of coming up neutral until chrome.js is primed. The seed covers the background color, the foreground-contrast (`.titlebar-surface`, so the icons/text aren't briefly dark-on-dark), and the workspace title (`<name> — Minds`, so it doesn't briefly show a hardcoded "Minds").

- Fixed the Back arrow landing on an empty page: opening a workspace's settings page hid + parked the workspace content view and left the empty `/_chrome` wrapper as a back-history entry, so Back showed a blank wrapper. Back off a workspace-scoped local page now returns to the workspace it belongs to (the window remembers the last workspace URL and reopens it when a Back/Forward lands the chrome view on the bare wrapper).
