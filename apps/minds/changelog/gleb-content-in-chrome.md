...
- The titlebar no longer flickers when opening a workspace: the `/_chrome` wrapper now seeds the workspace accent inline (resolved server-side and passed as `?accent=<agent-id>`), so the freshly loaded wrapper renderer paints the correct titlebar on its first frame instead of coming up neutral until chrome.js is primed. The seed covers the background color, the foreground-contrast (`.titlebar-surface`, so the icons/text aren't briefly dark-on-dark), and the workspace title (`<name> — Minds`, so it doesn't briefly show a hardcoded "Minds"). The workspace-scoped local pages (workspace settings, sharing, destroying, recovery) seed the same accent inline too, so navigating to them from a workspace no longer flashes a neutral (white) background before the accent applies.
