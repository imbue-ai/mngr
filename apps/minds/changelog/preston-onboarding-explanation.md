...
- latchkey phase: credentials are encrypted and stored locally, never visible to agents, plus a scrolling carousel of the services latchkey can connect to (sourced from the bundled latchkey services catalog, with vendored simple-icons brand icons). The latchkey tile uses the real brand mark (the hand-drawn key glyph from the imbue.com latchkey page, recolored to brand pink on the olive tile).
