...
It also checks availability: if a name is already taken by an active mind on the selected compute provider (and account, for Imbue Cloud), the field says so right away. Names freed by destroyed minds are treated as available, and the check is case-insensitive. Pressing Create with a known-invalid or taken name surfaces the inline error instead of starting creation.
