...
- When the container is reachable the recovery page immediately runs a
  surgical system-interface restart (which does not interrupt your
  agents), so the workspace recovers with no clicks. When the container
  is unreachable it offers a full "Restart workspace" button instead.
...
- The recovery page's host-health probe no longer starts the workspace
  ...
  reported as unreachable and offered the full workspace restart.
