...
`agent-<hex>.localhost:8421` origin. Minds forwards the shell service
(`system_interface`) by default.

Sharing the workspace (the `system_interface` service) now fans out to every
