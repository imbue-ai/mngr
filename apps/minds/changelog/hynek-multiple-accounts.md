...
Under each service, minds lists the accounts you've connected (read from `latchkey services info <service> --offline`), each with a "Disconnect" action, above the existing per-workspace permission list (now labelled "Allowed on all accounts:").
