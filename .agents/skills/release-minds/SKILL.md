...
Invoke this skill with the release branch name.
...
```bash
mkdir -p ~/project/forever-claude-template/.tmp
cd ~/project/minds_prod && git archive --format=tar HEAD > ~/project/forever-claude-template/.tmp/mngr_sync.tar
