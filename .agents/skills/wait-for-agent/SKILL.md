...
Invoke this skill with an agent name as the first word and optional follow-up instructions after it.

## Tool Guidance

The source Claude skill limited shell access to `uv run mngr list`, the polling loop below, and `find-agent`. Codex skills do not enforce that as a permission boundary, so keep shell usage scoped to this workflow.
