...
Invoke this skill with the target agent name as the first word and the message intent after it.

## Tool Guidance

The source Claude skill limited access to `MNGR_AGENT_NAME`, `uv run mngr message`, file writing, and `find-agent`. Codex skills do not enforce that as a permission boundary, so keep tool usage scoped to composing and sending this message.
...
Write the composed message to a project-scoped temporary file using the Write tool, then send it with `--message-file`. Name the temp file `.tmp/mngr-message-from-YOUR_NAME-to-AGENT_NAME.txt` (using your agent name and the resolved target name):
