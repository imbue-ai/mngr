...
`_build_delegated_vps_provider` now returns a `_DelegatedVpsDockerProvider` subclass instead of the base `VpsDockerProvider`. The base class's `_parse_build_args` was made abstract in this branch (it raises so each concrete provider must bind its own `--<provider>-*` prefix); the imbue_cloud slow path needs a working override that just extracts `--git-depth=N` and forwards everything else to docker. Without this, every slow-path container rebuild would raise `must override _parse_build_args` before any docker work happened.
