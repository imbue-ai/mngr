...
- Added: `auto_shutdown_seconds` field on `VpsDockerProviderConfig` schedules `shutdown -P +N` via cloud-init when set (the value is rounded up to whole minutes, with a 1-minute floor for any positive value); combined with AWS's always-on `InstanceInitiatedShutdownBehavior=terminate`, the instance auto-terminates from inside — useful as a runaway-cost safety net for ephemeral / test hosts.
...
- Changed: `_wait_for_cloud_init` swallows transient `HostConnectionError` per poll so the loop survives the brief SSH disruption when cloud-init's sshd-tuning step restarts sshd; the outer `timeout_seconds` remains the hard wall.
