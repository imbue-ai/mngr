...
- Changed: Cloud-init sshd bump uses a drop-in + reload instead of restart -- `MaxSessions` / `MaxStartups` is written to `/etc/ssh/sshd_config.d/99-mngr.conf` and applied with `systemctl reload ssh` (SIGHUP, no connection drop). The restart was tearing down in-flight SSH connections and hanging the provisioning poll loop on pyinfra's 10s per-command read timeout.
...
- Changed: Cloud-init installs Docker via the Debian `docker.io` package instead of `curl get.docker.com | sh` (~5-15s vs ~60-120s on `t3.small`).
