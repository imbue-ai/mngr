...
- Fixed: gVisor (runsc) host backups captured data only on the first cycle. The outer-side btrfs snapshot helper (`snapshot_helper.sh`) was snapshotting into a single fixed path and recreating it each cycle, but the container reads that path through the gofer, which cached a handle to the first subvolume it opened -- so subsequent cycles read an empty snapshot and restic backed up nothing. The helper now creates each snapshot at a unique caller-named path (`snapshots/<name>`), fails on name collision, and validates cleanup targets to a single path component.
