...
Auth note: to share auth across agents without re-logging-in per agent, sign in once on the host so the file token exists (on Linux a normal `agy` login writes it; on macOS the login lands in the keychain, so produce the file by signing in once from a relocated `$HOME`). See the package README.
