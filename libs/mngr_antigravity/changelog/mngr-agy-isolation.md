...
- agy's first-run NUX is skipped via a seeded `cache/onboarding.json`, and the shared `antigravity-oauth-token` is symlinked/copied into each agent's home so the isolated agy is authenticated without its own login flow.
...
Prerequisite: the user must run `agy` and sign in on the host once so the shared oauth token exists; otherwise `mngr create` errors with guidance.
