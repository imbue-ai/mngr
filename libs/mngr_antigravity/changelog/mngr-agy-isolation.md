...
Auth note: on Linux (mngr's runtime) a normal `agy` login writes the shared file token, so auth is shared across agents automatically. On macOS `agy` uses the keychain, so you sign in the first time you launch each agent. See the package README.
