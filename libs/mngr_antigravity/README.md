...
- `sync_home_settings` (bool, default `true`) -- base the per-agent `settings.json` on a copy of the user's real `~/.gemini/antigravity-cli/settings.json` (so each agent inherits the user's preferences/model/permissions), with `settings_overrides` layered on top. When `false`, start from an empty base. This is a data-source choice, not a separate code path.
