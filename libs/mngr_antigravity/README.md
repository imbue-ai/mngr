...
`auto_allow_permissions = true` auto-approves every tool call without prompting. It is wired through a `PreToolUse` hook that returns `{"decision": "allow"}` (provisioned per-agent; see below), not the `--dangerously-skip-permissions` flag.
...
- **Hooks (lifecycle + auto-allow)**: `mngr_antigravity` provisions a per-agent `hooks.json` and points agy at it with `--add-dir`. A `PreInvocation`/`Stop` pair maintains an `active` marker so the agent reports RUNNING while working and WAITING when idle (agy writes no such marker on its own). When `auto_allow_permissions = true`, a `PreToolUse` hook returning `{"decision": "allow"}` suppresses the permission dialog. Verified live against agy 1.0.3 that hooks both load and execute. Note: the in-TUI `/hooks` command writes to `~/.gemini/antigravity-cli/hooks.json`, which the runtime does *not* read ([antigravity-cli#49](https://github.com/google-antigravity/antigravity-cli/issues/49)); only `~/.gemini/config/hooks.json` and per-workspace `.agents/hooks.json` are executed, so mngr writes its own per-agent file and never relies on the TUI.
