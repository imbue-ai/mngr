...
- **Per-host create with delete-options cascade**: each create makes a Standard-SKU static public IP + a NIC bound to the prepared subnet + a VM. The OS disk, NIC, and public IP are all created with `delete_option=Delete`, so `destroy_instance` deletes only the VM and the rest is reaped automatically — no orphaned resources.
