...
- Added: `allowed_ssh_cidrs` defaults to `0.0.0.0/0` and is fail-open, matching the AWS and GCP providers; a `0.0.0.0/0` range is warned at create time and at `prepare`, prompting you to tighten it for production. Setting `allowed_ssh_cidrs = []` opts out entirely (no SSH allow rule; the NSG's implicit default-deny applies). SSH auth is key-only (`disable_password_authentication=True`), so an open NSG exposes the port but not a usable login.

- Added: `auto_shutdown_seconds` schedules cloud-init `shutdown -P +N`, but an OS shutdown on Azure leaves the VM "Stopped (not deallocated)" which still bills for compute — Azure has no native delete-after-duration like AWS/GCP. The real cost backstop for tests is the session-end orphan scanner. A future improvement is true self-deletion via a managed identity + a cloud-init systemd timer.
