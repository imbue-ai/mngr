...
def _output_prepare_result(
    ...
    if output_format is OutputFormat.HUMAN:
        write_human_line(f"Prepared Azure resource group {result.resource_group} in region {result.region}")
        if state_account_name is not None:
            verb = "Created" if was_bucket_created else "Reused existing"
            write_human_line(f"{verb} Azure state storage account {state_account_name} in region {result.region}")
        if host_identity_name is not None:
            write_human_line(f"Provisioned host-dir managed identity {host_identity_name}")
...
def _output_cleanup_result(
    ...
    if output_format is OutputFormat.HUMAN:
        if deleted_resource_group is None:
            write_human_line(f"Nothing to clean up: no mngr-owned resource group in subscription {subscription_id}.")
        else:
            write_human_line(f"Cleaned up Azure resource group {deleted_resource_group} in region {region}")
        if deleted_account_name is not None:
            write_human_line(f"Deleted Azure state storage account {deleted_account_name} in region {region}")
        if deleted_host_identity_name is not None:
            write_human_line(f"Deleted host-dir managed identity {deleted_host_identity_name}")
