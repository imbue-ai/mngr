...
- **Self-stopping idle watcher**: an idle agent stops its own EC2 instance (so a paused agent costs only EBS), reusing the in-container activity watcher. Because the IMDS hop limit is 1, the container cannot use the self-stop IAM role -- only the outer host can. So on idle the in-container `shutdown.sh` *signals* by touching a sentinel file (`stop-instance-requested`) on the shared host volume rather than killing the container. At host finalization the provider installs (on the outer host) a systemd path unit (`mngr-aws-idle-watcher.path`) that watches the outer-filesystem location of that sentinel; when it appears, a paired oneshot service runs `aws ec2 stop-instances` on this instance via its IAM role. This needs awscli on the host (installed at finalization) and the `mngr-aws` self-stop instance profile (attached by default; see `attach_self_stop_role`). The install is best-effort: if awscli install or the unit setup fails, finalization logs a warning and proceeds with no auto-stop (manual `mngr stop --stop-host` still works). `mngr start` resumes a self-stopped host exactly as it resumes a `mngr stop --stop-host` host.
