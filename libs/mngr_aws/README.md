...
- **No snapshot workflow**: unlike `mngr_modal`, where every sandbox is snapshotted at create time so a hard-killed host can be rehydrated, this provider has no host snapshot workflow today. The AWS client exposes no EBS-snapshot surface (the speculative `create_snapshot` / `list_snapshots` / `delete_snapshot` client methods had no consumers and are not part of `VpsClientInterface`). Restore from a fresh `mngr create` instead.
