...
`cleanup` also deletes the S3 state bucket, with the same safety: it **refuses** (deleting nothing) while the bucket still holds any host state, so it can never strand a managed host's offline state. Destroy those hosts first, then re-run.
