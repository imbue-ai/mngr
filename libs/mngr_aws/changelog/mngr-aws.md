...
- **AWS build args use the `--aws-` prefix**: `--aws-region=` and `--aws-instance-type=` (not `--aws-plan=` -- "instance type" is what AWS docs / console call it). `AwsProviderConfig.default_plan` renamed to `default_instance_type` to match. The old `--vps-region=` / `--vps-plan=` raise a migration error pointing at the new names.
