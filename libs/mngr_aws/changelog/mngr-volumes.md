...
- `mngr aws prepare` now also creates (idempotently) a private, encrypted S3 state bucket, named `mngr-state-<account_id>-<region>` by default or overridable via the new `state_bucket_name` provider config field. The bucket is required infrastructure, so it is prepare's primary job: a missing S3/STS permission, an unresolvable bucket name, or any create failure now fails the command (rather than warning and continuing with a security-group-only prepare).
...
- The per-agent `mngr-agent-<id>-*` EC2 tags are no longer written; agent records and the full offline host record live in the required bucket instead. This removes both the silent 256-char `labels` drop and the `TagLimitExceeded` failure at EC2's 50-tag ceiling (there is no longer a tag mirror or a `TagLimitExceededError`).
...
- When `is_offline_host_dir_enabled` is on, `mngr aws prepare` provisions a least-privilege IAM role + instance profile that lets an instance push its `host_dir` to the bucket. A missing-permission/API failure now fails prepare (the identity is part of its setup when the feature is on); set `is_offline_host_dir_enabled = false` in `[providers.<name>]` to skip the identity entirely. The inline policy grants only `s3:PutObject` / `s3:GetObject` / `s3:DeleteObject` on the bucket's `hosts/*` prefix and `s3:ListBucket` on the bucket.
...
The S3 state bucket is now required infrastructure rather than best-effort (no degraded fallback): mngr raises an actionable error pointing at `mngr aws prepare` when the bucket is absent, on the create/label write path as well as on offline reads, and transient S3 errors on mirror reads/writes propagate instead of being swallowed. With `is_offline_host_dir_enabled` on, host_dir setup at create (the IAM-identity check and sync-daemon install) now fails create on error rather than warning and launching without offline host_dir. `mngr aws cleanup`'s host-dir IAM-identity deletion now raises on a delete failure (still an idempotent no-op when already absent). The offline host_dir *read* path is unchanged: a genuinely empty `host_dir` prefix still returns no volume (with the existing missing-identity diagnostic), and only a bucket probe *error* now propagates.
