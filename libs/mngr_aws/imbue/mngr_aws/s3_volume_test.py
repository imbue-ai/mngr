...
def aws_session() -> Iterator[boto3.Session]:
    with mock_aws():
        ...
        bucket = S3StateBucket(session=session, region=_US_EAST_1, bucket_name=_BUCKET)
...
def test_read_file_returns_object_bytes(aws_session: boto3.Session) -> None:
    _seed(aws_session, {"hosts/abc/host_dir/events/messages.jsonl": b"line1\nline2\n"})
    volume = _raw_volume(aws_session).scoped("hosts/abc/host_dir")
    assert volume.read_file("events/messages.jsonl") == b"line1\nline2\n"


def test_read_file_missing_raises(aws_session: boto3.Session) -> None:
    volume = _raw_volume(aws_session).scoped("hosts/abc/host_dir")
    with pytest.raises(S3StateBucketError):
        volume.read_file("nope.txt")


def test_path_exists_for_file_and_directory(aws_session: boto3.Session) -> None:
    _seed(aws_session, {"hosts/abc/host_dir/logs/out.txt": b"x"})
    volume = _raw_volume(aws_session).scoped("hosts/abc/host_dir")
...
def test_volume_for_host_scopes_to_host_dir(aws_session: boto3.Session) -> None:
    host_id = HostId.generate()
    hex_id = host_id.get_uuid().hex
    _seed(aws_session, {f"hosts/{hex_id}/host_dir/events/e.jsonl": b"evt"})
    bucket = S3StateBucket(session=aws_session, region=_US_EAST_1, bucket_name=_BUCKET)
    volume = bucket.volume_for_host(host_id)
    assert volume.read_file("events/e.jsonl") == b"evt"


def test_host_dir_prefix_has_objects(aws_session: boto3.Session) -> None:
    host_id = HostId.generate()
    hex_id = host_id.get_uuid().hex
    bucket = S3StateBucket(session=aws_session, region=_US_EAST_1, bucket_name=_BUCKET)
    assert bucket.host_dir_prefix_has_objects(host_id) is False
    _seed(aws_session, {f"hosts/{hex_id}/host_dir/x": b"1"})
    assert bucket.host_dir_prefix_has_objects(host_id) is True


def test_write_and_remove_round_trip(aws_session: boto3.Session) -> None:
    volume = _raw_volume(aws_session).scoped("hosts/abc/host_dir")
