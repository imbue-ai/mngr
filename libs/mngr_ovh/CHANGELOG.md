...
- Changed: Per-host build args renamed -- `--vps-datacenter=` is now `--ovh-datacenter=` (`--ovh-region=` accepted as an alias) and `--vps-plan=` is now `--ovh-plan=`. The old `--vps-*` prefix raises a migration error. `--git-depth=` stays shared.
