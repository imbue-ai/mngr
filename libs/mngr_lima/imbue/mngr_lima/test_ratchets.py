...
def test_prevent_bare_print() -> None:
    # The lima btrfs and host-start-race release helpers use print() to signal
    # pass/fail to their parent tests via stdout (the only IPC channel available
    # across the `runuser` privilege drop). loguru would write to stderr by
    # default and confuse the parent's assertion on the "HELPER_RESULT: OK" marker.
...
def test_prevent_yaml_usage() -> None:
    ...
    # config, the release tests write a Lima override.yaml to make the VM bootable
    # in CI, and the start-shape detection tests reference realistic `.yaml` paths.
