...
def test_prevent_bare_print() -> None:
    # The lima btrfs + resize release helpers use print() to signal pass/fail
    # to their parent tests via stdout (the only IPC channel available across
    # the `runuser` privilege drop). loguru would write to stderr by default
    # and confuse the parents' assertion on the "HELPER_RESULT: OK" marker.
...
def test_prevent_yaml_usage() -> None:
    ...
    # config, and the btrfs + resize release tests write Lima override yamls to
    # make their VMs bootable in CI.
    rc.check_yaml_usage(_DIR, snapshot(107))
