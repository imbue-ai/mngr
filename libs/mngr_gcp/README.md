...
- **Offline discovery**: host name and per-agent records are mirrored into instance *metadata* (GCE labels are too restricted -- lowercased `[a-z0-9_-]`, 63 chars -- to hold them), so a stopped VM still appears in `mngr list` and resolves by name without SSH. `discover_hosts_and_agents` reconstructs stopped (`TERMINATED`/`STOPPING`) hosts from labels + metadata.
