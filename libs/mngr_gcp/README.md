...
- **No snapshot workflow**: unlike `mngr_modal`, where every sandbox is snapshotted at create time so a hard-killed host can be rehydrated, this provider has no host snapshot workflow today. `GcpVpsClient.create_snapshot` / `list_snapshots` / `delete_snapshot` are intentionally unwired -- they raise `VpsDockerError` with an actionable "disk snapshot support is not implemented in mngr_gcp" message rather than running real Compute Engine snapshot API calls that nothing else consumes. Restore from a fresh `mngr create` instead.
