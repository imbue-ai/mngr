...
> This plugin is **experimental** — it has not been exercised in a production setting at the same scale as `mngr_modal` or `mngr_vultr`. The shared `mngr_vps` machinery underneath it is well-tested, but GCP-specific defaults and the IAM permission set may change. Treat the security defaults (see "GCP-specific configuration" below) as a starting point: review the firewall rule, image choice, service account, and `auto_shutdown_seconds` before pointing this at production resources.
...
- Instances carry a single discovery label, `mngr-provider=<name>` (the server-side `instances.list` filter and the project-wide cleanup scan). GCE label values are restricted to `[a-z0-9_-]`, so the provider lowercases the value before applying it and applies the same transform to the discovery filter (two provider instances whose names differ only by case would collide — name them distinctly). All other mngr host identity — host id (`mngr-host-id`), human host name (`mngr-host-name`), ISO created-at (`mngr-created-at`), and per-agent records (`mngr-agent-<id>-<field>`) — lives in instance *metadata*, whose values are arbitrary strings (no charset encoding), so a stopped VM still resolves by id/name and lists its agents.
...
- Discovery: `instances.list` filtered by the `mngr-provider` label (the one mngr identity that stays a label, so it can drive the server-side filter), then SSH to each VPS to read host records from the state volume.
...
- **VM-level stop/start (idle-pause + resume)**: `mngr stop` stops the agent's container and then the GCE VM (`instances.stop`), so a paused agent costs only disk storage and the boot disk preserves all state; `mngr start` finds the stopped instance by its `mngr-host-id` metadata (it has no external IP while stopped), starts it (`instances.start`), and rebinds known_hosts to the fresh ephemeral external IP before resuming the container. Needs `compute.instances.stop` / `compute.instances.start`. Mirrors `mngr_aws`; the shared `mngr_vps` base is untouched.
...
- **Offline discovery**: host id, host name, created-at, and per-agent records are mirrored into instance *metadata* (GCE labels are too restricted -- lowercased `[a-z0-9_-]`, 63 chars -- to hold them), so a stopped VM still appears in `mngr list` and resolves by id/name without SSH. `discover_hosts_and_agents` reconstructs stopped (`TERMINATED`/`STOPPING`) hosts from that metadata (the `mngr-provider` label is used only as the server-side discovery filter).
