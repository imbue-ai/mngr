...
- Changed: `ClaudeAgentConfig.merge_with` follows mngr's new assign-by-default semantics — an override's `cli_args` replaces the base's; use `cli_args__extend = [...]` for additive layering.
- Changed: Plugin now uses the structured `TmuxWindowTarget` type for tmux pane targeting; `_send_enter_and_validate` and `_preflight_send_message` take `tmux_target: TmuxWindowTarget` instead of a bare string.
...
- Fixed: `claude_background_tasks.sh` now uses the `=` exact-match prefix in its `tmux has-session` polling loop, so it no longer leaks the transcript streamer and common-transcript converter when a sibling session shares a name prefix.
