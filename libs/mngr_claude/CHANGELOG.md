...
- Changed: mngr now bakes its own Claude hooks into the per-agent config-dir `settings.json` (`$MNGR_AGENT_STATE_DIR/plugin/claude/anthropic/settings.json` — the "user" settings layer Claude reads from `$CLAUDE_CONFIG_DIR`), instead of the project's `.claude/settings.local.json`. Previously a plain `claude` run in the project dir also picked up mngr's hooks and fired them outside mngr where `$MNGR_AGENT_STATE_DIR`/`$MNGR_HOST_DIR` are unset, producing errors like `mkdir: cannot create directory '/events': Permission denied`. Note: this stops *new* leaks; pre-existing hooks already written into `settings.local.json` by a prior mngr are not removed (clean those up manually).
- Changed: `settings_overrides` is now applied as a **config-consistent patch** folded onto that `settings.json` (replacing the previous deep-merge-by-default). A bare key assigns and the narrowing guard hard-errors if it would silently drop a non-empty sibling aggregate from the home base; `key__extend` merges onto the home value (lists concatenate, sets union, dicts merge), and nesting `__extend` merges deeper. So a `permissions__extend = {allow__extend = [...]}` override merges its `allow` while preserving `permissions.defaultMode` from the home settings, whereas a bare `permissions = {allow = [...]}` that would drop `defaultMode` now warns. The narrowing guard is recursive (even a bare key nested inside an `__extend` is now caught). `settings_overrides` now **accumulates across config scopes and `parent_type` inheritance** rather than a higher/child scope replacing the entire lower/parent value.
- Changed: `settings_overrides` now expresses merge intent with a Claude-compatible `__mngr_merge` map keyed by dotted path instead of the `__extend` / `__assign` key suffixes. The suffixes leaked into the generated `settings.json` as keys Claude does not recognise; they are now rejected in `settings_overrides`. A bare key assigns with the narrowing guard, and the narrowing error prints the exact `__mngr_merge` patch to add. A `__mngr_merge` key found in your home `settings.json` is stripped (it is a no-op there).
- Changed: A user-supplied `--settings` (in an agent type's `cli_args` or passed through on the `mngr create` command line) now passes through to `claude` verbatim. mngr injects no `--settings` of its own, and Claude natively layers the user's `--settings` over the config-dir `settings.json`, so the user's hooks and mngr's both fire with no mngr merge code and nothing to collide. In `use_env_config_dir` mode (no per-agent config dir), a user-supplied `--settings` in `cli_args`/`agent_args` is now rejected at provision with a `UserInputError`.
- Changed: `mngr create` no longer requires the project's `.claude/settings.local.json` to be gitignored across the board. The requirement now applies only when the `claude_subagent_proxy` plugin (PROXY mode) actually needs to rewrite user-defined Stop hooks in `settings.local.json` — enforced by that plugin, at the point it writes.
