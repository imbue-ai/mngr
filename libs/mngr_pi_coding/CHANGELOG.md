...
- Added: `version` field on the pi agent type that pins the installed pi CLI. Installation runs `npm install -g @earendil-works/pi-coding-agent@<version>` and provisioning verifies the installed pi matches, erroring on mismatch.
- Added: `update_policy` field that governs pi's startup version check. `NEVER` sets `PI_SKIP_VERSION_CHECK=1` so pi does not phone home to compare against the latest release; `AUTO` leaves the check enabled; `ASK` behaves like `AUTO`. Defaults to `NEVER`.
