...
- Added: `update_policy` field that governs pi's startup version check. `NEVER` sets `PI_SKIP_VERSION_CHECK=1`; `AUTO` leaves the check enabled; `ASK` behaves like `AUTO`. Defaults to `NEVER` when unset.
