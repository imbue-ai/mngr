...
- Changed: Plugin now uses the structured `TmuxWindowTarget` type for tmux pane targeting; `_send_enter_and_validate` takes `tmux_target: TmuxWindowTarget` instead of a bare string, matching the `BaseAgent` API change.
