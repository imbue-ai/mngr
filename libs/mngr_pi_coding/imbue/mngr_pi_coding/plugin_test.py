...
def test_wait_for_ready_signal_non_creating_just_runs_start_action(pi_agent: PiCodingAgent) -> None:
    calls: list[int] = []
    pi_agent.wait_for_ready_signal(is_tui_ready_awaited=False, start_action=lambda: calls.append(1))
    assert calls == [1]
...
def test_wait_for_ready_signal_returns_once_sentinel_present(pi_agent: PiCodingAgent) -> None:
    ...
    calls: list[int] = []
    pi_agent.wait_for_ready_signal(is_tui_ready_awaited=True, start_action=lambda: calls.append(1))
    assert calls == [1]
