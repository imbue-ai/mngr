"""Tests for the shared common-transcript normalizer (opencode / codex / pi-coding)."""
...
def test_assistant_without_finish_reason_gets_null_stop_reason() -> None:
    # codex/pi-coding omit finish_reason; the normalized event still carries the key.
    record = {**_ASSISTANT}
...
def test_same_normalizer_serves_codex_and_pi_sources() -> None:
    # The one shared parser handles every common-transcript harness -- only the
    # ``source`` string differs; codex and pi ride the identical envelope.
    codex = {**_ASSISTANT, "event_id": "cx-1", "source": "codex/common_transcript", "model": None, "usage": None}
    pi = {**_USER, "event_id": "pi-1", "source": "pi-coding/common_transcript"}
    events = parse_common_transcript_lines([_line(codex), _line(pi)])
    assert {e["event_id"] for e in events} == {"cx-1", "pi-1"}
    codex_event = next(e for e in events if e["event_id"] == "cx-1")
    assert codex_event["stop_reason"] == "tool_use" and codex_event["model"] is None
