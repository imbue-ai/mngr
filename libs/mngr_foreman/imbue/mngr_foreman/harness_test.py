...
def test_codex_uses_common_transcript_without_pane_detection() -> None:
    ...
    # codex has no ❯-dialogs; a permission block promotes its state to WAITING and
    # surfaces via the pane-less waiting_reason field instead.
...
def test_pi_coding_uses_common_transcript_without_pane_detection() -> None:
    strategy = transcript_strategy_for("pi-coding")
    assert strategy is not None
    assert strategy.subpath == "events/pi-coding/common_transcript/events.jsonl"
    assert strategy.parse is parse_common_transcript_lines
    # pi-coding has no needs-input state at all.
    assert strategy.uses_pane_dialog_detection is False


def test_all_common_transcript_harnesses_share_one_parser() -> None:
    # codex, opencode, and pi-coding all dispatch to the single shared normalizer
    # -- the whole point of dropping the per-harness parser modules.
    parsers = set()
    for agent_type in ("codex", "opencode", "pi-coding"):
        strategy = transcript_strategy_for(agent_type)
        assert strategy is not None
        parsers.add(strategy.parse)
    assert parsers == {parse_common_transcript_lines}
