...
from imbue.mngr_llm.resources.conversation_db import _warn
from imbue.mngr_llm.resources.conversation_db import _write_stdout
from imbue.mngr_llm.resources.conversation_db import count
from imbue.mngr_llm.resources.conversation_db import insert
from imbue.mngr_llm.resources.conversation_db import last_response_id
...
from imbue.mngr_llm.resources.conversation_db import main
...
def test_lookup_by_name_found(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    db_path = tmp_path / "test.db"
    _create_db(db_path)
    insert(str(db_path), "conv-named", '{"name":"My Chat"}', "2025-01-15T10:00:00Z")

    lookup_by_name(str(db_path), "My Chat")
    captured = capsys.readouterr()
    assert captured.out == "conv-named\n"


def test_lookup_by_name_not_found(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    db_path = tmp_path / "test.db"
    _create_db(db_path)
    insert(str(db_path), "conv-other", '{"name":"Other"}', "2025-01-15T10:00:00Z")

    lookup_by_name(str(db_path), "Nonexistent")
    captured = capsys.readouterr()
    assert captured.out == ""


def test_lookup_by_name_returns_most_recent(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    db_path = tmp_path / "test.db"
    _create_db(db_path)
    insert(str(db_path), "conv-old", '{"name":"Shared Name"}', "2025-01-01T00:00:00Z")
    insert(str(db_path), "conv-new", '{"name":"Shared Name"}', "2025-06-01T00:00:00Z")

    lookup_by_name(str(db_path), "Shared Name")
    captured = capsys.readouterr()
...
def _create_db_with_responses(db_path: Path) -> None:
    """Create a test database with mind_conversations, conversations, and responses tables."""
    _create_db(db_path)
    with sqlite3.connect(str(db_path)) as conn:
...
def test_last_response_id_found(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    db_path = tmp_path / "test.db"
    _create_db_with_responses(db_path)
    with sqlite3.connect(str(db_path)) as conn:
        conn.execute(
    ...
    captured = capsys.readouterr()
    assert captured.out == "resp-new\n"


def test_last_response_id_not_found(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    db_path = tmp_path / "test.db"
    _create_db_with_responses(db_path)

    last_response_id(str(db_path), "nonexistent")
    captured = capsys.readouterr()
    assert captured.out == ""
...
def test_main_lookup_by_name(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    db_path = tmp_path / "test.db"
    _create_db(db_path)
    insert(str(db_path), "conv-via-main", '{"name":"Via Main"}', "2025-01-01T00:00:00Z")

    with _override_argv(["conversation_db", "lookup-by-name", str(db_path), "Via Main"]):
        main()

    captured = capsys.readouterr()
    assert captured.out == "conv-via-main\n"


def test_main_last_response_id(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    db_path = tmp_path / "test.db"
    _create_db_with_responses(db_path)
    with sqlite3.connect(str(db_path)) as conn:
        conn.execute(
            "INSERT INTO responses (id, conversation_id, datetime_utc) VALUES (?, ?, ?)",
            ("resp-main", "conv-main", "2026-03-16T10:00:00Z"),
        )
        conn.commit()

    with _override_argv(["conversation_db", "last-response-id", str(db_path), "conv-main"]):
        main()
