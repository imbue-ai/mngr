...
Environment:
    ...
    MNGR_AGENT_NAME       - This agent's name
    MNGR_HOST_NAME        - Name of the host this agent runs on
    MNGR_AGENT_WORK_DIR   - Agent work directory (contains minds.toml)
...
AGENT_STATE_DIR: Final[str] = os.environ.get("MNGR_AGENT_STATE_DIR", "")
AGENT_NAME: Final[str] = os.environ.get("MNGR_AGENT_NAME", "")
HOST_NAME: Final[str] = os.environ.get("MNGR_HOST_NAME", "")
...
if not _LLM_USER_PATH:
    _early_ts = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S.%f")
...
def _poll_agent_list_forever() -> None:
    """Background thread: periodically run mngr list --format json and cache results."""
    global _cached_agents
    while not _is_shutting_down:
        try:
            result = subprocess.run(
                [*get_mngr_command(), "list", "--format", "json", "--quiet"],
                capture_output=True,
                text=True,
                timeout=60,
            )
            if result.returncode != 0:
                _log(f"mngr list failed (exit {result.returncode}): {result.stderr.strip()}")
                ...
                try:
                    ...
                    _log(f"Failed to parse mngr list JSON output: {e}")
        except subprocess.TimeoutExpired:
            _log("mngr list timed out")
        except (FileNotFoundError, MngrNotInstalledError):
            _log("mngr not found, cannot poll agent list")
        except OSError as e:
...
def _build_template() -> str | None:
    ...
    ``$MNGR_AGENT_STATE_DIR/plugin/llm/template.yml``, matching the
