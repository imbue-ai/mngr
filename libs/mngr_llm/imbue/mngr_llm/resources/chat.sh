...
#
# Environment:
#   MNGR_AGENT_STATE_DIR  - agent state directory (contains events/, commands/)
#   MNGR_AGENT_WORK_DIR   - agent work directory (contains talking/PROMPT.md)
#   MNGR_LLM_MODEL        - model to use for llm commands (default: claude-haiku-4.5)
...
_MNGR_LOG_TYPE="chat"
_MNGR_LOG_SOURCE="logs/chat"
_MNGR_LOG_FILE="${MNGR_AGENT_STATE_DIR}/events/logs/chat/events.jsonl"
# shellcheck source=../../../../mngr/imbue/mngr/resources/mngr_log.sh
source "$MNGR_AGENT_STATE_DIR/commands/mngr_log.sh"

LOG_FILE="$_MNGR_LOG_FILE"
...
# Get the model from MNGR_LLM_MODEL env var, falling back to hardcoded default.
get_model() {
    echo "${MNGR_LLM_MODEL:-claude-haiku-4.5}"
}
...
build_template() {
    local template_dir="$MNGR_AGENT_STATE_DIR/plugin/llm"
    ...
    local system_prompt=""
    local global_md="${MNGR_AGENT_WORK_DIR:-}/GLOBAL.md"

    if [ -n "${MNGR_AGENT_WORK_DIR:-}" ] && [ -f "$global_md" ]; then
        system_prompt="$(cat "$global_md")"
        log "Loaded GLOBAL.md for template (${#system_prompt} chars)"
    fi

    if [ -n "${MNGR_AGENT_WORK_DIR:-}" ] && [ -f "$TALKING_PROMPT" ]; then
...
show_help() {
    ...
    echo ""
    echo "Environment:"
    echo "  MNGR_LLM_MODEL   Model for llm commands (default: claude-haiku-4.5)"
}
