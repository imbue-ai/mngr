...
def test_wait_for_ready_signal_returns_when_sentinel_present(
    ...
    agent.wait_for_ready_signal(is_tui_ready_awaited=True, start_action=_noop_start, timeout=2.0)


def test_wait_for_ready_signal_skips_when_not_creating(local_provider: LocalProviderInstance, tmp_path: Path) -> None:
    agent = _make_opencode_agent(local_provider, tmp_path, OpenCodeAgentConfig())
    # On a restart (is_tui_ready_awaited=False) we do not block on the sentinel.
    agent.wait_for_ready_signal(is_tui_ready_awaited=False, start_action=_noop_start, timeout=0.2)
...
def test_wait_for_ready_signal_raises_when_sentinel_never_appears(
    ...
    with pytest.raises(AgentStartError):
        agent.wait_for_ready_signal(is_tui_ready_awaited=True, start_action=_noop_start, timeout=0.2)
