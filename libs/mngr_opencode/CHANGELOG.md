...
- Added: `update_policy` field that governs opencode's startup auto-update. `NEVER` writes `"autoupdate": false` into the per-agent `opencode.json` so opencode does not update itself on launch; `AUTO` leaves auto-update enabled; `ASK` behaves like `AUTO`. Defaults to `NEVER` when unset. An explicit `autoupdate` key in `config_overrides` always wins.
