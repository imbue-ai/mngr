...
def test_rename_dry_run_does_not_rename(e2e: E2eSession) -> None:
    ...
    # Scope discovery to the local provider (where the agent was created) so the
    # verification does not fan out to credential-requiring cloud providers, which
    # would abort the listing when their credentials are absent.
    list_result = e2e.run(
