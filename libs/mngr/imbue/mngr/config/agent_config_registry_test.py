...
def test_list_registered_agent_config_types() -> None:
    ...
    # This asserts on the exact registry contents, so it needs an empty registry
    # rather than the fixture's placeholder-populated state.
...
def test_get_agent_class_unknown_includes_install_hint_for_known_plugin() -> None:
    ...
    # The autouse fixture loads agent plugins into the class registry; clear it so
    # "claude" is genuinely unregistered (but still cataloged) and the install hint fires.
...
def test_get_orphan_agent_class_returns_none_when_unset() -> None:
    """get_orphan_agent_class returns None when no orphan fallback is set."""
    # The autouse fixture's load_agents_from_plugins sets the orphan slot to
    # BaseAgent; reset it so the "unset" state is the one under test.
    reset_agent_class_registry()
...
def test_is_known_agent_type_accepts_class_and_config() -> None:
    """is_known_agent_type accepts a name with both a registered class and config.

    This is the common case for a normally-installed plugin.
    """
...
def test_is_known_agent_type_accepts_config_only() -> None:
    """is_known_agent_type accepts a name with only a registered config (no class).

    Exercises the config-side OR branch (is_agent_config_registered) in isolation;
    "only-config" has no registered class. This mirrors a partial registration,
    e.g. a plugin that only contributes a config-side hookimpl during inspection.
    """
    register_agent_config("only-config", AgentTypeConfig)
    assert is_known_agent_type("only-config", MngrConfig()) is True


def test_is_known_agent_type_accepts_class_only() -> None:
    """is_known_agent_type accepts a name with only a registered class (no config).

    Exercises the class-side OR branch (is_agent_class_registered) in isolation;
    "only-class" has no registered config.
    """
