...
def test_agent_type_config_merge_with_replaces_permissions() -> None:
    """AgentTypeConfig.merge_with assigns permissions from override (no concat)."""
    base = AgentTypeConfig(permissions=[Permission("read")])
    override = AgentTypeConfig(permissions=[Permission("write")])
    merged = base.merge_with(override)
    assert merged.permissions == [Permission("write")]
