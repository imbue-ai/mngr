    ...
    for raw in (
        ...
        if raw is not None:
            config = config.merge_with(
                parse_config(raw, disabled_plugins=config_disabled_plugins, strict=strict, silent=silent)
            )
...
def _check_unknown_fields(
    ...
    *,
    ...
    When silent=True, removes unknown fields without logging (used by `mngr plugin add`,
    where the config is expected to reference plugins that are about to be installed).
    silent has no effect when strict=True.
    """
...
def parse_config(
    ...
    *,
    ...
    When silent=True, suppresses the unknown-field warnings (used by `mngr plugin add`,
    where the config is expected to reference plugins about to be installed).
    silent has no effect when strict=True.
    """
