...
class OnlineHostInterface(HostInterface, ABC):
    ...
    @abstractmethod
    def is_generated_work_dir(self, work_dir: Path) -> bool:
        """Check if a work directory was generated by mngr.

        Returns True only when mngr created the directory itself (e.g. via
        --transfer=git-worktree, --transfer=git-mirror, or --transfer=rsync).
        Returns False for in-place agents (--transfer=none), whose work_dir
        is a pre-existing user directory that mngr must not delete.
        """
        ...
