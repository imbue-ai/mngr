...
def _message_impl(ctx: click.Context, **kwargs) -> None:
    ...
    # Validate input: must target agents either by name/address, --all, or stdin.
    if agent_addresses and opts.all_agents:
    ...
    if not agent_addresses and not opts.all_agents:
        if not stdin_consumed:
            raise UserInputError("Must specify at least one agent (use '-' to read from stdin) or --all")
        return
...
Use '-' in place of agent names to read them from stdin, one per line.""",
    ...
    examples=(
        ...
        ("Send to all agents matching a filter", "mngr list --ids | mngr message - --message 'Hello everyone'"),
