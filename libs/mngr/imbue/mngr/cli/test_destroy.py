...
def test_destroy_multiple_agents(
    ...
    agent_name1 = f"test-destroy-multi1-{unique_suffix}"
    agent_name2 = f"test-destroy-multi2-{unique_suffix}"
...
def test_destroy_transfer_none_keeps_shared_worktree(
    ...
    """
    unique_suffix = uuid4().hex
    owner_name = f"test-tn-owner-{unique_suffix}"
    rider_name = f"test-tn-rider-{unique_suffix}"
...
def test_destroy_transfer_none_standalone_keeps_user_worktree(
    ...
    agent_name = f"test-tn-standalone-{unique_suffix}"
    session_name = f"{mngr_test_prefix}{agent_name}"
...
def test_destroy_via_stdin(
    ...
    unique_suffix = uuid4().hex
    agent_name1 = f"test-destroy-stdin1-{unique_suffix}"
    agent_name2 = f"test-destroy-stdin2-{unique_suffix}"
    session_name1 = f"{mngr_test_prefix}{agent_name1}"
    session_name2 = f"{mngr_test_prefix}{agent_name2}"
