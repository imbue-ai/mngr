...
def extras(ctx: click.Context, **kwargs: Any) -> None:
    ...
    write_human_line("")
    write_human_line("User-scope config (default agent type, isolation) is set up separately by 'mngr config wizard'.")
...
CommandHelpMetadata(
    key="extras",
    one_line_description="Install optional extras (plugins, completion, Claude Code plugins)",
...
User-scope config settings (default agent type, isolation) are set up by
`mngr config wizard`, not here.""",
    examples=(
    ...
    see_also=(
        ...
        ("config wizard", "Set up common user-scope configuration"),
    ),
).register()
