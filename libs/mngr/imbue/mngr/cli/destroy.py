...
def destroy(ctx: click.Context, **kwargs) -> None:
    ...
    worktrees_to_remove: list[tuple[Path, Path, OnlineHostInterface]] = []
    branches_to_remove: list[tuple[str, Path, OnlineHostInterface, Path]] = []
    ...
    # Filter the queued worktrees so we never delete one mngr did not create
    # (e.g. the user's own worktree reused by an --transfer=none agent), nor
    # one that another still-running agent on the same host depends on.
    # This runs after the destroy phase so host.get_agents() reflects the
    # post-destroy state.
    removable_worktrees, removable_worktree_keys = _filter_removable_worktrees(worktrees_to_remove)

    # Remove worktrees (must happen before branch deletion)
    for work_dir, source_repo_path, _host in removable_worktrees:
        try:
    ...
    # Delete created branches (after worktree removal).  Only delete a branch
    # whose worktree was actually removed -- otherwise the branch is still
    # checked out and `git branch -D` would fail anyway.
    for created_branch, source_repo_path, host, work_dir in branches_to_remove:
        if (host.id, str(work_dir)) in removable_worktree_keys:
...
def _destroy_single_online_agent(
    ...
    try:
        ...
        # Read worktree info before destroy removes the work_dir.  The actual
        # decision to remove (i.e., this work_dir was generated by mngr and is
        # not still in use by another agent) is deferred until after every
        # destroy in this run completes -- see the filter loop below.
        ...
        if source_repo_path is not None:
            if opts.allow_worktree_removal:
                with results_lock:
                    worktrees_to_remove.append((agent.work_dir, source_repo_path, host))
            if opts.remove_created_branch:
                created_branch = agent.get_created_branch_name()
                if created_branch is not None:
                    with results_lock:
                        branches_to_remove.append((created_branch, source_repo_path, host, agent.work_dir))
...
def _filter_removable_worktrees(
    queued: Sequence[tuple[Path, Path, OnlineHostInterface]],
) -> tuple[list[tuple[Path, Path, OnlineHostInterface]], set[tuple[HostId, str]]]:
    """Decide which queued worktrees may safely be removed.

    A worktree is safe to remove only when both conditions hold on its host:
    1. mngr generated it (it appears in the host's certified_data.generated_work_dirs).
       In-place agents (--transfer=none) reuse a pre-existing user directory,
       so removing it would clobber another agent's or the user's own state.
    2. No other agent on that host still references it as work_dir.  Sharing
       happens whenever multiple agents are created with --transfer=none from
       the same parent worktree.  We defer to post-destroy GC, which will
       clean up the directory once the last referencing agent is gone.

    Returns the kept worktrees plus a set of (host_id, work_dir) keys for use
    when deciding which created branches are safe to delete.
    """
    kept: list[tuple[Path, Path, OnlineHostInterface]] = []
    keys: set[tuple[HostId, str]] = set()
    seen: set[tuple[HostId, str]] = set()
    for work_dir, source_repo_path, host in queued:
        key = (host.id, str(work_dir))
        if key in seen:
            continue
        seen.add(key)
        if not host.is_generated_work_dir(work_dir):
            continue
        surviving_agents = host.get_agents()
        if any(str(a.work_dir) == str(work_dir) for a in surviving_agents):
            continue
        kept.append((work_dir, source_repo_path, host))
        keys.add(key)
    return kept, keys
