...
def setup_bootstrap_command_context(
    ...
    2. Top-level fields *loaded from config files* are parsed in non-strict
       mode (unknown fields warn instead of raising), to match
       ``load_bootstrap_context``. There is therefore no ``strict`` parameter
       on this function. Note that ``-S`` overrides typed on the command line
       are still strict for unknown top-level keys: since the user typed them
       explicitly, typos should surface as errors rather than warnings.
    """
    initial_opts, cg, pm = _acquire_command_resources(ctx, command_name, command_class)
...
def _finalize_command_setup(
    ...
    *,
    ...
    # (e.g. disable_plugin from [commands.create] in settings.toml). Both load
    # paths already block plugins from CLI args and from [plugins.<name>]
    # enabled=false in config files (the full path via _apply_plugin_overrides,
    # the bootstrap path via the lightweight read_disabled_plugins() pre-reader
    # invoked at plugin-manager creation time). Command defaults are applied
    # after the initial load and need a second blocking pass.
