...
2. Set `allow_settings_key_assignment_narrowing = true` (a top-level field on `MngrConfig`) to opt into the assign-by-default behavior. Set it in a `settings.toml` or via the `MNGR__ALLOW_SETTINGS_KEY_ASSIGNMENT_NARROWING=true` env var. A `--setting` override cannot toggle this guard: it runs while the settings files and env vars are being loaded, before `--setting` is applied (which is necessary because `--setting`'s `__extend` keys resolve against the already-loaded config).
