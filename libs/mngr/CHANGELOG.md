...
- Added: Agent-type aliases via the new `register_agent_aliases` hook — plugins can register short alternate names (e.g. `agy` for `antigravity`, `pi` for `pi-coding`) accepted anywhere the canonical type is, and tab-completable for `--type`. A custom `[agent_types.X]` block matching a built-in alias takes precedence (alias is dropped with a warning).
- Added: `mngr create --adopt-session` now also resolves a bare session id against every live local mngr agent's per-agent config dir and the preserved session files of destroyed agents (was: only the current and user-scope Claude config dirs). The option is repeatable; when multiple sessions are named, every named session is made available in the new agent but only the last one is resumed on startup.
- Added: Disabled-plugin errors now name the actual registering plugin instead of assuming the plugin name equals the agent-type name (fixes the message for types whose name differs from the plugin's entry-point name, e.g. `pi-coding` registered by the `pi_coding` plugin).
- Added: Tab completion now completes the `-S` / `--setting KEY=VALUE` config override on every command. Keys with a constrained value set (booleans, enums, provider/agent-type names) complete to `KEY=` and list allowed values on the next TAB; free-form keys complete to the bare key name. Config-key completion now drills one dotted segment at a time, and `agent_types.*` completes for builtin/registered agent types (not just custom types in your config).
- Added: Shell completion now installs as a small, stable shim in your rc that sources a managed completion file (`~/.mngr/completions/mngr.{zsh,bash}`) which mngr keeps up to date, so completion improvements apply on upgrade without re-editing your rc. `mngr extras completion` recognizes an outdated install, installs the up-to-date shim, and removes the old self-contained completion block if it matches a form mngr generated.
- Added: `mngr list --schema`, a machine- and human-readable catalog of every field referenceable in `--include` / `--exclude`, `--sort`, and `--fields` / `--format`. The catalog is derived live from the real data shape so it always reflects the actual models, and the `mngr list` help "Available Fields" section is rendered from it.
- Added: `project` field is now usable in CEL filters and sorts (e.g. `--include 'project == "mngr"'`, `--sort project`), mirroring the existing `host.provider` alias and the `--project` flag; previously it only worked in `--fields` / `--format` templates.
- Added: Reusable gitignore-status helpers (`check_path_gitignore_status` / `check_path_repo_gitignore_status`) in `mngr.api.git` that report whether a path is gitignored, resolving symlinks along the path first so `git check-ignore` doesn't choke with "beyond a symbolic link". Plugins use these to guard files they write into an agent worktree.
- Added: Canonical schema for the agent-agnostic common-transcript envelope (`imbue.mngr.agents.common_transcript_records`) and a shared agent release-lifecycle harness (`imbue.mngr.agents.agent_release_testing`). A conformance test asserts that every registered agent plugin's emitted records match the canonical envelope, and a meta-test fails if any registered agent type lacks such a conformance test — a new agent plugin cannot merge without one.
- Added: AWS provider support in the plugin catalog and the remote-backend list (new `imbue-mngr-aws` plugin — see its changelog for the full provider surface). The shared layer picks up `resolve_backend_and_config` and `bootstrap_backend_for_host_creation` helpers, and a new `bootstrap_for_host_creation` method on `ProviderBackendInterface` that replaces the Modal-specific `is_for_host_creation` flag.
- Added: `OPT_IN_PLUGINS` set on the config pre-reader for plugins that are disabled by default and must be explicitly enabled via `[plugins.<name>] enabled = true`. The first opt-in plugin is `claude_subagent_proxy`, which intercepts Claude Code's built-in `Task` tool and interferes with other tooling.
...
- Changed: Provider config parsing now coerces field types — `_parse_providers` switched from `model_construct` to `model_validate`, so an enum field like `builder = "DEPOT"` is now `DockerBuilder.DEPOT` instead of the raw string, and nested-model fields (e.g. SSH provider `hosts` tables) are coerced into their pydantic-model form. This fixes identity checks like `builder is DockerBuilder.DEPOT` silently falling back to the non-depot path and silences pydantic serializer warnings on `model_dump`.
- Changed: New `ScalarTuple` marker type (and the `ScalarStrTuple` annotated alias) for tuple-typed settings fields that are semantically a single scalar value — a higher-precedence config layer that sets one replaces the whole value rather than tripping the settings-narrowing guard. `StringDerivedTuple` is now a specialization. Combining values across config layers is never the intent for such fields.
- Changed: Codex moved out of mngr core into the external `imbue-mngr-codex` plugin, added to the plugin catalog as a recommended INDEPENDENT-tier agent type; the in-core `codex_agent` stub and its direct registration are removed.
- Changed: `pi-coding` agent-type plugin (`imbue-mngr-pi-coding`) is marked as recommended in the plugin catalog so `mngr extras` offers it by default alongside claude, opencode, and antigravity.
- Changed: Resizing the terminal while attached to a remote agent (AWS or any SSH-backed provider) now reflows correctly. The post-attach step no longer pins `window-size` to `manual` via `tmux resize-window -A`; it now only sends `SIGWINCH` so the window keeps tracking the client.
- Changed: De-duplicated the SSH error-translation chain in `OuterHost` (`_run_shell_command`, `_get_file`, `_put_file`, `execute_streaming_command`, and `list_directory` previously copy-pasted the same `except TimeoutError / OSError "Socket is closed" / EOFError, SSHException -> HostConnectionError` block; now a single `_translate_ssh_errors` context manager parameterized by the per-operation messages).
- Changed: Docker provider now raises a typed, actionable `DockerRuntimeNotRegisteredError` when the configured `docker_runtime` (e.g. `runsc` for gVisor) is not registered with the Docker daemon, instead of letting Docker's raw exit-125 `ProcessError` propagate. Renders as a clean message naming the runtime and provider, with `user_help_text` pointing at the fix.
...
- Changed: Regenerated the `mngr schedule` CLI reference docs to include the new `--timezone` option on `schedule add` (implemented in the mngr-schedule plugin).
...
- Fixed: Environment-variable forwarding for remote streaming SSH commands (`OuterHost.execute_streaming_command` with `env=...`). The streaming path prepended env vars as `KEY=VAL command`, which in the shell only applies to the single simple command it precedes — so for a compound command like `install && tool ...` the var was gone by the time the second command ran. It now uses `export KEY=VAL && command`, matching the non-streaming pyinfra path; this is what made remote `depot build` fail with "missing API token" even though `DEPOT_TOKEN` was supplied via `env`.
- Fixed: `_is_transient_ssh_error` now treats Python's built-in `TimeoutError` as transient. pyinfra's `read_output_buffers` raises a bare `TimeoutError` when an SSH command's response doesn't arrive within its per-command read timeout (e.g. when the remote sshd is reloaded mid-read during cloud-init bootstrap); the retry loop now picks it up rather than letting it escape host creation. The host file/streaming/shell-command paths also surface a post-retry `TimeoutError` as a structured `HostConnectionError` and disconnect on `TimeoutError` so retries rebuild the SSH connection rather than reusing the dead channel.
