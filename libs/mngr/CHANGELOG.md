...
- Added: Container hosts now self-heal sshd after an out-of-band restart. The host container entrypoint (re)starts sshd on boot whenever mngr has already provisioned this host (tracked by a marker so a host key pre-baked into the base image is never used by mistake), so a `docker restart`, docker daemon restart, or host reboot brings ssh back without waiting for `mngr start`. mngr's own sshd start is now idempotent.
- Added: `mngr start` is now safe to run concurrently for the same host. The agent (re)launch is serialized by a dedicated cross-actor `flock` (local in-host starts coordinate with remote over-SSH starts), so a desktop-driven start and an in-host boot-hook start cannot race.
- Added: New `--post-host-create-outer-command` create option (and matching create-template / settings key `post_host_create_outer_command`) that runs shell commands once on the host's outer machine after the host is created — e.g. to install a VM-level systemd unit.
- Added: New `[tmux]` configuration section: `tmux.attach_args` (extra tmux client flags before the `attach` subcommand, e.g. `["-CC"]` for iTerm2 control mode); `tmux.additional_config_path` (an extra tmux config file sourced into every mngr session, never overwritten); and `tmux.primary_window_name` (default `agent`) — mngr now names the agent's primary window and targets it by name instead of the literal `:0` index, so mngr works regardless of the user's tmux `base-index`. mngr self-heals in-flight agent sessions on first inspection.
- Added: `mngr config assign <key> <value>` command (mirrors `mngr config extend`). Writes a `key__assign` entry (replace without the narrowing guard), or — on a `settings_overrides` path — a `__mngr_merge` `assign` directive.

### Changed

- Changed: Reworked the config-merge algebra and routed it through the new standalone `overlay` library. A bare `key` assigns (narrowing-checked); `key__extend` merges onto the layer below — now **recursively** (a nested `__extend` merges deeper while a nested bare key still replaces); the new `key__assign` assigns **without** the narrowing warning (the per-key opt-out to `allow_settings_key_assignment_narrowing`). A settings *patch* field (a claude agent type's `settings_overrides`) now accumulates across config scopes (user < project < local) and `parent_type` inheritance instead of being replaced; cross-scope narrowing is surfaced at config-load. New `Static*` markers (`StaticTuple` / `StaticList` / `StaticDict`) mark an aggregate atomic so replacing it is exempt from the narrowing guard.
- Changed: Merge intent for an agent type's `settings_overrides` is now declared with a Claude-compatible `__mngr_merge` map keyed by dotted path instead of the `__extend` / `__assign` key suffixes (the external AI CLI silently ignores `__mngr_merge`, whereas the suffixes surfaced as junk literal keys). Raw `__extend` / `__assign` keys under `settings_overrides` are a hard error pointing to `__mngr_merge`. The narrowing error now prints the exact `__mngr_merge` patch to add. mngr's own (non-`settings_overrides`) config is unchanged and still uses the suffixes.
- Changed: Stop `mngr event --follow` from polling the persisted files of an offline (stopped-but-not-destroyed) agent every second. Each per-source tail thread now lives for the whole follow session and gates its own I/O on a shared online/offline signal: while the target is offline it parks and does no reads at all. On a transition the loop swaps the shared target handle and flips the gate, so threads are never torn down and recreated. Eliminates a wasted-CPU footgun where a handful of stopped Docker-provider agents could drive the engine to ~90% CPU.
- Changed: A `[providers.<name>].isolation` config knob (`CONTAINER` | `NONE`) on the VPS provider family selects whether the agent runs inside a Docker container (default, behavior unchanged) or directly on the VM (bare, reached at `vps_ip:22` as root with the same VPS keypair). Bare placement is gated to providers with a real machine stop/start lifecycle (AWS/GCP/Azure); other providers reject it with `BareIsolationNotSupportedError`. Snapshot support is now structural: a snapshot on a bare placement fails up front with `SnapshotsNotSupportedError`.
- Changed: An interactive TUI agent's `TUI_READY_INDICATOR` may now be either a plain string (matched as exact substring, as before) or a compiled `re.Pattern` (matched with `re.search`). Lets an agent whose ready state can't be captured by a single substring (e.g. an input box bounded by horizontal rules) express it as a regex.

### Fixed

- Fixed: An expected (`MngrError`) error raised during agent provisioning — e.g. a settings-narrowing `ConfigParseError` while building the per-agent `settings.json` — now surfaces as a clean one-line error instead of an "Unexpected error" with a full traceback. The `create` flow unwraps a single expected error from the provisioning concurrency group's `ConcurrencyExceptionGroup`.
- Fixed: `mngr config` is now exempt from the settings-narrowing guard, so a config that would otherwise narrow can still be loaded in order to *edit* it. Previously `mngr config set` / `mngr config unset` — the way to fix a narrowing config — failed with the narrowing error themselves. The guard still fires for every other command.
- Fixed: `mngr transcript` failing with "Unknown agent type" for config-defined agent subtypes (a custom `[agent_types.X]` with a `parent_type`, e.g. `parent_type = "claude"`). The command now resolves the type through its parent chain like every other command, so it recognizes such subtypes and reads the parent's transcript.
- Fixed: `mngr plugin list` mislabeling opt-in plugins (e.g. `claude_subagent_proxy`) as `enabled=true` when they were actually blocked. The reported `enabled` state now reflects the plugin's real block state; `config.disabled_plugins` now faithfully includes opt-in plugins that are disabled by default. Suppressed under `MNGR_LOAD_ALL_PLUGINS` (matching plugin-manager startup).
- Fixed: Sending a message to a resumed interactive TUI agent could time out. The TUI-ready wait now runs inside `send_message` (not only when an agent is first created), so every send path — including the resume message and on-demand restart — waits for the TUI to finish rendering before pasting.
