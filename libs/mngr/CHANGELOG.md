...
- Changed: `mngr create --new-host` now tears down a freshly-created host on *any* failure up to and including the initial-message send (host env-var write, on_host_created hooks, post-host-create commands, locking, provisioning, agent start, and the CLI's `--edit-message` send are now wrapped in one continuous teardown guard), closing a gap where failures between the previous two separate guard blocks could leak the host. The existing `MNGR_DEBUG_RETAIN_LOCK_FOR_FAILED_HOSTS_DURING_CREATE=1` escape hatch still retains the failed host for debugging.
- Changed: `UNPUBLISHED_PACKAGES` (in `plugin_catalog.py`) is now the single source of truth for "deliberately not published to PyPI", consulted by both the install wizard (which never offers an unpublished package) and the release tooling (which auto-discovers every `libs/*` package as a publish candidate and subtracts this set). `imbue-mngr-mapreduce`, `imbue-mngr-claude-subagent-proxy`, and `skitwright` are now listed alongside `imbue-mngr-tmr`.
- Changed: Shared mngr plugin test fixtures (HOME isolation via the autouse `setup_test_mngr_env`, temp host/profile/config dirs, git-repo helpers, and the shell-stub fixtures `stub_mngr_log_sh` / `mngr_transcript_lib_sh`) are now single-sourced in `imbue.mngr.utils.plugin_testing` and exposed through `register_plugin_test_fixtures`; mngr's own `conftest.py` registers that shared set rather than redefining ~20 duplicate fixtures.
- Changed: Tutorial-tied e2e tests (those with `write_tutorial_block()` calls corresponding to blocks in `mega_tutorial.sh`) have moved into a `tutorial/` subdirectory under `libs/mngr/imbue/mngr/e2e/`, so other e2e tests can live at the top level without the tutorial-matcher script treating them as tutorial gaps.

### Fixed

- Fixed: Shell-quoting bug in agent launch where extra `agent_args` (those passed after `--` on `mngr create`) were spliced into the launch command unquoted. Each element now goes through `shlex.quote` via a shared `quote_agent_args` helper (also used by `mngr_claude`), so a value like `--model "Gemini 3.5 Flash (Medium)"` no longer fails the shell with `syntax error near unexpected token '('`. `cli_args` are left unquoted (string-form configs are already shlex-split shell-safe). Fixes every agent type that inherits the base, including `antigravity`/`agy`.
- Fixed: `ProviderInstanceConfig.merge_with` now only overrides the provider fields the higher-precedence layer actually set (via `model_fields_set`, matching `AgentTypeConfig`/`PluginConfig`). Previously a higher layer touching the provider block at all would silently reset any field whose default was non-None (e.g. a `bool` defaulting to `False`, an empty tuple); concretely, applying `providers.lima.is_enabled=true` reset `is_host_in_docker`, `install_gvisor_runtime`, and `default_container_run_args` to defaults, so the Lima provider silently ran in direct-in-VM mode instead of docker-in-VM mode.
