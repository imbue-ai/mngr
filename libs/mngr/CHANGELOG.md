...
- Added: `mngr observe --discovery-only` now honors `--events-dir` — `DIR` relocates the whole discovery stream (snapshots, host SSH info, discovery errors) under `DIR`, so multiple discovery-only observers can run against the same host without their snapshots cross-contaminating each other's streams. Implemented via a new internal `MngrConfig.events_base_dir_override` field.
...
- Changed: The discovery event log now always lives under the default host dir. The internal `MngrConfig.events_base_dir_override` config field was removed, and passing `--events-dir` together with `--discovery-only` to `mngr observe` is now a usage error (it never affected the discovery log in that mode). `--events-dir` still relocates the full observer's agent-state events.
