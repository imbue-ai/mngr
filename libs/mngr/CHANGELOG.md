...
- Removed: **Breaking** -- `--agent` / `--host` flags on `mngr snapshot create` / `list` / `destroy`. Targets are now a single positional list -- write `agent`, `agent@host[.provider]`, `@host[.provider]`, or a bare `host-...` ID instead.
- Removed: **Breaking** -- `--provider` flag on `mngr message` (alias `msg`). The set of providers to query is now derived from the agent addresses themselves (union of providers named by the addresses, or a full scan if any address omits its provider), so `--provider` can no longer contradict the address and query the wrong provider.

### Fixed

- Fixed: Volume garbage collection no longer wipes per-host data when the Docker daemon is briefly unavailable. The Docker provider's `discover_hosts` had silently returned an empty host list on daemon-unreachability, so GC treated every volume as orphaned and deleted it; it now raises `ProviderUnavailableError` and GC skips an unavailable provider at its own boundary.
- Fixed: Multi-provider discovery no longer silently skips a provider whose backend is down. Commands that scan every provider (`message`, `limit`, `snapshot`, `create`) now fail loudly with a "provider is not available" error instead of quietly omitting agents on the unreachable provider. Targeted commands (`mngr rsync`, `mngr git push` / `pull`, `mngr event <host>`, `mngr event @host[.provider]`) now scope discovery to only the provider(s) that could hold the target, so an unrelated down provider no longer fails them.
- Fixed: `mngr <command> --help` DESCRIPTION (and other prose) sections in the pager / rich path are now indented to the man-page depth of seven spaces, matching the piped/plain output and the surrounding sections.
