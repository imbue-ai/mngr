Added a reusable `AutoToggle` enum (`yes` / `auto` / `no`) to `mngr.primitives` for force-on / best-effort / force-off CLI options, with a shared `AUTO_TOGGLE_HELP_SUFFIX`. The `mngr aws` / `mngr azure` `prepare --use-offline-host-dir` flag uses it.
