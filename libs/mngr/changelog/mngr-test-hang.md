...
`Host.stop_agents`, `Host.destroy_agent`, and `ProviderInstance.destroy_host` now return `list[CleanupFailure]`. `execute_idempotent_command` gains an opt-in `raise_on_timeout` flag that normalizes the two backends' differing timeout signals (local: a killed process; remote: `socket.timeout`) into a single `CommandTimeoutError`; other callers are unchanged. See `specs/cleanup-error-aggregation.md`.
