...
- **Reply**: type into the panel's `reply>` input and press `Enter` to send the message to that agent (equivalent to `mngr message`); an empty reply does nothing. The panel shows a brief `sending...` while delivery is in flight (`mngr message` can wait up to ~90s for the agent's submission signal) and clears it once delivery returns; the reply itself appears in the panel body as it lands. The send runs in the background, so the panel stays live. Sending to a stopped or non-interactive agent reports the error inline.
