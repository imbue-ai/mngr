...
- **Peek** (`Space`): open a live bordered panel below the board showing the focused agent's recent user/assistant conversation (via `mngr transcript --role user --role assistant`), refreshed every couple of seconds, with the board still visible above. Tool calls and framework-injected turns do not appear, so the peek reads like the human conversation. Each message's `[timestamp] role:` header is trimmed to a dim role cue. Messages render as markdown (via `markdown-it-py`): inline code and fenced blocks, bold, emphasis, headings, lists, and blockquotes are styled, and links show their text without the URL. Each message is rendered before the window is trimmed, so a code block cut off by the trim still renders as code; anything that isn't markdown renders as plain text. It shows the newest lines, so a long final message renders its end (the agent's conclusion, or the question it is waiting on) under a `⋯` marker, rather than mirroring the agent's scrolled-up screen. The panel says `(no messages yet)` when there is no readable message.
