...
- **Peek** (`Space`): open a live bordered panel below the board showing the focused agent's recent user/assistant conversation (via `mngr transcript --role user --role assistant`), refreshed every couple of seconds, with the board still visible above. Tool calls and framework-injected turns do not appear, so the peek reads like the human conversation. Each message's `[timestamp] role:` header is trimmed to a dim role cue. Markdown in the messages is accented in place -- `code` spans and ``` fences, **bold**, headings, and list markers -- with the syntax characters kept but dimmed, so anything that isn't markdown renders as plain text. A fence cut off by the trim still renders as code (the styler tracks fence state across the whole transcript). It shows the newest lines, so a long final message renders its end (the agent's conclusion, or the question it is waiting on) under a `⋯` marker, rather than mirroring the agent's scrolled-up screen. The panel says `(no messages yet)` when there is no readable message.
