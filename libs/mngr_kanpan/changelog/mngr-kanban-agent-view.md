...
- Peek at an agent with `Space`: a live panel below the board shows the agent's most recent user/assistant messages (via `mngr transcript`), refreshed every couple of seconds, with the board still visible above. Tool events are not shown, so the peek reads like the conversation; tool-only turns (which have no text) are skipped to reach the last message with text, however far back it sits -- there is no fixed window. It shows the newest lines, so a long final message renders its end under a `N earlier lines hidden` marker, rather than being cut off at the top or mirroring the agent's scrolled-up screen. The panel says `(no messages yet)` only when the transcript has no readable message at all. `Esc` closes the panel; to peek a different agent, close it, move on the board, and press `Space` again.
