...
- Changed: Replaced direct `ValueError` raises in the constrained `primitives` types and the `RegexPattern` validator with dedicated `InvalidPrimitiveValueError` / `InvalidRegexPatternError` exception types.

### Fixed

- Fixed: `PREVENT_BUILTIN_EXCEPTION_RAISES` ratchet regex now matches `raise ValueError("literal")` and `raise OSError()` forms (a trailing `\b` after the opening paren previously skipped any raise whose first argument did not start with a word character). The ratchet now also excludes test files, consistent with tests legitimately raising built-in exceptions to simulate error conditions.
