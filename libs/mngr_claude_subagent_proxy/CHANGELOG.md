...
- Changed: The subagent-proxy's own hooks (PROXY spawn/cleanup/reap, DENY deny/reap, and the proxy-child permission auto-allow) now go into the agent's per-agent config-dir `settings.json` (`$MNGR_AGENT_STATE_DIR/plugin/claude/anthropic/settings.json`) instead of the project's `.claude/settings.local.json`, keeping mngr-injected hooks out of the "normal" Claude config that plain (non-mngr) `claude` reads. In `use_env_config_dir` mode the hooks land in the private mngr-managed `--settings` file. The plugin still writes `settings.local.json` for the one thing that genuinely concerns the user's hooks — wrapping user-defined Stop/SubagentStop commands with the `MNGR_CLAUDE_SUBAGENT_PROXY_CHILD` guard so they no-op inside proxy children — and now enforces the gitignored-`settings.local.json` requirement only at the point it makes that wrap (and only when there is actually a user Stop hook to wrap), rather than `mngr create` enforcing it unconditionally for every Claude agent.
