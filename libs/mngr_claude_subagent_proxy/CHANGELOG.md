...
- Changed: **Breaking** — `claude_subagent_proxy` plugin is now **disabled by default** and must be explicitly opted into via `[plugins.claude_subagent_proxy] enabled = true` (it inverts the usual load-unless-disabled default). The plugin is very experimental and interferes with a lot of other tooling — it intercepts Claude Code's built-in `Task` tool.
- Changed: Provisioning artifacts moved under a `mngr-proxy/` subdirectory and the plugin now refuses to write either artifact into a git-tracked worktree where the path is not gitignored (raises a clear error instead of silently leaving an untracked file). The PROXY-mode agent definition moves from `.claude/agents/mngr-proxy.md` to `.claude/agents/mngr-proxy/proxy.md` and the DENY-mode skill from `.claude/skills/mngr-subagents/SKILL.md` to `.claude/skills/mngr-proxy/SKILL.md` (the DENY skill is renamed from `mngr-subagents` to `mngr-proxy`), so a single `.gitignore` line per artifact suffices. Discovery is unaffected: Claude Code identifies the subagent by its frontmatter `name:` field.
