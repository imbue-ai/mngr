...
- Changed: The subagent-proxy's own hooks (PROXY spawn/cleanup/reap, DENY deny/reap, and the proxy-child permission auto-allow) now go into the agent's per-agent config-dir `settings.json` (matching where `mngr_claude` now bakes its own hooks) instead of the project's `.claude/settings.local.json`. In `use_env_config_dir` mode the hooks land in the private mngr-managed `--settings` file.

- Changed: The plugin still operates on `settings.local.json` to wrap user-defined Stop/SubagentStop commands with the `MNGR_CLAUDE_SUBAGENT_PROXY_CHILD` guard, and the requirement that the file be gitignored now lives in this plugin (verified right before the rewrite, only when there is actually a user Stop hook to wrap) rather than `mngr create` enforcing it unconditionally for every Claude agent.
