"""Regression test: gemini_background_tasks.sh must exit promptly when its
...
def state_dir_with_log_lib(tmp_path: Path) -> Path:
    """Lay out a minimal MNGR_AGENT_STATE_DIR the gemini background script needs."""
    state_dir = tmp_path / "state"
    (state_dir / "commands").mkdir(parents=True)
    (state_dir / "activity").mkdir()
    (state_dir / "events" / "logs" / "gemini_background_tasks").mkdir(parents=True)
...
@pytest.mark.tmux
def test_gemini_background_tasks_exits_when_target_session_is_gone_despite_prefix_collision(
    ...
    script = importlib.resources.files(_gemini_resources) / "gemini_background_tasks.sh"
