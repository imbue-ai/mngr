...
| `agent_field_generators`   | Return functions for computing additional fields for agents. Fields are namespaced under `plugin.<plugin_name>`. [experimental]                 |

**Dependency ordering:** Dependency ordering (allowing plugin A's fields to depend on values computed by plugin B) is not yet supported. Field generators receive the agent and host objects directly.
