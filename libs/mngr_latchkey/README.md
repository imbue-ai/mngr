...
* `GET /permissions/available/<service_name>` returns the permission
  ...
  as a `{"scope": "<schema_name>", "permissions": ["...", ...]}`
  object, or 404 if the service is unknown. The catalog is backed by
  a `services.json` file (keyed by raw service name) that ships
  alongside the extension; the path query parameter is not consulted.
