...
def _merge_call(outer: OuterHostInterface) -> str:
    """What the machine's CLI was told to take out of the bundle the last push carried."""
    merges = [call for call in as_vps(outer).latchkey_calls() if call.startswith("auth re-encrypt")]
...
def test_connecting_a_service_keeps_what_the_machine_already_had(tmp_path: Path) -> None:
    ...
    assert as_vps(outer).machine_accounts() == {"slack": ["already@example.com", "new@example.com"]}
...
def test_connecting_a_second_account_leaves_the_machines_first_alone(tmp_path: Path) -> None:
    ...
    """
    host_id = HostId.generate()
    outer = fake_vps(tmp_path, {"slack": ["first@example.com"]})
    ...
    assert _merge_call(outer).endswith("--services slack --account second@example.com")
    assert as_vps(outer).machine_accounts() == {"slack": ["first@example.com", "second@example.com"]}
...
def test_connecting_without_an_account_hands_over_every_stored_account(tmp_path: Path) -> None:
    ...
    host_id = HostId.generate()
    outer = fake_vps(tmp_path, {"slack": ["old@example.com"]})
    credentials = _credentials_of(
    ...
    assert "--account" not in _merge_call(outer)
    assert as_vps(outer).machine_accounts() == {"slack": ["a@example.com", "b@example.com"]}
...
def test_connecting_a_service_costs_one_round_trip_and_no_read_back(tmp_path: Path) -> None:
    ...
    startup) once more for every change made.
    """
    host_id = HostId.generate()
    outer = fake_vps(tmp_path, {"slack": ["already@example.com"]})
...
def test_a_machine_store_written_here_is_not_lost_by_a_later_operation(tmp_path: Path) -> None:
    ...
    assert as_vps(outer).machine_accounts() == {"slack": ["fresh@example.com"]}
...
def test_a_push_refuses_a_machine_rekeyed_out_from_under_this_computer(tmp_path: Path) -> None:
    ...
    host_id = HostId.generate()
    outer = fake_vps(tmp_path)
    as_vps(outer).hold({"slack": ["a@example.com"]}, key="another-installs-key")
    as_vps(outer).run_under_key("another-installs-key")
    credentials = _credentials_of(tmp_path, host_id, outer, machine_accounts={"slack": ["a@example.com"]})
...
def test_connecting_a_service_tells_the_machine_exactly_what_to_take(tmp_path: Path) -> None:
    ...
    assert _merge_call(outer).endswith("--services slack --account a@example.com")


def test_a_read_adopts_an_account_this_computer_has_never_seen(tmp_path: Path) -> None:
    """The machine owns its credentials: an account connected on the user's other computer, or on the machine itself, is not ours to delete."""
    host_id = HostId.generate()
...
def test_a_permissions_snapshot_reaches_a_machine_that_has_lost_its_key(tmp_path: Path) -> None:
    ...
    assert as_vps(outer).machine_permissions() == SLACK_GRANTED
    assert as_vps(outer).secret(GATEWAY_ENCRYPTION_KEY_FILENAME) is None
    assert len(as_vps(outer).recorded) == 1
    assert as_vps(outer).written == []
...
def test_connecting_carries_this_computers_half_of_the_config_with_the_credential(tmp_path: Path) -> None:
    ...
    machine_config = json.loads(as_vps(outer).machine_config() or "")
