...
def _terminate_pid(pid: int) -> None:
    ...
    (otherwise a recycled PID is signalled).
    """
    try:
...
class LatchkeyForwardSupervisor(MutableModel):
    ...
    observe_exit_alongside_pid: int | None = Field(
        default=None,
        frozen=True,
        description=(
            "When set, the spawned forward's ``mngr observe`` discovery child is launched with "
            "``--exit-alongside-pid PID`` so it terminates when that PID exits. Only the producer "
            "is tied to the PID; the detached forward (and its gateway and reverse tunnels) is "
            "not. The minds desktop client passes its own PID so the single discovery producer "
            "dies with the app -- keeping stale/duplicate producers from outliving the app and "
            "polluting the shared discovery-events file -- while the gateway survives a restart."
        ),
    )
    ...
    def ensure_running(self) -> LatchkeyForwardInfo:
        ...
        * If no record exists, a fresh supervisor is spawned.
        ...
        The single-discovery-producer invariant is not enforced here by
        hunting the process table. Instead the embedder ties the forward's
        ``mngr observe`` child to its own lifetime via
        :attr:`observe_exit_alongside_pid`, so a prior instance's producer
        dies with that instance rather than lingering to race the shared
        events file. A fresh forward is launched from the current binary
        whenever there is no live record to adopt.
            ...
            with log_span(
                ...
                try:
                    pid = spawn_detached_mngr_latchkey_forward(
                        ...
                        cwd=self.cwd,
                        observe_exit_alongside_pid=self.observe_exit_alongside_pid,
                    )
                except OSError as e:
