 ...
 *                                            fields "agent_id", "scope",
 *                                            "rationale" and the string-list
 *                                            field "permissions". The
 *                                            ``request_id`` is generated by
 *                                            the extension (a random
 *                                            UUID-shaped string) and returned
...
const PERSISTED_STRING_FIELDS = ['request_id', 'agent_id', 'scope', 'rationale'];
const POST_BODY_STRING_FIELDS = ['agent_id', 'scope', 'rationale'];
const POST_BODY_STRING_LIST_FIELDS = ['permissions'];
...
/**
 * Parse the POST body. The body must contain ``agent_id``, ``scope``,
 * and ``rationale`` as non-empty strings, and ``permissions`` as a
 * (possibly empty) array of strings. Any other field (including a
 * caller-supplied ``request_id``) is rejected so the server side stays
 * the single source of truth on identity.
 */
async function parsePermissionRequestBody(request) {
  ...
  const extraneousFields = Object.keys(parsed).filter((name) => !allowedFields.has(name));
