 ...
 *                                            fields "agent_id", "key",
 *                                            "rationale" and the string-list
 *                                            field "value". The ``request_id``
 *                                            is generated by the extension (a
 *                                            random UUID-shaped string) and
 *                                            returned in the response object.
 *                                            Key and values correspond to Detent's
 *                                            keys and values that are meant to be added.
 *                                            (See the `/permissions/available/<service_name>`
 ...
 * `<latchkey-directory>/permission_requests/v1/<request_id>.json`,
 * where `<latchkey-directory>` is `LATCHKEY_DIRECTORY` if set,
 * otherwise `~/.latchkey`. The ``v1`` segment is part of the on-disk
 * schema version: any pre-v1 files that happen to live in the parent
 * directory are simply not visited and are therefore ignored.
 *
...
const PERSISTED_STRING_FIELDS = ['request_id', 'agent_id', 'key', 'rationale'];
const POST_BODY_STRING_FIELDS = ['agent_id', 'key', 'rationale'];
const POST_BODY_STRING_LIST_FIELDS = ['value'];
...
function resolvePermissionRequestsDirectory() {
  return join(resolveLatchkeyDirectory(), 'permission_requests', 'v1');
}
...
/**
 * Parse the POST body. The body must contain ``agent_id``, ``key``,
 * and ``rationale`` as non-empty strings, and ``value`` as a (possibly
 * empty) array of strings. Any other field (including a caller-supplied
 * ``request_id``) is rejected so the server side stays the single
...
async function parsePermissionRequestBody(request) {
  ...
  for (const field of POST_BODY_STRING_FIELDS) {
  ...
  for (const field of POST_BODY_STRING_LIST_FIELDS) {
    const value = parsed[field];
    if (!Array.isArray(value) || !value.every((item) => typeof item === 'string')) {
      throw new InvalidRequestBodyError(
        `field '${field}' is required and must be an array of strings.`,
      );
    }
  }
  const allowedFields = new Set([...POST_BODY_STRING_FIELDS, ...POST_BODY_STRING_LIST_FIELDS]);
  const extraneousFields = Object.keys(parsed).filter((key) => !allowedFields.has(key));
