...
def list_host_permissions_paths(data_dir: Path) -> list[Path]:
    ...
    Walks each ``<data_dir>/hosts/<host_id>/`` subdirectory and collects
    its ``latchkey_permissions.json`` when present; host directories
    without one (and stray non-directory entries) are skipped. Returns
    an empty list when the hosts directory itself does not exist yet.
