...
suggested Bash command, and uses the script's stdout as if it were the
Task tool's `tool_result`.

The per-Task-call wait-script (one per `tool_use_id`) is generated by
the deny hook under `$MNGR_AGENT_STATE_DIR/proxy_commands/`. It does
the full spawn + wait + print in a single Bash call:

1. Captures the parent's env to a temp file (under EXIT trap so a
   partial write cannot leak secrets).
2. Runs `mngr create --reuse --type claude --label
   mngr_subagent_proxy=child --message-file <prompt>`.
3. Blocks on `imbue.mngr_subagent_proxy.subagent_wait` until the
...
user) resolves with `mngr connect <name>` and re-runs the script; the
init flag makes it idempotent.

What deny mode installs:
- `PreToolUse:Agent` hook only.
- `.claude/skills/mngr-subagents/SKILL.md` -- the verbose protocol
  context, loaded on demand by Claude.
- Per-Task-call sidefiles under `$MNGR_AGENT_STATE_DIR/`:
  - `subagent_prompts/<tool_use_id>.md` (the original Task prompt).
  - `proxy_commands/wait-<tool_use_id>.sh` (the executable wait-script).
...
- No `subagent_map/` (cascade-destroy state) or `subagent_results/`
  (Haiku-relayed body) sidefiles.

Children spawned in deny mode carry the `mngr_subagent_proxy=child`
label so they still hide from
`mngr list --exclude 'labels.mngr_subagent_proxy == "child"'`.
