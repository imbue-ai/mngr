...
# There is work to do. Background it so the SessionStart hook returns
# immediately and does not block Claude session startup.
do_reap() {
    ...
    for map_file in "${MAP_FILES[@]}"; do
        ...
        case "$state_upper" in
            DONE|STOPPED|FAILED|DESTROYED|TERMINATED)
                # Detach so the reap background task does not block.
                (
