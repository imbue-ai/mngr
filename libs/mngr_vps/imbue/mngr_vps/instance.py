...
class VpsProvider(BaseProviderInstance):
    ...
    def get_ssh_host_public_keys(self, host_id: HostId) -> tuple[str | None, str | None]:
        ...
        # container's sshd), so the public halves are known deterministically with
        # no scan. Surfaced via ``mngr create --format json`` for the pool bake to
        # persist and pin.
        _vps_host_key_path, vps_host_public_key = self._get_vps_host_keypair()
