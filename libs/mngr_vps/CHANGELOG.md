...
- Added: A `HostRealizer` seam inside the VPS provider as the first step toward running agents directly on a cloud VM (no Docker container). The provider now selects a realizer from a new `isolation` config knob (`IsolationMode.CONTAINER` | `NONE`); `CONTAINER` is the default and preserves the original behavior exactly. All Docker-container placement logic (image build/pull, container run, in-container sshd setup, btrfs volume + snapshot helper, container stop/start/teardown, and `docker commit` snapshots) moved behind a `DockerRealizer`. A new `BareRealizer` places the agent directly on the VM's OS (no Docker), reached at `vps_ip:22` as root with the same VPS keypair. Host-record store resolution also moved behind the realizer (`realizer.open_host_store(outer, host_id)`).
- Added: Bare placement (`isolation=NONE`) is gated to providers with a machine stop/start lifecycle: a provider without one rejects it at create time with `BareIsolationNotSupportedError`. AWS, GCP, and Azure all enable it. A bare create also rejects container-only inputs up front (image override, Dockerfile build, docker-run start-args) rather than silently ignoring them.
- Added: Snapshot support is now a structural fact rather than a method that raises. `snapshot_placement` / `delete_snapshot_placement` moved off the base `HostRealizer` into a narrow `SnapshotCapableRealizer` sub-interface that only the container realizer implements. A snapshot request on a bare placement now fails up front with `SnapshotsNotSupportedError`.

### Changed

- Changed: Renamed the package from `mngr_vps_docker` to `mngr_vps` (the distribution `imbue-mngr-vps-docker` to `imbue-mngr-vps`), since Docker is now one of two placement shapes rather than the whole package. The shape-agnostic classes dropped "Docker" from their names: `VpsDockerProvider` → `VpsProvider`, `VpsDockerProviderConfig` → `VpsProviderConfig`, `MinimalVpsDockerProvider` → `MinimalVpsProvider`, `OfflineCapableVpsDockerProvider` → `OfflineCapableVpsProvider`, `VpsDockerHostRecord` → `VpsHostRecord`, `VpsDockerHostStore` → `VpsHostStore`, error base `VpsDockerError` → `VpsError`. The genuinely Docker-specific `DockerRealizer` keeps its name. Mechanical rename; no behavior change.
- Changed: The agent container's PID-1 entrypoint now self-heals sshd: on every (re)start it restarts sshd once mngr has provisioned a host key, so the container is reachable again after a VM reboot or `docker restart` without waiting for `mngr start`. The explicit sshd (re)start during setup and after a container restart is now idempotent.
- Changed: Register the gVisor (runsc) runtime with `--overlay2=none` so a container's writable layer is written through to the persistent Docker overlay2 layer and survives a `docker stop`/`start` or host reboot. Previously runsc used its default per-sandbox overlay (`--overlay2=root:self`), recreated on every start, so the injected sshd host key, the `/mngr` host_dir symlink, and mngr's provisioning markers were silently lost on restart. Applies to every provider that installs runsc (aws, vultr, ovh, gcp, azure, imbue_cloud).
- Changed: The shared `OfflineCapableVpsProvider` now owns the cloud stop/start lifecycle: `stop_host` pauses the whole instance and `start_host` resumes it, doing the resumed record's on-volume write *and* its external-store mirror together in one place. Providers supply only the `_pause_cloud_instance` / `_resume_cloud_instance` hooks and override `_sync_host_dir_before_pause` / the known_hosts rebind where their behavior differs.
- Changed: Per-agent instance-tag mirror replaced with a single uniform external `HostStateStore` per provider. AWS/Azure use their object-storage state bucket as the sole offline store (a stopped host's offline metadata now requires the bucket; the provider raises an actionable `missing_state_bucket_error` pointing at `mngr <cloud> prepare` when absent); GCP uses a lossless instance-metadata-backed store (full host record + one JSON value per agent). AWS/Azure/GCP now extend `OfflineCapableVpsProvider` directly.
- Changed: New shared `cli_helpers` module owns the `[providers.<name>]` lookup and the cleanup-refusal guard (`refuse_if_managed_resources_exist`); AWS/Azure/GCP/OVH CLIs delegate. The refusal now raises a single `ManagedResourcesExistError` (a `VpsError` / `MngrError`) across all providers, so `mngr <cloud> cleanup` renders the "refusing to clean up" message identically whichever cloud you are on. New config bases `OfflineCapableVpsProviderConfig` and `PublicIpVpsProviderConfig` unify `allowed_ssh_cidrs` to `ScalarStrTuple`.
- Changed: `BaseObjectStoreVolume` deduplicates the cloud state-bucket / offline-read-volume layer. `S3Volume` / `BlobVolume` become thin subclasses supplying only per-cloud SDK primitives and an error seam.
- Changed: Hardened systemd unit generation. A new `imbue.mngr_vps.systemd.render_systemd_unit` helper centralizes the unit-file format; the idle-watcher poweroff action and the host_dir-sync command now run from installed `/usr/local/sbin/mngr-idle-watcher.sh` and `/usr/local/sbin/mngr-host-dir-sync.sh` scripts referenced by `ExecStart` (instead of inline `ExecStart=/bin/sh -c '...'`), removing systemd-plus-shell quoting around the sentinel path and target URI. No behavior change.

### Fixed

- Fixed: A running bare (`isolation=NONE`) host is now discoverable and reachable with the default provider config — `mngr conn <agent>` (and every other operation: list, stop, start, destroy, snapshot, label, agent persistence) no longer requires re-specifying `-S providers.<cloud>.isolation=NONE` at connect time. Instances now carry a `mngr-isolation` instance marker (an EC2/Azure tag, a GCE metadata item) stamped at create and readable from the cloud API without SSH. Pre-existing hosts have no marker and default to container.
- Fixed: On resume, the shared `OfflineCapableVpsProvider.start_host` now waits for the VM to actually serve mngr's expected host key (on port 22, using mngr's VPS host public key) before its strict-host-key-checked connect — mirroring create's host-key wait. Previously it waited only for *any* sshd handshake, which on GCP's startup-script ssh restart could land mid-window and fail `mngr start` with "Unable to connect to port 22". Surfaces for bare placement specifically (its agent endpoint is port 22; containers use a separate stable port).
- Fixed: `mngr snapshot delete` on a VPS now actually takes effect. `delete_snapshot` previously removed the docker image but never dropped the snapshot from the host record, so `mngr snapshot list` kept showing a deleted snapshot. It now removes the entry from the record (raising `SnapshotNotFoundError` for an unknown id) and refreshes the cache; an already-absent image is treated as success and any other delete failure raises.
- Fixed: `rename_host` now re-stamps the cheap identity tag/metadata offline discovery reads (via the new shared `_remirror_host_name` hook), so a renamed-then-stopped host lists under its new name rather than its old one. AWS/Azure/GCP each override the hook.
- Fixed: Hardened the post-finalize idle-watcher install. A missing host record at `_on_host_finalized` (which runs only after the record has been made durable) now fails `create_host` (raising `HostCreationError`, whose cleanup tears the VPS back down) instead of logging a WARNING and silently shipping a host that can never auto-stop on idle. Genuine install failures (network or systemctl errors) stay best-effort.
- Fixed: `mngr stop` on a bare (`isolation=NONE`) host no longer hangs for minutes while it captures the host's `host_dir`. A bare `host_dir` holds the agent's full working tree, and the offline capture previously wrote one object per file to the state bucket serially. The per-file uploads now run concurrently across a bounded worker pool, turning a minutes-long stop into seconds. Container hosts were unaffected.
