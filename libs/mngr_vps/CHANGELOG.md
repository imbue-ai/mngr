...
- Changed: The shared `OfflineCapableVpsProvider` now owns the cloud stop/start lifecycle (`stop_host` pauses the whole instance, `start_host` resumes it with the on-volume write *and* its external-store mirror in one place); providers supply only `_pause_cloud_instance` / `_resume_cloud_instance` hooks and override `_capture_host_dir_before_pause` / the known_hosts rebind where their behavior differs.
- Changed: The self-stopping idle watcher and the best-effort `_on_host_finalized` step runner moved up into `OfflineCapableVpsProvider`. The host-side systemd unit name changed from per-provider (`mngr-aws-idle-watcher` etc.) to the shared `mngr-idle-watcher`.
- Changed: Centralized systemd unit generation via `render_systemd_unit`, and moved the idle-watcher `ExecStart` body into an installed script (`/usr/local/sbin/mngr-idle-watcher.sh`) instead of a nested-quoted inline `/bin/sh -c` command.
