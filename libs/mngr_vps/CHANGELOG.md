...
- Added: Shared `OfflineCapableVpsProvider` now owns the cloud stop/start lifecycle (`stop_host` / `start_host`), the self-stopping idle watcher install, the host_dir-to-bucket sync daemon, and the best-effort `_on_host_finalized` step runner — providers supply only small per-cloud hooks. Host-side systemd unit names changed from per-provider (`mngr-aws-idle-watcher`, etc.) to the shared `mngr-idle-watcher` / `mngr-host-dir-sync`.
