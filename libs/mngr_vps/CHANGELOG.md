...
- Added: `HostRealizer` seam inside the VPS provider, selected by a new `isolation` config knob (`IsolationMode.CONTAINER` | `NONE`). `CONTAINER` is the default and preserves the original behavior exactly. All Docker-container placement logic (image build/pull, container run, in-container sshd setup, btrfs volume + snapshot helper, container stop/start/teardown, `docker commit` snapshots) moved behind a `DockerRealizer`; a new `BareRealizer` (gated to providers with a machine stop/start lifecycle, raising `BareIsolationNotSupportedError` otherwise) places the agent directly on the VM's OS, reached at `vps_ip:22` as root. Discovery, listing, the placement "is running" predicate, and the host-record store all moved behind the realizer; bare placements have no docker volume.
- Added: Snapshot support is now a structural fact — `SnapshotCapableRealizer` sub-interface that only the container realizer implements; the bare realizer is a plain `HostRealizer`. A snapshot request on a bare placement fails up front with `SnapshotsNotSupportedError`.
- Added: Per-instance `mngr-isolation` marker (an EC2/Azure tag or a GCE metadata item) stamped at create, so a running bare host is discoverable and reachable with the default provider config — `mngr conn`/`list`/`stop`/`start`/`destroy`/`snapshot`/`label`/agent persistence no longer require `-S providers.<cloud>.isolation=NONE`. Pre-existing instances have no marker and default to container.
- Added: Uniform offline host/agent-state store for the cloud providers via a new `HostStateStore` abstraction. AWS/Azure use their object-storage state bucket as the sole offline store (a stopped host's offline metadata now requires the bucket; the provider's `_state_store` raises an actionable `missing_state_bucket_error` pointing at `mngr <cloud> prepare` when the bucket is absent), GCP uses a lossless instance-metadata-backed store. AWS/Azure/GCP now extend `OfflineCapableVpsProvider` directly.
- Added: Offline `host_dir` capability as a select-once `HostDirBackend` strategy — a cloud-agnostic, bucket-backed `BucketHostDirBackend` (capture + offline-read volume) or a no-op `NullHostDirBackend`. Capture is operator-driven (rsync at `mngr stop` with the operator's credentials); no instance/managed identity and no on-box sync daemon are needed.
- Added: Shared `cli_helpers` module (`resolve_provider_config`, `refuse_if_managed_resources_exist` raising the unified `ManagedResourcesExistError`) and shared config bases (`OfflineCapableVpsProviderConfig` carrying `allowed_ssh_cidrs`, `PublicIpVpsProviderConfig` adding `associate_public_ip`), with `allowed_ssh_cidrs` unified to `ScalarStrTuple` across AWS/Azure/GCP.

### Changed

- Changed: Renamed the package from `mngr_vps_docker` to `mngr_vps` (distribution `imbue-mngr-vps-docker` → `imbue-mngr-vps`), since Docker is now one of two placement shapes. Shape-agnostic classes dropped "Docker" from their names: `VpsDockerProvider` → `VpsProvider`, `VpsDockerProviderConfig` → `VpsProviderConfig`, `MinimalVpsDockerProvider` → `MinimalVpsProvider`, `OfflineCapableVpsDockerProvider` → `OfflineCapableVpsProvider`, `VpsDockerHostRecord` → `VpsHostRecord`, `VpsDockerHostStore` → `VpsHostStore`, and the error base `VpsDockerError` → `VpsError`. The genuinely Docker-specific `DockerRealizer` and `container_setup` helpers keep their names.
- Changed: The shared `OfflineCapableVpsProvider` now owns the cloud stop/start lifecycle (`stop_host` pauses the whole instance, `start_host` resumes it with the on-volume write *and* its external-store mirror in one place); providers supply only `_pause_cloud_instance` / `_resume_cloud_instance` hooks and override `_sync_host_dir_before_pause` / the known_hosts rebind where their behavior differs.
- Changed: The self-stopping idle watcher, the host_dir-to-bucket sync daemon, and the best-effort `_on_host_finalized` step runner moved up into `OfflineCapableVpsProvider`. The host-side systemd unit names changed from per-provider (`mngr-aws-idle-watcher` etc.) to the shared `mngr-idle-watcher` / `mngr-host-dir-sync`.
- Changed: Centralized systemd unit generation via `render_systemd_unit`, and moved the idle-watcher / host_dir-sync `ExecStart` bodies into installed scripts (`/usr/local/sbin/mngr-idle-watcher.sh`, `/usr/local/sbin/mngr-host-dir-sync.sh`) instead of nested-quoted inline `/bin/sh -c` commands.
- Changed: Container hosts now register the gVisor (runsc) runtime with `--overlay2=none` so a container's writable layer is written through to the persistent Docker overlay2 layer and survives a `docker stop`/`start` or host reboot. Previously runsc used its default per-sandbox overlay (`--overlay2=root:self`), recreated on every start, so the injected sshd host key, the `/mngr` host_dir symlink, and mngr's provisioning markers were silently lost on restart, leaving the container unreachable until mngr re-provisioned it. Applies to every provider that installs runsc via the shared VPS host-setup (aws, vultr, ovh, gcp, azure, imbue_cloud).
- Changed: The agent container's PID-1 entrypoint now self-heals sshd on every (re)start once mngr has provisioned a host key, so the container is reachable again after a VM reboot or `docker restart` without waiting for `mngr start`.
- Changed: `BaseObjectStoreVolume` in `state_bucket_base.py` now implements the six `Volume` methods plus the `_as_dir_prefix` normalization once, in terms of a small set of per-cloud SDK primitives plus a `_ObjectStoreErrorSeam`; `S3Volume` / `BlobVolume` and `BaseStateBucket` become thin subclasses. The only previously-disagreeing edge (`path_exists("")` on an unscoped raw volume, never reached in practice) is now uniformly `False`.
- Changed: `VpsHostConfig.container_name`/`volume_name` are now nullable so a bare host record (which has no container or Docker volume) is representable. The agent-sshd wait now targets the realizer's endpoint port (the container port for Docker, port 22 for bare) instead of hard-coding the container port.
- Changed: A bare create rejects container-only inputs up front (an image override, a Dockerfile build, or docker run start-args) rather than silently ignoring them.

### Fixed

- Fixed: `mngr destroy` of a stopped (deallocated / powered-off) host no longer leaks the underlying cloud instance and its mirrored state. The base `VpsDockerProvider.destroy` tears the host down over SSH, which works only while reachable; a stopped instance previously either failed with `HostNotFoundError` and leaked the still-billing instance, or — with a stale cached record — ran a doomed SSH teardown against a dead address and still leaked. `OfflineCapableVpsDockerProvider` now dispatches up front on the instance's own power state (resolved from its `mngr-host-id` tag/label, no SSH): a stopped instance goes straight to an offline teardown (`destroy_instance`), cleans up the per-host SSH key, and deletes external state. Fails loudly via `CleanupFailedGroup` if termination cannot be carried out.
- Fixed: On resume, the cloud trio's `start_host` no longer fails on a bare host trying to read the host record through the Docker volume; it resolves the store through the realizer (the fixed root-disk path for bare). Also fixed `start_host` race in `OfflineCapableVpsProvider.start_host` where the any-key SSH wait could return inside a sshd-restart window; it now also waits for the VM to serve mngr's expected host key (GCP, with cloud-init backends as a no-op).
- Fixed: `mngr stop` on a bare host no longer hangs for minutes during offline `host_dir` capture. A bare `host_dir` holds the agent's full working tree (a git checkout is thousands of small files) and the bucket upload previously ran serially; uploads now run concurrently across a bounded worker pool, turning a minutes-long stop into seconds. Container hosts unaffected.
- Fixed: `mngr rename` now re-stamps the cheap `mngr-host-name` / `Name` instance identity tag via a shared `_remirror_host_name` hook, so a renamed-then-stopped host lists under its new name (the tag was previously stamped only at create). Offline-capable cloud providers (AWS/Azure/GCP) override the hook to update through their cloud API.
- Fixed: `mngr snapshot delete` on a VPS now actually takes effect. `delete_snapshot` previously removed the docker image but never dropped the snapshot from the host record, so `mngr snapshot list` kept showing a deleted snapshot (and an unknown id succeeded silently). It now removes the entry from the record (raising `SnapshotNotFoundError` for an unknown id) and refreshes the cache. A failed image removal now raises rather than being silently reported as success.
- Fixed: Host lock reporting for VPS/docker/bare hosts is now derived from a real `flock` held-probe rather than the lock file's presence.
- Fixed: `_on_host_finalized` now fails `create_host` (raising `HostCreationError`, whose cleanup tears the VPS back down) when the host record is missing — that runs only after the record has been made durable, so a missing record there is a broken invariant. Genuine idle-watcher install failures (network/systemctl errors) stay best-effort and tolerated.
