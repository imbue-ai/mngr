...
# ``DirectModalInterface`` is imported lazily inside
# ``ModalProviderBackend.build_provider_instance`` because its module pulls the
# ``modal`` SDK at import time (~90ms). This module is loaded eagerly at CLI
# startup as a plugin entry-point; the heavy import only matters when a Modal
...
"""
    ...
    def build_provider_instance(
        ...
        # Create the ModalInterface based on the configured mode. The import is
