# Unabridged Changelog - mngr_robinhood_claude

Full, unedited changelog entries consolidated nightly from individual files in `libs/mngr_robinhood_claude/changelog/`.

For a concise summary, see [CHANGELOG.md](CHANGELOG.md).

## 2026-06-02

`RobinhoodClaudeError` (the plugin's base error) now inherits from `MngrError` instead of
...
### Add the missing changelog/ directory to mngr_robinhood_claude

The recently added `mngr_robinhood_claude` project shipped with
...
`CHANGELOG.md`. No behavior of the `mngr robinhood-claude` command
changes.
...
Add `mngr robinhood-claude`, a new top-level command provided by the `imbue-mngr-robinhood-claude` plugin. It acts as a drop-in replacement for `claude -p`: every claude flag is forwarded verbatim to a fresh, ephemeral mngr claude agent that runs in-place in the current directory. The prompt is read from positional argv (or stdin), the agent runs to end-of-turn, the response is harvested from the agent's common transcript, and the agent is destroyed on exit.
