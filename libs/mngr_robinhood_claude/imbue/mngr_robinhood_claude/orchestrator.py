...
from imbue.mngr_robinhood_claude.output_modes import monotonic_ms_since
from imbue.mngr_robinhood_claude.raw_transcript import RAW_TRANSCRIPT_PATH
...
# which is computed as ``not host.is_local``; robinhood-claude always runs
...
# ``mngr robinhood-claude`` in ``timeout(1)`` per the spec.
...
def _normalize_credentials_env() -> None:
    ...
    When ``mngr robinhood-claude`` runs from inside another mngr claude agent,
...
def run(
    ...
    """Drive a single ``mngr robinhood-claude`` invocation end-to-end.
...
def _run_with_agent(
    ...
    options = CreateAgentOptions(
        ...
        label_options=AgentLabelOptions(labels={"created-by": "robinhood-claude"}),
