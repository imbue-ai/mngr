...
class StubHost:
    ...
    def __init__(
        self,
        host_dir: Path = Path("/tmp/mngr-test/host"),
...
def create_fake_mngr_binary(bin_dir: Path) -> Path:
    """Create a fake mngr binary at <bin_dir>/mngr."""
...
class FakeWaitProcess:
    """Controllable fake for the 'mngr wait' subprocess used in idle detection.
...
@pytest.fixture()
def fake_mngr_binary(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Create a fake mngr binary and set UV_TOOL_BIN_DIR to point to it."""
    bin_dir = tmp_path / "fake_bin"
    create_fake_mngr_binary(bin_dir)
    monkeypatch.setenv("UV_TOOL_BIN_DIR", str(bin_dir))
    return bin_dir


@pytest.fixture()
def mock_subprocess_success(monkeypatch: pytest.MonkeyPatch, fake_mngr_binary: Path) -> EventWatcherSubprocessCapture:
...
@pytest.fixture()
def mock_subprocess_failure(monkeypatch: pytest.MonkeyPatch, fake_mngr_binary: Path) -> EventWatcherSubprocessCapture:
