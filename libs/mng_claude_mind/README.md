...
This plugin implements the core functionality for Claude-based "minds": composite agents built from multiple mng agents that, together, form a single higher-level agent from the user's perspective.

## What is a mind?

A mind is a *collection of persistent mng agents* that serve a web interface and support chat input/output. Each mng agent within the mind fulfills a specific **role** (e.g., thinking, working, verifying), and they coordinate through shared event streams in a common git repo.
...
- **Role agent**: A standard mng agent (a process in a tmux session, visible in `mng list`, with its own lifecycle state). Each role agent is created via `mng create` and fulfills a specific role within the mind.
- **Mind**: The collection of role agents that together form a single higher-level agent from the user's perspective. Not itself an mng agent -- it's the emergent whole.
...
Each role directory in the mind repo corresponds to an mng agent type:

- `thinking/` - the primary role agent; the "inner monologue" that reacts to events and coordinates the mind
...
The **thinking** role agent is the first role agent created when a mind is deployed. It is the coordinator: it decides when to create other role agents based on the events it receives.
...
Per-deployment settings are read from `minds.toml` in the agent work directory (`$MNG_AGENT_WORK_DIR/minds.toml`). This file is optional -- all settings have built-in defaults. See `ClaudeMindSettings` in `data_types.py` for the full schema.
...
Conversation metadata (tags, created_at) is stored in the `mind_conversations` table in the llm sqlite database (`$LLM_USER_PATH/logs.db`). The model for each conversation lives in the llm tool's native `conversations` table.

## Provisioning

The `ClaudeMindAgent.provision()` method transforms the mind repo into a running role agent:
