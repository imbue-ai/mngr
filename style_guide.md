...
Release tests are comprehensive tests that verify the application is ready for release and may include slower, more thorough tests that would be too time-consuming to run on every PR. They do not run per-PR; instead CI runs them on `v*` release-tag pushes, and the TMR workflow also exercises them on a daily schedule (and on manual dispatch).
