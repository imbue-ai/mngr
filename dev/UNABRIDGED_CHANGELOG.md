...
Root-level surface changes for the `mngr_robinhood_claude` plugin: README updated to advertise the new `robinhood-claude` command and link to the new sub-project, and the auto-generated CLI docs gained an entry at `libs/mngr/docs/commands/secondary/robinhood-claude.md` so `mngr ask` and `mngr --help` know about the command.
