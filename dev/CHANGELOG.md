...
- Added: Daily 14:00 UTC schedule on the `minds launch-to-first-message` workflow that builds + verifies the current mngr `main` HEAD against FCT `main` with the full slack flow (latchkey + mocked slack server), surfacing drift the morning it happens. `commit_sha` and `template_ref` inputs become optional with sensible empty-value fallbacks (`github.sha` and `main` respectively); the cron only fires once the workflow file lands on `main`.
...
- Changed: `minds-playwright-vanilla.yml` renamed to `minds-macos-launch.yml`; trigger moved to `branches: [main]` + open `pull_request` so the workflow keeps running post-merge, with html reporter + always-upload + `run_attempt`-suffixed artifact so passing reruns no longer erase failing-attempt screenshots.
...
- Changed: Removed the contradictory "commit when finished" parenthetical from the `identify-style-issues`, `identify-doc-code-disagreements`, `identify-outdated-docstrings`, `identify-inconsistencies`, and `identify-suspicious-edge-cases` skill intros (their output files are gitignored and the closing line says no commit is needed).
