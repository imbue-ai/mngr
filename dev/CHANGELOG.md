...
- Added: Registered the new `overlay` workspace library in the root `pyproject.toml` (`[tool.uv.sources]` workspace source, `--cov=imbue.overlay` in the shared coverage flags). Updated root pytest coverage config to track the renamed `imbue.mngr_vps` package (was `imbue.mngr_vps_docker`). `google-cloud-storage>=2.18` (declared in `libs/mngr_gcp/pyproject.toml` for the GCP provider's offline `host_dir` GCS state bucket) propagates to the root `uv.lock`.
