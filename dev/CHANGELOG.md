...
- Added: New blueprint plans under `blueprint/` — `mngr-agent-sdk/plan-mngr-agent-sdk.md` (the mngr-backed Claude Agent SDK at `imbue.mngr_robinhood.agent_sdk`), `finish-agent-sdk/plan-finish-agent-sdk.md` (completing the remaining control surfaces of the Agent SDK), and `tmux-window-size/` (the configurable per-agent tmux window-size feature for mngr/mngr_robinhood). The implementations themselves live under `libs/mngr_robinhood` and `libs/mngr`.
- Added: `just test-sdk-live` recipe that sets `RUN_SDK_LIVE_TESTS=1` and runs the `sdk_live`-marked tests in `libs/mngr_robinhood` (the opt-in, paid live integration test suite against the real Anthropic API).
...
- Changed: Excluded the opt-in live Claude Agent SDK test suite from CI by adding `and not sdk_live` to both pytest filter expressions in `offload-modal.toml`.
