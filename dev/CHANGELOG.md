...
- Added: `.github/workflows/minds-launch-to-msg.yml`, a `workflow_dispatch` job that (given a minds commit SHA and forever-claude-template ref) either reuses an existing ToDesktop build matching the commit or runs `pnpm dist` to build a fresh draft, then on the self-hosted `minds-runner` macOS host downloads the resulting `.app`, launches it, waits for the backend to come up, and optionally round-trips a real first-message chat against a LIMA agent before cleaning up; collects diagnostic artifacts on failure. Companion `.github/workflows/minds-runner-reset.yml` `workflow_dispatch` job manually resets the self-hosted runner to a clean state (and optionally installs a fresh `.app` from a ToDesktop `.zip` URL). The runner mac is registered at the `imbue-ai` org level and targeted by `runs-on: [self-hosted, macOS, minds-runner]`.
- Added: Design spec under `specs/docker-cleanup-state-and-images/` documenting the Docker build-image and state-container cleanup work.
- Added: Implementation spec for Imbue Cloud R2 bucket support (`specs/imbue-cloud-r2-buckets/spec.md`).
- Added: `specs/minds-backup-provider/concise.md` — spec for wiring the imbue_cloud bucket capability into the minds workspace-creation flow (backup provider toggle, async post-creation restic config injection, FCT `host_backup` contract changes).
- Added: `specs/host-backup/concise.md` — spec for a new continuous-backup service that runs inside every mind workspace; uses restic against a Cloudflare R2 bucket by default and takes consistent btrfs subvolume snapshots on lima / vps-docker (no-op on plain docker). This monorepo's changes provision the outer-side `snapshot_helper.sh` systemd unit on vps-docker hosts.
- Added: `specs/symlink-code-onto-mngr-volume/concise.md` describing the relocation of the forever-claude-template workspace from `/code/` onto the `/mngr/` persistent volume (as `/mngr/code/`), with safety-net `/code -> /mngr/code` and `/worktree -> /mngr/worktree` symlinks. Covers the Dockerfile bake-and-relocate dance and the per-template scope.
- Added: `specs/vps-docker-btrfs/concise.md` documenting the design for putting the per-host VPS docker unified volume onto a loop-mounted btrfs subvolume.
- Added: `specs/vps-docker-unified-volume/concise.md` documenting the docker_vps provider's move from a two-volume layout (per-user state container + per-host data volume) to a single unified per-host Docker volume on the VPS.
...
- Changed: Dropped the now-removed `--use-snapshot` flag from the TMR GHA workflow (`.github/workflows/tmr.yml`) so scheduled/manual TMR runs don't fail at invocation; snapshot building on `--provider modal` is automatic now. Refreshed a stale comment in `.github/workflows/tmr-reintegrate.yml` that mentioned the same removed flag.
- Changed: `.minds/template/cloudflare.sh` secret template now documents that `CLOUDFLARE_API_TOKEN` must be an account-owned (`cfat_`) token carrying `Workers R2 Storage: Edit` + `Account API Tokens: Edit` (on top of existing tunnel/DNS/Access/KV permissions), and that R2 must be enabled on the Cloudflare account.
- Changed: `specs/electron-desktop-app/concise.md` and `specs/electron-desktop-app/spec.md` file-tree listings for `apps/minds/` now show `todesktop.js` instead of `todesktop.json`, keeping the documented layout in sync with the rename in `apps/minds`.
