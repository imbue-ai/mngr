...
- Sped up the minds snapshot build: `playwright install` (chromium + apt deps, ~24s, source-independent) now runs in a cached pre-repo image layer instead of re-running on every commit. Verified the pre-code image layers (apt/node/uv/claude/depot CLI) are already cached; only the post-`COPY` deps layer rebuilds per commit. (A producer-sandbox CPU bump was tried and reverted: the in-sandbox FCT build is network/IO-bound, not CPU-bound, so more cores did not help.)
