...
- Ran `uv lock --upgrade` to bump all floating dependencies to their latest constraint-satisfying versions (93 packages), including major bumps such as `starlette` 0.50 -> 1.1, `urwid` 3.0 -> 4.0, `pydantic` 2.12 -> 2.13, `cryptography` 46 -> 48, `typer` 0.21 -> 0.25, and `uvicorn` 0.40 -> 0.48.
  - `wsgidav` 4.3.3 -> 4.3.4 adds a `passlib` dependency and a `bcrypt<5` cap, so `bcrypt` moves 5.0 -> 4.3 (paramiko itself is fine with bcrypt 5; the cap is wsgidav's). Accepted as a benign downgrade.
