...
- Every aggregate field flips to assign-by-default in `MngrConfig.merge_with`, `AgentTypeConfig.merge_with`, and the related provider/plugin/template merges. The named concat-tuple fields on `AgentTypeConfig` (`cli_args`, `extra_provision_command`, `upload_file`, `create_directory`, `env`, `env_file`) lose their auto-concat behavior; parent-type-inherited args must be re-added via `__extend` if desired. `unset_vars`, `permissions`, and `enabled_backends` similarly flip to plain assign. `disabled_plugins: frozenset[str]` flips to assign.
