...
| 7 | AWS/GCP `ProviderUnavailableError` falls through to default "start Docker" help text (Azure curated only) | medium-high | Credentials |
| 8 | SSH `supports_shutdown_hosts=True` but `stop_host` raises `NotImplementedError` — user sees stack trace | medium | Capability |
| 9 | `supports_volumes=True` on VPS family but `list_volumes()` returns `[]` | medium | Capability |
...
| 12 | Vultr `build_provider_instance` silently swallows missing-creds `ValueError`; OVH never raises `ProviderUnavailableError`; Modal raises wrong class (`PluginMngrError`) | high | Errors |
| 13 | OVH `mngr destroy` cancels at billing-cycle end, not immediately — VPS keeps running until expiration | medium | Lifecycle |
| 14 | Docker provider `-p :22` binds `0.0.0.0:<random>:22` on host's LAN | medium (security) | Networking |
| 15 | `auto_shutdown_seconds` wired through to cloud API is not pinned by any test | medium | Tests |
| 16 | GCP lowercases mngr-provider label — mixed-case provider names silently collide | medium | Discovery |
| 17 | Container-shape knobs (`--cpu`/`--memory`/`--gpu`) are Modal-only; no cross-provider "~2 vCPU/4GB" alias | medium | Create UX |
| 18 | Modal has no `mngr modal cleanup` analog (cloud trio + OVH-list all do) | low-medium | Destroy |
| 19 | Modal underscore tag keys (`mngr_host_id`) vs dash elsewhere (`mngr-host-id`) | low | Tagging |
...
- **`--stop-host` compute leak on AWS/GCP/Azure** — now a real machine-level stop/deallocate via cloud-subclass `stop_host` overrides (`mngr_aws/backend.py:505`, `mngr_gcp/backend.py:383`, `mngr_azure/backend.py:585` → `deallocate_instance`). Still open only for Vultr/OVH (finding #1).
- **Azure `auto_shutdown_seconds` "Stopped (not deallocated)" billing leak** — the idle path now runs an ARM self-deallocate via the VM's managed identity + IMDS token (`mngr_azure/backend.py:_build_self_deallocate_script`), so compute billing actually stops.
...
| `NONE` (bare) | `BareRealizer` (`bare_realizer.py`) | Directly on the VM OS at `vps_ip:22`, no Docker, systemd-owned; host store on root disk at `/var/lib/mngr-host` | **`False`** (`snapshot_placement` raises `SnapshotsNotSupportedError`) | `idle_shutdown_command = "shutdown -P now"`, `idle_shutdown_stops_host = True` — the bare VM powers itself off directly (no sentinel indirection) |
...
**Bare gating.** `VpsProvider.create_host` raises `BareIsolationNotSupportedError` (`errors.py:20`) when `isolation is NONE` and `not self._supports_bare_isolation` (`instance.py:682`). `_supports_bare_isolation` defaults `False` (`instance.py:427`) and is overridden `True` only by AWS (`mngr_aws/backend.py`), GCP (`mngr_gcp/backend.py:528`), and Azure (`mngr_azure/backend.py`). Vultr and OVH inherit the default and reject `isolation=NONE` — bare needs a machine stop/start lifecycle (idle agent powers the VM off, `mngr start` boots it again), which those providers don't have. On Azure, bare runs the same managed-identity ARM deallocate as the container path.
...
| aws | Base path; AWS contributes `_create_vps_instance` (RunInstances + SG + EBS DeleteOnTermination + IMDSv2 + spot + AMI + IAM self-stop profile), sentinel-file shutdown script, `_on_host_finalized` (systemd `.path`/`.service` self-stop units), pytest gate, `_supports_bare_isolation=True`. | base `mngr_vps/instance.py:664`; `mngr_aws/backend.py` |
| azure | Base path; Azure contributes `_create_vps_instance` (orphan NIC/IP reclaim sweep, vnet/subnet resolution, VM with cascade-delete on NIC/IP/OS-disk, cloud-init via base64 `custom_data`, spot+eviction-Delete), pytest gate, managed-identity self-deallocate idle watcher, `_supports_bare_isolation=True`. | `mngr_azure/client.py`; `backend.py` |
...
| aws | Realizer `stop_placement` (docker-stop / bare no-op), then EC2 `StopInstances` with EBS preserved; base writes record + mirrors externally. Idempotent. | Compute billing stops; EBS storage continues. | `mngr_aws/backend.py:505`; base `mngr_vps/instance.py:1222` |
| azure | Realizer `stop_placement`, then `begin_deallocate` (true deallocate — billing stops, not OS-stop). | Compute billing stops; OS disk continues. | `mngr_azure/backend.py:585` → `deallocate_instance` |
| gcp | Realizer `stop_placement`, then GCE `stop_instance` (instance → `TERMINATED`). | Compute billing stops; boot disk continues. | `mngr_gcp/backend.py:383` |
...
Base `start_host` now relaunches the activity watcher on resume (`libs/mngr_vps/imbue/mngr_vps/instance.py:1274`) — fixes the idle-host re-stop bug Vultr/OVH used to hit. Like `stop_host`, machine-level start is layered on by the AWS/GCP/Azure subclass overrides; Vultr/OVH inherit container-only.
...
| aws | Base teardown (`DockerRealizer.teardown_placement` for container) → `TerminateInstances` → EBS auto-deleted; EC2 SSH key removed. | None AWS-specific. | base `mngr_vps/instance.py:1325`; `docker_realizer.py:361` |
...
`supports_volumes` is still `True`-but-`[]` on the whole VPS family: `list_volumes` returns `[]` (`mngr_vps/instance.py:2172`) and `delete_volume` is a no-op, inherited unchanged by AWS/GCP/Azure/Vultr/OVH. The state bucket added a `get_volume_for_host` (S3Volume/BlobVolume) on AWS/Azure, but `list_volumes`/`delete_volume` remain unimplemented and the flag was not flipped.
...
| STOPPED (after `--stop-host`) | shown (record on Volume) | shown (S3 state bucket; tag fallback) | shown (Blob state bucket; tag fallback) | shown (GCE metadata mirror) | N/A — no machine stop | N/A — no machine stop | shown | shown | N/A |
| CRASHED / unreachable | shown | shown (bucket / tag fallback) | shown (bucket / tag fallback) | shown (metadata) | shown if cached | shown if cached | shown | shown | **always "RUNNING"** (hard-coded) |
...
Stopped-host (offline) reconstruction is lifted into the base class hierarchy: `OfflineCapableVpsProvider` (`mngr_vps/instance.py:2240`) with abstract hooks (`_offline_discovered_host_from_instance`, `_is_instance_offline`, `_persisted_agent_dicts_from_instance`, `_offline_host_from_instance`), and the tag-based layer `TagMirrorVpsProvider` (`:2522`). The offline mirror is no longer Modal+AWS only:
- AWS, Azure: shared state bucket (`S3StateBucket`/`BlobStateBucket` + `BucketHostStateStore`, `libs/mngr_vps/host_state_store.py`) holds full host *and* agent records — no 256-char tag cap. With a bucket configured, per-agent `mngr-agent-<id>-*` tags are bypassed; the tag mirror (with its 256-char `labels` drop, `_MAX_TAG_VALUE_LEN=256`) survives only as the no-bucket fallback.
- GCP: deliberately no bucket — uses a GCE metadata mirror (extends `OfflineCapableVpsProvider` directly).
...
Modal auto-snapshots on every agent create (`is_snapshotted_after_create=True`, `on_agent_created` hook in `libs/mngr_modal/imbue/mngr_modal/instance.py`). On the VPS family `supports_snapshots` is now **realizer-derived** (`VpsProvider.supports_snapshots = self._realizer.supports_snapshots`, `mngr_vps/instance.py:411`): container placements (`DockerRealizer`) report `True` and snapshot via `docker commit` of the container layer on the host's own disk — survives `mngr stop` but **not** `mngr destroy`; bare placements (`BareRealizer`) report `False` and `snapshot_placement` raises `SnapshotsNotSupportedError`. The cloud-trio `VpsClientInterface` disk-snapshot methods were deleted entirely; user-facing `mngr snapshot create` everywhere except Modal is `docker commit` (container) or unsupported (bare).
...
Roughly half of in-scope user-visible behaviors have a provider-specific test pin. New bare-isolation release tests landed with the merge: AWS has the deepest coverage (`test_release_aws.py::test_bare_provider_lifecycle_create_exec_and_destroy`, `…stop_host_stops_ec2_instance_and_start_resumes`, `…idle_watcher_auto_stops_then_resumes`); GCP/Azure have create/exec/destroy lifecycle only (`test_release_gcp.py`, `test_release_azure.py`). Plus unit tests (`mngr_vps/bare_realizer_test.py`) and S3 volume round-trips (`mngr_aws/s3_volume_test.py`). Top remaining holes:
...
15. **Adopt OVH's `mngr <provider> list` operator-CLI pattern** uniformly: ship `mngr aws list`, `mngr azure list`, `mngr gcp list`, `mngr vultr list`. (The offline-reconstruction triad has already been lifted into `OfflineCapableVpsProvider`/`TagMirrorVpsProvider`; Vultr/OVH would inherit stopped-host visibility automatically once they gain VM-level stop.)

### Multi-agent (shape §1.8)

16. **Document the AWS ~16-agent tag cap** in the AWS README (now only the no-bucket fallback — with a bucket configured, full agent records live in the state bucket) and surface it in `mngr exec --new-agent` before the cap is hit.
