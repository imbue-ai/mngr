# mngr_uncapped_claude

## Refined prompt

we're going to make a new mngr plugin, mngr_uncapped_claude, that exposes a single command, "mngr uncapped-claude" that acts *exactly* like "claude -p", except it will use mngr create, mngr message, and mngr transcript to implement it.
...
* Environment forwarding: the *full* current env is forwarded to the agent via `mngr create --pass-env` for every key in `os.environ`. Simple and matches what `claude -p` would see if invoked directly.
...
- Adds a new top-level mngr command `mngr uncapped-claude` that behaves as a drop-in replacement for `claude -p`.
...
- `mngr uncapped-claude "summarize this repo"` runs in the current directory, prints claude's text response to stdout, exits 0.
- `cat error.log | mngr uncapped-claude "explain this"` and `cat error.log | mngr uncapped-claude` both work: stdin is read when no positional prompt is given.
- `mngr uncapped-claude` with neither argv prompt nor piped stdin → exit 2 with `error: no prompt provided`.
- `mngr uncapped-claude "..." --output-format json` prints a single JSON object matching `claude -p --output-format json`'s shape (synthesized `result` envelope with `result`, `session_id`, `duration_ms`, `is_error`, plus zeroed/null cost/usage fields).
- `mngr uncapped-claude "..." --output-format stream-json --verbose` streams NDJSON events as the agent works; first line is a `system/init` envelope, subsequent lines are reformatted transcript events, final line is the `result` envelope.
- `mngr uncapped-claude --input-format=stream-json --output-format=stream-json` reads NDJSON `user` lines from stdin, sends each one as a `mngr message`, waits for end-of-turn, emits NDJSON output events between turns, and exits 0 on stdin EOF (after destroying the agent).
- All claude-side flags (`--model`, `--allowedTools`, `--system-prompt`, `--bare`, etc.) reach the spawned claude unchanged.
- `mngr uncapped-claude -c "..."` (or `--resume`, `--session-id`) exits 2 immediately with `error: --continue / --resume / --session-id are not supported by mngr uncapped-claude in v1`.
- `--include-partial-messages`, `--max-budget-usd`, `--include-hook-events`, `--fallback-model`, `--no-session-persistence` each error the same way (`error: --X is not supported by mngr uncapped-claude in v1`).
- A stream-json input line that is anything other than `{"type":"user","message":{"role":"user","content":"<string>"}}` exits 2 with `error: only simple text user messages are supported by mngr uncapped-claude in v1` and includes the offending line in the message.
- Ctrl-C destroys the spawned agent before re-raising SIGINT; no orphan `uncapped-*` agents remain on the local host.
- The spawned agent name is `uncapped-<coolname>` (e.g. `uncapped-graceful-unicorn`) with a `created-by=uncapped-claude` label; visible briefly in `mngr list` while the run is in progress.
- Multiple `mngr uncapped-claude` invocations in the same cwd run concurrently without locking; each has its own unique agent.
...
- `mngr --help` lists `uncapped-claude`; `mngr ask "how do I run claude unattended"` surfaces it.
...
- `libs/mngr_uncapped_claude/`
  ...
  - `imbue/mngr_uncapped_claude/`
    ...
    - `cli.py` — the `uncapped-claude` click command + top-level `_run(...)` orchestrator.
...
- `plugin.py`
  - `@hookimpl def register_cli_commands() -> Sequence[click.Command]` → `[uncapped_claude]`.

- `cli.py`
  - `class UncappedClaudeCliOptions(CommonCliOptions)` — captures top-level argv before partitioning.
  - `@click.command(name="uncapped-claude", context_settings={"ignore_unknown_options": True, "allow_extra_args": True})` plus an `@click.argument("argv", nargs=-1, type=click.UNPROCESSED)` so click does no opinionated parsing of claude's own flags.
  - `@click.pass_context def uncapped_claude(ctx, **kwargs)` — sets up the mngr context via `setup_command_context`, parses argv via `partition_args(...)`, dispatches to `orchestrator.run(...)`, exits with the resulting code.
  - `CommandHelpMetadata(key="uncapped-claude", one_line_description="Drop-in mngr-backed replacement for `claude -p`", ...).register()` so `mngr ask` and `mngr --help` know about it.
...
- `orchestrator.py`
  - `class UncappedRun(MutableModel)` — holds the lifetime state (`agent_id`, `agent_name`, `host`, `created_at`).
...
- Add `libs/mngr_uncapped_claude/README.md` with: one-paragraph description, install snippet, three short usage examples (text, json, stream-json), and a list of v1 unsupported flags.
- Update the top-level `README.md` "Sub-projects" list to include `libs/mngr_uncapped_claude/`.
- Add changelog entry at `changelog/mngr-uncapped-claude.md` (stub already exists; expand at PR time).

## Implementation Phases

1. **Skeleton & arg partitioning.** Create the package layout. Implement `arg_partition.py` + tests (rejects bad flags, partitions correctly across all forms `--flag=value`, `--flag value`, short forms). Wire up `plugin.py` and a no-op `cli.py` that just prints the partition. Verify `mngr uncapped-claude --help` works. No agent code yet.
...
- `arg_partition_test.py`:
  - partitions `mngr uncapped-claude -p --output-format=json "hello"` correctly across forms.
...
### Integration tests (no marker, mock-agent based — `test_uncapped_claude.py`)
...
### Release tests (`@pytest.mark.release`, `test_uncapped_claude_release.py`)
...
- `mngr uncapped-claude "say hi"` produces a short text reply, exits 0.
- `echo "say hi" | mngr uncapped-claude` same.
- `mngr uncapped-claude "say hi" --output-format=json | jq .result` returns the reply text.
- Multi-turn via stream-json: `printf '%s\n%s\n' '{"type":"user","message":{"role":"user","content":"hi"}}' '{"type":"user","message":{"role":"user","content":"again"}}' | mngr uncapped-claude --input-format=stream-json --output-format=stream-json` emits two assistant turns.
- Ctrl-C during a run leaves no `uncapped-*` agent in `mngr list`.
