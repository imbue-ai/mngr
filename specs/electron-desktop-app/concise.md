...
* `electron/pyproject/pyproject.toml` -- Standalone pyproject.toml that declares `minds` (and transitively `mngr`) plus the `imbue-mngr-claude` / `imbue-mngr-modal` plugins as dependencies, with a lockfile (`electron/pyproject/uv.lock`) pinning exact versions. This is what `uv sync` operates on inside the app -- it is separate from the monorepo's pyproject.toml.
