...
**MUST.** Show every host that has ever been created via this provider — RUNNING, STOPPED, CRASHED, and (with `--include-destroyed`) DESTROYED. The set of states MUST be exactly the `HostState` enum; the provider does not get to invent new ones. AWS's `discover_hosts_and_agents` override at `libs/mngr_aws/imbue/mngr_aws/backend.py:655-794` shows what "stopped hosts visible" means in practice: it reconstructs the host from EC2 tags when the instance is stopped and the SSH probe fails. Providers that grow VM-level stop later MUST extend their discovery the same way; inheriting the base's "drop anything without a current public IP" behavior is a regression waiting to happen.
...
**SHOULD.** Use a single cached read of the cloud listing endpoint per command. AWS/GCP/Azure use `_list_instances_cached` (`libs/mngr_vps_docker/imbue/mngr_vps_docker/instance.py:433-442`).
...
**Right (real stop).** AWS overrides `stop_host` at `libs/mngr_aws/imbue/mngr_aws/backend.py:335-365` to call `ec2:StopInstances`, preserves the EBS volume, records `stop_reason=STOPPED`, and surfaces the stopped instance in `mngr list` via tag-based reconstruction.

**SHOULD.** When `--stop-host` would stop *only* the container and leave the VM billing, the provider MUST NOT inherit the base's `stop_host`. Either override it to also stop the VM, or override `supports_shutdown_hosts` to `False` and document the gap in the README.
...
**SHOULD.** Re-bind the known-hosts file if the public IP changed during stop+start (`_rebind_known_hosts` in AWS at `backend.py:419` is the pattern). Providers whose stop preserves the IP (Azure if it ever implements stop; Vultr if VPS retains IP) can skip this.
...
**MUST (live multi-agent).** A provider that returns a running host from `mngr create` MUST allow a second agent to be added to that host via `mngr exec <host> --new-agent` (or equivalent). For VPS-backed providers this means the live discovery scan MUST find in-container agents that were not present at create time — `mngr_vps_docker/instance.py:1506-1565` (introduced in commit `8cca7406d`) is the model: it runs a listing script *inside* the container instead of relying on the outer persisted store, so agents created in-container (e.g. by minds' chat flow) are visible immediately.

**MUST (persisted multi-agent).** `persist_agent_data` MUST be keyed per-agent. Modal: `/hosts/{host_id}/{agent_id}.json` on the Modal state volume (`libs/mngr_modal/imbue/mngr_modal/instance.py:785-804`). VPS family: `host_state/{host_id}/{agent_id}.json` on the on-VPS volume via `DockerHostStore.persist_agent_data` (`libs/mngr/imbue/mngr/providers/docker/host_store.py:160-170`). AWS additionally mirrors up to ~16 agents into per-agent EC2 tags `mngr-agent-<id>-name` / `-type` / `-labels` (`libs/mngr_aws/imbue/mngr_aws/backend.py:660-687`, helpers at `:832-871` — commit `446d6a964`). A provider whose `persist_agent_data` packs multiple agents into a single blob or overwrites is broken.
...
**SHOULD (offline mirror).** When the host's compute is stopped or unreachable, the provider SHOULD continue to report all N agents in `mngr list` via an offline mirror in provider-side metadata. AWS does this via per-agent EC2 tags reassembled at `libs/mngr_aws/imbue/mngr_aws/backend.py:709-724`. **Azure, GCP, Vultr, OVH inherit the base path that raises `HostNotFoundError` for `list_persisted_agent_data_for_host` when the VPS IP is unreachable** (`libs/mngr_vps_docker/imbue/mngr_vps_docker/instance.py:2402-2405`). When those providers grow VM-level stop (which they MUST per §1.4), they MUST also grow an analogous offline mirror — Azure tags, GCP labels, OVH metadata, or the planned shared S3-backed store referenced in `libs/mngr_aws/README.md`. The data is still safe on the per-host volume; it's just invisible in `mngr list` until the VM is up again.
...
**Out of scope.** "N hosts share one VM" (multi-container packing onto a single VM, e.g. the AWS README's `[future]` item) is a separate roadmap question that is **not** covered by §1.8. Every VPS provider documents in `VpsDockerProvider`'s docstring that each host maps to exactly one VPS running exactly one container (`libs/mngr_vps_docker/imbue/mngr_vps_docker/instance.py:439-441`).
...
Every cloud provider MUST pass curated `user_help_text` on `ProviderUnavailableError`. The default text (`libs/mngr/imbue/mngr/errors.py:224-227`) tells the user to "start Docker" — wrong advice for an AWS auth failure. Pattern: `_azure_unavailable_error` (`libs/mngr_azure/imbue/mngr_azure/backend.py:36-53`) returns provider-specific guidance ("run `az login`"). AWS and GCP currently fall through to the default; this MUST be fixed.
...
**Anti-pattern.** AWS/Azure/GCP today inherit `supports_snapshots=True` from `VpsDockerProvider` (`libs/mngr_vps_docker/imbue/mngr_vps_docker/instance.py:399-400`) but `create_snapshot` is a `docker commit` of the container layer stored on the VPS's own disk. It survives `mngr stop` but not `mngr destroy` and is not portable across hosts. This is a different product from a Modal snapshot. Open question (§11): should this be split into `supports_persistent_snapshots`?

### 2.2 `supports_shutdown_hosts`

`True` means `stop_host` actually stops compute billing. The base `VpsDockerProvider.stop_host` only stops the container — providers MUST override this to stop the VM itself if they want to honestly claim `True`. See AWS at `libs/mngr_aws/imbue/mngr_aws/backend.py:335-365` for the override pattern.

**Anti-pattern.** SSH provider at `libs/mngr/imbue/mngr/providers/ssh/instance.py:104-106` returns `True` but `stop_host` at `instance.py:184-190` raises `NotImplementedError`. Any CLI branch that consults the flag will fail. Either flip the flag to `False`, or implement `stop_host` as a no-op (the BYO-host user expects to manage compute themselves).

### 2.3 `supports_volumes`

`True` means `list_volumes`, `delete_volume`, and `get_volume_for_host` all work and return real volumes. Local providers (Docker, SSH) where the user's own filesystem IS the volume return `False`. Modal returns `True` (volumes are first-class Modal objects). VPS-Docker returns `True` (the host_dir is mounted from a directory on the VPS).

### 2.4 `supports_mutable_tags`

`True` means `add_tags_to_host` / `remove_tags_from_host` / `set_host_tags` mutate the underlying tags after host creation. Docker (immutable container labels) returns `False`; AWS (EC2 tags are mutable) returns inherited `False` from VPS-Docker base; Lima (mutable on-disk file) returns `True`.
...
**SHOULD.** Map the container SSH port the same way as the VM SSH port — i.e. the cloud firewall is the security perimeter, NOT the container's own port mapping. The container's sshd binds to localhost on the VPS (`container_ssh_port: 2222`, `libs/mngr_vps_docker/imbue/mngr_vps_docker/config.py:46-49`) and is exposed via the same cloud-firewall ingress as VM port 22. Never expose the container directly on `0.0.0.0` on a public IP without going through the cloud firewall — the cloud firewall is the user's control point.
...
**SHOULD.** `default_idle_timeout = 800` seconds (the value at `libs/mngr_vps_docker/imbue/mngr_vps_docker/config.py:22-25`) is a reasonable shared default. Providers MAY override; the user can override per agent.
...
- `supports_shutdown_hosts=True`: idle stops compute. AWS's sentinel-file + systemd `.path` unit pattern is the model (the idle watcher on the inner host writes a sentinel; an outer-host systemd path-unit fires `aws ec2 stop-instances`). GCP and Azure don't have this today and they SHOULD.

### 3.3 Auto-shutdown defaults — hard max-lifetime cap

`auto_shutdown_seconds` (`libs/mngr_vps_docker/imbue/mngr_vps_docker/config.py:58-68`) is a hard time-bomb on the host, distinct from activity-based idle. Default: `None` (off). When set:

**MUST** actually stop billing. On AWS, this works because the AMI runs `shutdown -P +N` and the instance has `InstanceInitiatedShutdownBehavior=terminate`. On GCP, it works via `scheduling.max_run_duration` + `instance_termination_action=DELETE`. On Azure today, `auto_shutdown_seconds` runs `shutdown -P` only — the VM goes to "Stopped (not deallocated)" and KEEPS billing (`libs/mngr_azure/imbue/mngr_azure/backend.py:102`). This is a footgun that pattern-matches the AWS/GCP behavior visually.
...
**SHOULD** mirror agent records onto host tags so stopped-host discovery can reconstruct the agent list. Pattern: `mngr-agent-<agent_id>=<json_blob>` (AWS `backend.py:66-69` and `AGENT_TAG_PREFIX`). This SHOULD be lifted into `mngr_vps_docker` base so Azure/GCP/Vultr/OVH inherit it for free.
...
**MUST** generate per-provider-instance SSH keys, NOT per-host. Pattern: `mngr_ctx.profile_dir / "providers" / <backend> / <name> / "keys"` (`libs/mngr_vps_docker/imbue/mngr_vps_docker/instance.py:461-465`).
...
A cloud provider's most common implementation shape is "subclass `VpsDockerProvider` (`libs/mngr_vps_docker/imbue/mngr_vps_docker/instance.py:380+`), override 4-6 hooks, get the rest for free". The shared base supplies the parallel-SSH discovery, the host-record cache, the snapshot machinery, the `mngr destroy` aggregation, and the build-args parser scaffolding.

These are the override hooks, with the contract for each. The full list of hook points is at `libs/mngr_vps_docker/README.md:124` and below.

### 4.1 `_fetch_provider_instances() -> list[dict[str, Any]]`

**Contract.** Return the raw instance dicts (one per active VM) from the cloud's list-instances endpoint, filtered to those carrying `mngr-provider=<self.name>`. Called at most once per command via `_list_instances_cached` (`libs/mngr_vps_docker/imbue/mngr_vps_docker/instance.py:433-442`).
...
**Raise** when the backend state is *unknown* (credentials missing, API down, subscription unresolvable). Read paths warn-and-skip; the user sees the provider in the warning output. MUST pass curated `user_help_text` per backend; the default text tells the user to "start Docker" which is wrong for cloud auth failures. Pattern: `_azure_unavailable_error` (`libs/mngr_azure/imbue/mngr_azure/backend.py:36-53`). AWS and GCP today fall through to the default and SHOULD be fixed.
...
- **Lying about capability.** SSH `supports_shutdown_hosts=True` at `libs/mngr/imbue/mngr/providers/ssh/instance.py:104-106` while `stop_host` at `:184-190` raises `NotImplementedError`. Either flip the flag or implement.
- **Open-by-default ingress.** AWS `allowed_ssh_cidrs=("0.0.0.0/0",)` at `libs/mngr_aws/imbue/mngr_aws/config.py:127-135` while GCP/Azure fail-closed at `()`. Same flag name, opposite security posture.
- **Auto-shutdown that doesn't stop billing.** Azure today: `shutdown -P` halts the OS but the VM stays in "Stopped (not deallocated)" and keeps billing (`libs/mngr_azure/imbue/mngr_azure/backend.py:102`). Pattern-matches AWS/GCP but isn't equivalent.
- **Claiming `supports_snapshots=True` for cloud trio.** `docker commit` on a single VPS is not a portable snapshot; survives `mngr stop` but not `mngr destroy`. Either implement true disk snapshots, or split the flag into `supports_persistent_snapshots`, or document the gap.
- **`start_host(snapshot_id=…)` silent no-op.** AWS/Azure/GCP base path ignores the kwarg (`libs/mngr/imbue/mngr/interfaces/provider_instance.py:384-391`). The user thinks they restored work and lost it instead. Either honor or raise.
- **Discovery silently dropping a provider with bad creds in `mngr gc`.** `get_all_provider_instances` logs at DEBUG only (`libs/mngr/imbue/mngr/api/providers.py:211-213`). A `mngr gc` after expired AWS SSO reports "0 resources" with no warning. Bump to WARNING.
- **Tag-key style drift.** Modal uses `mngr_host_id` (underscore, `libs/mngr_modal/imbue/mngr_modal/instance.py:136`); everyone else uses `mngr-host-id`. Scripts walking tags need two code paths.
- **`--stop-host` silently leaving the VM running.** Azure/GCP today; inheriting the base `stop_host` while claiming `supports_shutdown_hosts=True`.
- **AWS `default_region` overriding `AWS_REGION` env var** because `boto3.Session(region_name=self.default_region)` is unconditional (`libs/mngr_aws/imbue/mngr_aws/config.py:175`). Defer to env first.
- **Generic `ProviderUnavailableError` help text.** AWS/GCP fall through to the default "start Docker" text — wrong advice for cloud auth failures. Pattern to copy: `_azure_unavailable_error`.

---

## 9. Local-vs-cloud taxonomy

| Provider | Category | Stop semantics | Snapshots | Network exposure |
|---|---|---|---|---|
| modal | hosted-sandbox | terminate sandbox (rehydrates from snapshot) | yes, persistent | Modal-managed; user opts in via `--cidr-allowlist` |
| aws | cloud-VM | container only by default; VM with `--stop-host` | docker-commit (single-host) | `0.0.0.0/0` default (anti-pattern) |
| azure | cloud-VM | container only by default; no VM-level stop | docker-commit (single-host) | `()` fail-closed |
| gcp | cloud-VM | container only by default; no VM-level stop | docker-commit (single-host) | `()` fail-closed |
| vultr | cloud-VPS | container only; VPS billing continues | docker-commit | `()` (inherits) |
| ovh | cloud-VPS | container only; monthly billing | docker-commit (btrfs) | per-VPS firewall |
| lima | local-VM | `limactl stop` (real VM stop) | no | localhost only (no guest→host port-forward) |
| docker | local-container | `docker stop` (container) | yes (docker commit) | currently `0.0.0.0:rand:22` (anti-pattern, should be `127.0.0.1`) |
| ssh | BYO | NotImplementedError (anti-pattern, should be no-op) | no | user-managed |

The contractual differences across rows: hosted-sandbox providers have no "host" abstraction the user pays for separately from the agent (Modal). Cloud-VM and cloud-VPS providers have a billable host that outlives the agent — stop semantics matter for cost. Local providers have no billing surface but MUST be safe by default (bind to localhost). BYO has neither a billing surface nor lifecycle ownership — it MUST be honest about that.
...
- [ ] Subclass `VpsDockerProvider` (or `ProviderInstanceInterface` directly for a non-VPS shape).
...
- [ ] Decide `supports_shutdown_hosts` honestly. If `True`, override `stop_host` to stop the VM; if `False`, accept that `--stop-host` will error loudly.
- [ ] Decide `supports_snapshots` honestly. If you can't implement persistent disk snapshots, mark this clearly in the README until the `supports_persistent_snapshots` split lands.
...
2. **Lift the `mngr-agent-<id>` tag-mirror pattern into `mngr_vps_docker` base?** AWS does it (`libs/mngr_aws/imbue/mngr_aws/backend.py:66-69` and `:636-651`); Azure/GCP would benefit the moment they grow VM-level stop. Lift it now to avoid copy-pasting later.
