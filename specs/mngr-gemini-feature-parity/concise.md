...
**PR2 (PARTIALLY LANDED): hook injection (readiness sentinel + workspace trust)**
...
- REMAINING: wire the `BeforeTool` permission auto-allow hook. `build_permission_auto_allow_hooks_config()` exists in `gemini_config.py` but is not yet installed by `provision()`. Add it to the system-settings file alongside the readiness hook (or use `--approval-mode yolo`/`-y`). Document the GitHub #20469 non-interactive policy-engine caveat.
...
- Acceptance (for the LANDED portion): agent reaches ready state without `--skip-trust`; sentinel file appears within N seconds; `--debug` output shows `Hook registry initialized with N hook entries` where N > 0.
