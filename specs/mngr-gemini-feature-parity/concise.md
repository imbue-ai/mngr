...
- Provides path helpers, atomic read/write with `.bak` backup, malformed-JSON-tolerant reads, env-var interpolation, two hook-config builders (`SessionStart` readiness sentinel + `BeforeTool` wildcard auto-allow), `merge_hooks_config`/`hook_already_exists` dedup-preserving merge, and the placeholder `GeminiDirectoryNotTrustedError` class.
...
- Replace the current `--skip-trust` workaround with `GEMINI_CLI_TRUST_WORKSPACE=true` in the agent's environment. Smoke-testing against Gemini CLI 0.42.0 confirmed this is the correct path: workspace-level `<project>/.gemini/settings.json` hooks DO fire under the env var (`Hook registry initialized with 2 hook entries`), but are silently dropped under `--skip-trust` (`Hook registry initialized with 0 hook entries`). The env var is Gemini's documented headless-automation trust mechanism (https://geminicli.com/docs/cli/trusted-folders/). Write hooks to `<workspace>/.gemini/settings.json`; no need to touch the user's `~/.gemini/trustedFolders.json` or `~/.gemini/settings.json`.
- Investigate whether `gemini hooks migrate --from-claude` reformats anything for free before reimplementing builders.
- Files: extend `gemini_config.py` (add a workspace-settings install helper), wire `GEMINI_CLI_TRUST_WORKSPACE=true` into `GeminiAgent`'s env, drop `--skip-trust` from default `cli_args`. Add `resources/readiness_hook.sh` only if a multi-statement hook command body becomes too awkward inline.
