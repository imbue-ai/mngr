...
**Non-goal.** This is not a replacement for unit tests. Unit-level tests (build-arg parsing, capability flags, config defaults, credential error classification) belong in `*_test.py` and should be added per the round-2 review's test-holes punch list. This proposal covers the release tier only.
...
4. **Tags every test-launched cloud resource** with `mngr-pytest-launched=true` and runs in a `MNGR_PROJECT_CONFIG_DIR=<tmpdir>` settings.toml that sets `auto_shutdown_seconds=3600`. Pairs with each provider's existing `pytest_sessionfinish` orphan scanner; **gates Vultr/OVH on adding one** (open hole, prior round-2 finding).
...
7. **Stop --stop-host: contract branch.**
   ...
   - If `provider.supports_shutdown_hosts is True` (everyone else): `mngr stop --stop-host` → assert exit 0. Verify `mngr list` shows host as `STOPPED`. Probe cloud-side: instance/VM is genuinely powered off (shape §1.4 right-real-stop).
     - **[INCONSISTENT]** AWS overrides `stop_host` and actually stops the EC2 instance; cost probe expected to succeed. Azure/GCP/Vultr/OVH inherit the base which only stops the container — cost probe **WILL FAIL** today (round-1 F-STOP-1, round-2 v2 status table). Either lock the test to fail (forcing the fix), or skip with `pytest.xfail("shape §1.4: <provider> claims supports_shutdown_hosts but only stops container; F-STOP-1")`. Recommendation: `xfail` until the override lands, then flip to a hard fail.
...
10. **Capability-flag honesty: volumes.** `provider.list_volumes()`:
    ...
    - If `supports_volumes is True`: expect a non-empty list including this host's mount.
      - **[INCONSISTENT]** AWS/Azure/GCP/Vultr/OVH inherit `supports_volumes=True` but `list_volumes()` returns `[]` (round-2 finding N-3, `libs/mngr_vps_docker/imbue/mngr_vps_docker/instance.py:2215-2219`). `xfail` until either the flag is flipped to `False` or the implementation lands.
...
13. **Discovery reflects the kill.** `mngr list` → host appears as `CRASHED` (shape §1.2). For AWS specifically, the tag-based offline-host reconstruction should fall back gracefully (round-1 F-LIST-1).
...
15. **Verify backend is clean.** Probe cloud-API: no instance with `mngr-host-id=<id>`, no leaked NIC/IP/EBS volume (shape §1.6). For AWS specifically: per-host KeyPair removed.
    - **[INCONSISTENT]** OVH `destroy_host` is "cancel at expiration", not "destroy now" (round-2 finding N-2 / F-L-6). OVH's verify step needs a separate "VPS will expire on date X" assertion, not "VPS is gone". Trip should `xfail` the immediate-cleanup assertion for OVH and assert the cancellation flag instead.
...
1b.2 **Both agents visible live.** `mngr list` shows two agents under `<host>` with distinct `agent_id`s. The live discovery path lifted to in-container scan in commit `8cca7406d` (`libs/mngr_vps_docker/imbue/mngr_vps_docker/instance.py:1506-1565`) makes this work uniformly across the VPS family.
1b.3 **Per-agent persisted records.** Probe `provider.list_persisted_agent_data_for_host(host_id)` → assert length 2 with distinct `id`s.
   ...
   - **[INCONSISTENT — capped]** AWS hits its EC2 50-tag cap at ~16 agents, then raises `NotImplementedError` (`libs/mngr_aws/imbue/mngr_aws/backend.py:677-685`). Trip 1b should fork a parametrized variant `trip1b_at_capacity` that loops to N=20 and asserts the right error fires. SSH provider has no persisted store at all (defaults to `[]`, `libs/mngr/imbue/mngr/interfaces/provider_instance.py:803-813`) — Trip 1b runs against SSH but step 1b.3 is `xfail`'d with reason "shape §1.8 SHOULD: SSH has no offline mirror".
...
4. **Verify host stopped/terminated/deleted.** Probe cloud-API for the provider-specific cost-stop state:
   ...
   - Azure: **[INCONSISTENT]** VM state `Stopped (not deallocated)` (still billing!). The shape doc says `auto_shutdown_seconds` MUST actually stop billing (§3.3). `xfail` until Azure managed-identity self-delete lands (round-1 F-D-5, round-2 v2 status table).
   - Vultr: **[INCONSISTENT]** OS halts but VPS keeps billing hourly (round-2 finding F-D-6). `xfail`.
...
5. **Restore.** `mngr create <new-name> --snapshot <snapshot_id>` → assert exit 0.
   - **[INCONSISTENT]** Only Modal and Docker honor `--snapshot` at create time. AWS/Azure/GCP/Vultr/OVH base path silently ignores the parameter (round-2 finding N-4). Shape §1.5 says either honor or raise — silent no-op is the worst option. `xfail`.
...
|---|---|---|
| No `[providers.<X>]` block at all | `ProviderEmptyError` if env-derivable; `ProviderUnavailableError` if creds missing | **[INCONSISTENT]** Vultr / OVH silently return `[]` instead of raising (round-2 N-10, N-11). Modal raises `ModalAuthError` (a `PluginMngrError`, not the contract error) — round-2 N-12. `xfail` these. |
| Bogus credentials | `ProviderUnavailableError` with curated `user_help_text` | **[INCONSISTENT]** Only Azure passes curated help text via `_azure_unavailable_error`. AWS/GCP fall through to the default "start Docker" text (round-1 F-CRED-1, round-2 v2 half-fixed). Assert the text mentions the provider-correct command (`aws configure` / `gcloud auth application-default login` / `az login` / `uvx modal token set`); `xfail` AWS+GCP+Modal. |
...
| `mngr gc` with a provider whose creds are missing | Visible WARN-level message; non-zero exit OR visible error in summary | **[INCONSISTENT]** `mngr gc` currently DEBUG-logs `ProviderUnavailableError` (round-1 F-LIST-4, `libs/mngr/imbue/mngr/api/providers.py:211-213`). `xfail`. |
...
| `mngr stop --stop-host` on a provider where `supports_shutdown_hosts is False` | `HostShutdownNotSupportedError` | **[INCONSISTENT]** SSH provider returns `supports_shutdown_hosts=True` but `stop_host` raises `NotImplementedError` — gate at `mngr/cli/stop.py:72` lets the call through and the user gets a stack trace (round-2 finding F-D-9). `xfail` SSH. |
...
| §3.8 Tagging conventions | T1 step 3 | Modal-vs-dash divergence (N-16). |
| §3.9 SSH key location | Implicit in T1 step 4 (must work) | Pinned by harness. |
| §3.10 Container exposure | Could add: scan VPS port 2222 from outside SSH cidr | **Not currently in any trip.** Suggested addition: T1 step 3a — outside-CIDR probe of container_ssh_port → expect refused. Today VPS-Docker family binds `0.0.0.0:2222` (round-2 finding F-D-3) so the probe would succeed on Vultr/OVH (no firewall) and fail correctly on AWS/Azure/GCP only when their firewalls are configured to deny outside the allowed CIDR. |
...
- The `auto_shutdown_seconds`-flows-through-to-cloud-API assertion (round-2 v2 punch list item 2.6) — easier to pin at unit level.
...
- `CleanupFailedGroup` raise-on-partial-failure (round-2 N-6) — hard to provoke at the release tier without a chaos hook. Belongs in unit tests with mocked destroy helpers.
...
| Trip step | Inconsistent on | Symptom | Source finding |
|---|---|---|---|
| T1.7 `--stop-host` cost-stop probe | Azure, GCP, Vultr, OVH | Inherited base only stops container; cloud-API state still `running` | round-1 F-STOP-1; round-2 v2 status |
| T2.4 auto-shutdown cost-stop probe | Azure, Vultr, OVH | OS halts but VM/VPS keeps billing | round-1 F-D-5; round-2 F-D-6 |
| T1.13 stopped-host visibility in `mngr list` | Azure, GCP, Vultr, OVH | Force-terminated instance vanishes from listing | round-1 F-LIST-1 |
| T1 step 1 `prepare` warns on open CIDR | AWS (open by default) | `("0.0.0.0/0",)` default, only warns; Azure/GCP fail-closed | round-1 F-D-1; round-2 unchanged |
| §3.10 container ingress probe (proposed) | Vultr, OVH (no firewall surface at all) | port 2222 reachable from public internet | round-2 N-1 |
| T4 missing-creds raises `ProviderUnavailableError` | Vultr, OVH, Modal | Vultr/OVH silently return `[]`; Modal raises `ModalAuthError` | round-2 N-10, N-11, N-12 |

### Medium — capability-flag honesty

| Trip step | Inconsistent on | Symptom | Source finding |
|---|---|---|---|
| T1.7 `supports_shutdown_hosts` honesty | SSH (True but raises NotImplementedError) | User-visible stack trace | round-1 F-OTHER-4; round-2 unchanged |
| T1.10 `supports_volumes` non-empty list | AWS/Azure/GCP/Vultr/OVH (True but empty) | `list_volumes()` returns `[]` | round-2 N-3 |
| T1.11/T3 `supports_snapshots` survives destroy | AWS/Azure/GCP/Vultr/OVH/Docker (docker-commit, dies with VPS) | `mngr snapshot list` after destroy returns nothing | round-1 F-SNAP-1 |
| T3.5 `--snapshot` at `mngr create` | AWS/Azure/GCP/Vultr/OVH | Silently no-ops | round-2 N-4 |
| T4 `mngr stop --stop-host` on SSH | SSH | NotImplementedError stack trace (not `HostShutdownNotSupportedError`) | round-2 F-D-9 |
| T1.16 prepare/cleanup idempotency | Cloud trio honest. Modal/Lima/Docker/SSH/Vultr skip — no command. | (n/a — skip cleanly) | round-1 F-DESTROY-1 |
| T1b.3 per-agent persistence cap | AWS (~16 agents max, EC2 50-tag wall); SSH (no persisted store) | NotImplementedError at cap; empty for SSH | shape §1.8; commit `446d6a964` |
| T1b.5 offline mirror shows N agents while stopped | Azure/GCP/Vultr/OVH (no offline mirror); SSH (no offline view) | `mngr list` shows zero agents on stopped host | shape §1.8 SHOULD; verified |
...
- **Vultr/OVH `pytest_sessionfinish` orphan scanner** is a prerequisite (round-1 still-open finding). The shared harness should refuse to run release tests against a provider whose conftest doesn't register one — `pytest_sessionstart` check.

---

## Open questions

1. **`xfail`-driven discovery vs. blocking the CI gate.** With the inconsistencies surfaced above, Trip 1 will `xfail` 6+ steps across 4 providers from day 1. Is that the right signal (CI green, divergence visible in `xfail` reasons)? Or should the trip be hard-gated on shape-doc compliance and providers run the trip in "stash" mode until fixed? Recommendation: `xfail` for the 2026-06-14 baseline, hard-fail after a documented deadline per finding.
