...
|---|---|---|
| `worker` | create_worker.py:6, launch-task/SKILL.md:2 | The mngr agent doing the delegated work |
| `sub-agent` | launch-task/SKILL.md:1,16,20,23 | Same as worker (inconsistent synonym) |
...
- **DOC/CODE DIVERGENCE**: `launch-task/SKILL.md` line 1 description says "Create a sub-agent"; step 0 says "Delegate ... to a sub-agent"; but the module docstring of `create_worker.py:6` uses "Worker-creation driver" and every internal variable/function name uses "worker". The description field is user-facing and displayed by the skills system to trigger the skill, while the implementation calls it a worker. The two documents use different vocabulary for the same entity.
...
The `-step-` segment is load-bearing: the chat progress view uses it to identify step records by ID alone when the backing `.tickets/` file is absent (tickets_parser.py comment on line 100).
...
- **Crystallized**: A skill produced by `crystallize-task` (or equivalent). Carries `metadata.crystallized: true` in SKILL.md frontmatter (spec-summary.md:44-45). Requires a `scripts/run.py` with PEP 723 header (validate_skill.py enforces this).
...
2. Requires `scripts/run.py` to be present (the validator enforces this — validate_skill_test.py:89-92)
...
- Checks: SKILL.md present, frontmatter valid, name matches directory, kebab-case name rules, description length ≤ 1024 chars, body ≤ 500 lines, `scripts/run.py` required if `metadata.crystallized: true`
...
- **DOC/CODE DIVERGENCE**: `crystallize-task/SKILL.md` success criteria (line 191) says: "agentskills.io-compliant, `metadata.crystallized: true`". The validator (validate_skill.py) enforces that `metadata.crystallized: true` requires `scripts/run.py`. However, `crystallize-task-worker/SKILL.md` Stage 3 (line 108) says the worker should follow spec-summary.md layout which says "Include `scripts/run.py` when the skill has deterministic steps" — i.e., it's optional. The validator enforces `run.py` unconditionally when `crystallized: true` is set. This is a validator-vs-docs inconsistency about whether crystallized skills must always have a `run.py`.
...
Key properties:
- **Name**: `system-services` (constant across all workspaces; branch is `mngr/<host_name>` to avoid collision — see agent_creator.py:552-557)
- **Label**: `is_primary=true` (the authoritative signal; never env-var-based)
- **Label**: `workspace=<host_name>`
- **Command**: `sleep infinity && claude` — Claude never actually starts; the `&& claude` is unreachable, present only to satisfy `assemble_command`'s claude-shaped invariant (apps/minds/README.md:37)
- **Created by**: `agent_creator.py` at line 541-563 (desktop client side during workspace creation)

### 6.2 Where `is_primary` Is Defined and Enforced

**Definition / stamping** (where `is_primary=true` is written):
- `apps/minds/imbue/minds/desktop_client/agent_creator.py:562` — the `mngr create` command includes `"is_primary=true"` as a label when creating the system-services agent
- `libs/mngr_imbue_cloud/imbue/mngr_imbue_cloud/cli/admin.py:532` — the ImbueCloud admin CLI also stamps `is_primary=true` when baking the pool host agent
...
- `apps/minds/imbue/minds/desktop_client/backend_resolver.py:711-722` — `list_known_workspace_ids()` filters for agents with both `workspace` and `is_primary` labels
- `apps/minds/imbue/minds/desktop_client/backend_resolver.py:724-741` — `list_active_workspace_ids()` same but excludes DESTROYED hosts
...
Source: apps/minds/README.md:37, agent_discovery.py:86-117 (chain for resolving CLAUDE_CONFIG_DIR including the host-env fallback introduced for `use_env_config_dir=True`).
...
| Name | Where | Status |
|---|---|---|
| `system-services` | agent_creator.py:487, README.md:37, FCT CLAUDE.md | The mngr agent name (constant) |
| "primary agent" | apps/minds/README.md:37, docs/design.md:20 | General description |
| "services agent" | server.py:479, 839-840 | Used in error messages and comments |
| "the is_primary agent" | server_test.py:330,866, DockviewWorkspace.ts | Test/comment shorthand |
| "main agent" | agent_creator.py:238 (comment) | Legacy term for the system-services agent |
| `SYSTEM_SERVICES_AGENT_NAME` | agent_creator.py:46 (imported constant) | Python constant for the name |
...
The constant value is `"system-services"` (defined in `backend_resolver.py`; referenced in agent_creator.py:487 comment and in `_build_system_services_create_command()`).
...
4. **The bootstrap agent name is `system-services` globally** but the branch is per-host (`mngr/<host_name>`). This means:
   ...
   - On ImbueCloud, the pool host has a pre-baked `system-services` agent that the lease/adopt path hydrates (agent_creator.py:567-583)
...
7. **`metadata.crystallized: true` requires `scripts/run.py` in the validator but spec-summary.md says `run.py` is optional**: this is an undocumented validator constraint that practitioners only discover by running `validate_skill.py`.
