...
| **launch mode** | *"Where your mind runs"* — the actual choice at creation (this Mac / a VM / the cloud). ⚠ raw labels (DOCKER/LIMA/VULTR/AWS/IMBUE_CLOUD) are developer-facing; needs friendly location/plan names | The minds-level `LaunchMode` enum mapping a user's choice to a provider + create templates (`--template <mode>`). | `apps/minds/.../primitives.py:62` (`LaunchMode`); `agent_creator.py` | enum labels not user-ready |
| **region** | *"Where in the world your mind's computer runs."* ⚠ "region" vs "location" | A provider-specific datacenter string; no shared type; 5 incompatible formats (`US-EAST-VA`/`ewr`/`us-east`/`us-east-1`/`us-west1-a` zone). | per-provider `config.py` (`default_region`) | needs shared type |
...
| **LLM auth mode** (code: `AIProvider`) | *"How your mind connects to and pays for its AI."* ⚠ frame as "AI connection/billing"; the 3 options need friendly names | LiteLLM virtual key (`IMBUE_CLOUD`) vs raw `ANTHROPIC_API_KEY` (`API_KEY`) vs OAuth subscription (`SUBSCRIPTION`). Now a typed minds enum `AIProvider`, decoupled from the compute provider; the mngr provisioning layer still receives it as env vars. | `apps/minds/.../primitives.py:72` (`AIProvider`) | code names it "AI provider" (taxonomy advises against) |
...
| **crystallized skill** | internal — *"learned automatically"* vs *"written by hand"* | A skill with `metadata.crystallized: true`; `scripts/run.py` is optional even then (validated only if present). | `.agents/skills/.../SKILL.md`; `validate_skill.py` | OK |
...
| **workspace color** (UI: "accent") | *"A color you pick per mind, shown in its titlebar, sidebar dot, and tiles."* (settled) | A per-workspace `#rrggbb` stored as the `color` label on the primary agent; surfaced via SSE `accent`/`accent_fg` → `--workspace-accent` CSS vars. | `apps/minds/.../workspace_color.py` | named "color" at storage, "accent" at render |
