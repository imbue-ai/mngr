...
- Listing hosts (`mngr list`, `_discover_host_records`) enumerates VPSes via `vps_client`/`_list_provider_vps_hostnames` as before, then for each VPS SSHes in and reads the single `host_state.json` (and any `agents/*.json`) directly from the host volume's mountpoint. Same number of SSH round-trips as today.
...
- **`libs/mngr_vps_docker/imbue/mngr_vps_docker/instance.py`**
  ...
  - Update `_discover_host_records_with_agents` / `_read_records_from_vps`: instead of calling `_get_existing_host_store(outer)` (which probed the state container), enumerate the per-VPS host volume(s) via `docker volume ls --filter name=mngr-host-vol-` and read each one through a `VpsDockerHostStore`. With 1:1, this is typically a single volume per VPS, but the code should be tolerant of zero or N matches (e.g. orphaned volumes after a partial cleanup).
