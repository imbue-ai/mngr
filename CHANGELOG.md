...
- Added: TMR `integrator_branch` event on `mngr tmr`'s structured stdout (`--format jsonl`/`json`) and a manually-dispatched `.github/workflows/tmr.yml` workflow with `mngr_user_id` / `additional_authorized_hosts` inputs.
...
- Changed: Bumped bundled Latchkey to 2.11.1 (from 2.8.0 earlier in the window) and switched minds to a single shared `latchkey gateway` subprocess for all agents, with password-protected `LATCHKEY_GATEWAY_LISTEN_PASSWORD`.
