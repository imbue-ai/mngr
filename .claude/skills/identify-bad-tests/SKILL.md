...
- **Wrong type, location, or marking** -- slow or network-using `*_test.py` unit tests, integration/acceptance/release tests in the wrong filename pattern or missing their `@pytest.mark.*`, or realistic tests running as plain unit/integration tests.
