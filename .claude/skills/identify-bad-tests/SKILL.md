...
Go gather all the context for the containing library (per instructions in CLAUDE.md). Even when the scan scope is a small subdirectory, you still need the whole containing library's context (style guide, primitives, data_types, interfaces, utils) to judge whether a test actually exercises the right behavior. Be sure to read the containing library's non_issues.md as well, and read the entire "# Testing" section of style_guide.md closely (in particular "High quality tests", "Testing without mocks", "Snapshot testing", "Test isolation", "Test quality", "Test organization", and "Types of tests").

**Unlike most work in this repo, this skill is specifically about the test files.** The CLAUDE.md rule to skip `_test.py` and `test_*.py` files on the first pass does NOT apply here: those files are exactly what you are reviewing. Read the test files in the scan scope in full, along with the production code they exercise and the shared fixtures/helpers in the relevant `conftest.py`, `testing.py`, and `mock_*_test.py` files (so you can tell a real shared mock implementation from an ad-hoc fake).
...
Examine every test in the scan scope, including:

- **Tautological or unfalsifiable assertions.** Assertions that cannot fail if the code is buggy: asserting a constructor field equals the value just passed in, asserting a constant equals itself, asserting on a value the test constructed rather than on the *effect* of the operation under test, or assertions comparing a value to itself after a no-op. Per the style guide, every assertion must be able to fail if the code under test has a bug.
- **"No exception raised" as the only check.** A test that calls the code and asserts nothing (or only that it did not raise) does not verify the result. Flag tests whose body has no meaningful assertion on the output/effect.
- **Tests of implementation details rather than behavior.** Tests that assert on internal call order, private attributes, or the exact way a result was computed, such that a correct refactor would break them. Prefer asserting on observable effects.
- **Error tests that don't pin the error.** `pytest.raises(Exception)` without checking the specific error type, or catching an error without verifying its type/message. The style guide requires verifying the specific error type/message.
- **Weak coverage-chasing tests.** Tests that appear to exist only to execute a line for coverage, with assertions too loose to catch a wrong answer (e.g. `assert result is not None`, `assert len(x) > 0` where the real contract is far more specific).
- **Missing cases.** For functions over collections, are empty / single-element / typical / boundary inputs all tested? For conditionals, is each branch tested with assertions specific to that branch? A test file that exercises only the happy path of branchy code is a candidate.
- **Mock misuse and faking.** Mocks/fakes that disconnect the test from real behavior: hand-rolled fakes that do not inherit from the interface they stand in for, mock implementations defined inline instead of in a shared `mock_*_test.py`, overuse of `types.SimpleNamespace` where a real object or shared mock belongs, or mocking the very thing under test so the assertion only checks the mock. (The bare `unittest.mock` / `monkeypatch.setattr` ratchets already count occurrences -- report the *semantic* damage, not the raw line.)
- **Flakiness and isolation hazards.** Reliance on wall-clock timing or sleeps to synchronize (should be polling with a timeout); non-unique IDs/names/constants where the style guide requires `uuid4().hex` and globally-unique literals; shared mutable state or order-dependence between tests; tests that don't isolate themselves (e.g. touching the real `HOME` or a shared on-disk location); live network requests from unit or integration tests.
- **Wrong type, location, or marking.** Unit tests (`*_test.py`) that are slow (> 5s), hit the network, or aren't really unit-scoped; integration/acceptance/release tests that live in the wrong filename pattern or are missing their `@pytest.mark.acceptance` / `@pytest.mark.release` marker; slow or realistic tests that should be acceptance/release tests but run as plain unit/integration tests.
- **Organization and naming.** Classes created solely to group test functions (forbidden -- use module-level `def test_...` functions); test names that are not descriptive of the specific behavior under test; `parametrize` used where cases have meaningfully different setup/assertions (or, conversely, near-duplicate test functions that should be a single `parametrize`).
- **Snapshot misuse.** Hand-written "expected" values that simply duplicate the implementation's logic (so the test and the code can drift together silently) where an `inline_snapshot` would be clearer and harder to fudge; oversized inline snapshots that should be hashed/file-based; or snapshots so large/opaque that a reviewer cannot tell whether they are correct.
