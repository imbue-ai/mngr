...
3. **Flaky tests** -- `neutral` check conclusion or `Flaky-recovered > 0`. Record which tests, whether they're `@flaky`-marked, and their run-counts. **Read the failed-attempt traceback (the summary's `## Failures` blocks) and classify each flake as a timeout vs an error.** This is the key fact a fixer needs -- the two have different right answers and the remedy can't be chosen without it -- so capture it as part of identifying the anomaly:
   - *Timeout* (`Failed: Timeout (>Ns) from pytest-timeout`): the test body or (often) its teardown exceeded the budget (default `--timeout=10` in `pyproject.toml`). Note *where* the time went, because that decides the fix: if the slow part isn't essential to what the test verifies, making the test faster is preferable to just raising `@pytest.mark.timeout(N)`. A rerun (`@flaky`) never addresses latency, so an existing `@flaky` mark on a timeout-flake is likely a mis-applied remedy worth flagging.
   - *Transient error* (e.g. Modal `The selected app is locked -- ... Please retry`, network blips): here a rerun genuinely is the matching remedy, so `@flaky` / offload's retry fits.
4. **Slow tests / slow jobs** -- compare job durations across runs for the same branch/event; flag outliers. For a slow *job*, break it down per-step (the log's per-step timestamps) before blaming tests -- e.g. `actions/checkout` with `fetch-depth: 0` does a full-history, all-branches fetch that can take minutes and dominate wall-clock (and varies run-to-run with GitHub's pack-serving). For slow *tests*, use the run-count table / `test_durations` artifacts.
...
7. **Repeated log noise / misconfiguration** -- a warning that repeats on a fixed interval through a job (not a one-off), even on a *passing* job, usually points to a misconfigured environment worth flagging. Canonical case: a job lacking a provider's credentials (e.g. `test-docker-electron` has no Modal token) logging `Discovery error from modal: Modal is not authorized` every ~10s. The warning text itself usually names the cause and how to silence it.
...
This skill identifies and characterizes anomalies; it does not fix them. Group findings by category, most to least important. For each: what, where (run/check URL + test or job name), how often across the sampled runs, and the diagnostic facts a fixer would need (for a flake: timeout-vs-error and where the time went; for a slow job: which step dominated). Separate infra noise from real regressions, and separate already-being-fixed items (check `gh pr list` for an open PR addressing it) from open ones. State how many runs you sampled and over what window. Be concise; do not dump raw logs.
