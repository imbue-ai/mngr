...
- "fct" means forever-claude-template, by far the most common such repo. To develop it from this checkout, run `just fct-worktree`: it stands up (or reuses) an independent fct checkout at `.external_worktrees/forever-claude-template` on the current mngr branch, needing no configuration (it clones fct directly). Commit fct changes inside that checkout and open a separate PR against forever-claude-template. A human dev can optionally set `FCT_DIR` in a gitignored `apps/minds/.env` to a local fct clone to make the clone faster.
