...
- While iterating locally on specific tests, use `just test-quick <path>::<test_name>` for rapid feedback on individual tests. To run all tests for a single project locally, pass the package path: `just test-quick libs/mngr`. You can also append `-m 'not tmux and not modal and not docker and not docker_sdk and not acceptance and not release'` to skip slow infrastructure tests when running locally. `just test-quick` drops coverage and runs xdist-parallel; use the underlying `uv run pytest ...` directly only if you need different flags than `test-quick` provides.
