...
- The `mngr` on your PATH must be the dev shim (`scripts/mngr`), installed via `just install-mngr-shim`. It routes `mngr` (and anything that shells out to it, e.g. minds) to the checkout you're working in instead of a stale global install. A pre-commit hook enforces this; if it fails, run `just install-mngr-shim` (then `hash -r`) -- do not bypass it.
