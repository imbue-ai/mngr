...
- "fct" means forever-claude-template, by far the most common such repo. To develop it from this checkout, run `just fct-worktree`: it creates an independent fct checkout at `.external_worktrees/forever-claude-template` on the current mngr branch, needing no configuration (it clones fct directly; errors if a checkout is already there). Commit fct changes inside that checkout and open a separate PR against forever-claude-template. If the user already has a local fct clone and `FCT_DIR` is unset, suggest they add `FCT_DIR=<path>` to a gitignored `apps/minds/.env` so `just fct-worktree` (and `bake-slice-dev`) reuse it instead of a fresh clone.
