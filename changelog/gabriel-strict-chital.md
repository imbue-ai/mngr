Tear out the unused refresh-event plumbing from `minds.desktop_client` and `mngr_forward`:
...
- Stop following the per-agent `refresh` event source in `ForwardStreamManager` (default sources are now just `services` and `requests`).
