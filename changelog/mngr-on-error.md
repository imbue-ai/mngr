...
- The Vultr VPS read path is wired as the canonical case -- a VPS that fails outer SSH now produces a `ProviderErrorInfo` in `result.errors` instead of being silently swallowed by `logger.warning`. Other providers accept the new parameter as a pass-through and can opt in incrementally.
