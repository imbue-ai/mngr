...
- Per-resource errors render identically to whole-provider errors: in `--format json` they appear in the existing `errors` array; in `--format jsonl` as `event:error` lines; and on stderr the CLI logs each `result.errors` entry as a warning regardless of output format. No new fields, no new arrays.
