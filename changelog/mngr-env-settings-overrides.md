...
- **Removed env vars:** `MNGR_COMMANDS_<CMD>_<PARAM>`, `MNGR_ENABLE_PARAMIKO_LOGGING`, `MNGR_AGENT_READY_TIMEOUT`, `MNGR_COMPLETION_CACHE_DIR`. The latter three are promoted to first-class config fields (`logging.enable_paramiko_logging`, `agent_ready_timeout`, `completion_cache_dir`) and remain settable via `MNGR__*`.
